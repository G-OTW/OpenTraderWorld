...
3. **Per-token module permissions** — each token grants *no access*, *read*, or *read + write* **per module**. Agents only discover the modules you granted.
...
2. **New token** — name it after the client (e.g. `My Agent`), set per-module permissions (or use *All read* / *All read+write* as a starting point).
...
- **`otw_write`** — create/update/delete operations (need *read + write*).
