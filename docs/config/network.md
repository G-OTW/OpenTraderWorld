...
Only enable it after your admin account exists and with a strong password. Turn on
[two-factor authentication](/config/security#totp) before you do. Keep `deploy/.env` secret.
Switch back to a private mode any time in Settings → Network.
:::

## The setup token {#setup-token}

On **LAN + HTTPS** and **Public**, the first-run wizard asks for a **setup token** before it
will create the first account.

The reason is a race: those modes answer the network, and the wizard is open until an account
exists. Without a token, whoever loads the page first becomes the administrator — which is a
real risk in the gap while DNS propagates, and again if a database volume is ever recreated.

Core mints the token at startup and prints it to its log:
...
Paste it into the extra field the wizard shows. A fresh one is minted on every restart, so an
abandoned token stops working as soon as the container restarts.

You will not see this if you installed with `./setup.sh`: it creates the account from
`deploy/.env` on first boot, before anything can reach the wizard. The token only appears
