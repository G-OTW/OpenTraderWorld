...
The catalog has its own release cycle, separate from the app: the header shows which **snapshot** is installed and a **Check for updates** button asks the publisher whether a newer one exists. Updating re-imports the catalog in place; your favorites survive and re-link to the new rows.
