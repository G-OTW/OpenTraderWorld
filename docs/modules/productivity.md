...
A webhook is only useful if the sending service can open a connection to your host. In `local` (and plain LAN) network mode nothing outside can — the page warns you when the current mode isn't reachable from the internet. Either change the mode in [Settings → Network](/config/network), or point a tunnel (e.g. Cloudflare Tunnel) at the host and keep the app otherwise private.
:::
