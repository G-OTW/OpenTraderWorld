...
The everyday modules: documents, tasks, goals, calendar, reminders, plus a few built for a trader's discipline.
...
- **Channels**: also deliver to **email, Telegram, Slack or Discord**, picked from the shared [notification channels](/config/settings#notifications). A reminder lists the channels RemindMe has been granted; the credentials and the grants live in Settings, once for the whole app.

## Webhooks {#webhooks}

Give any external service a private URL to **POST alerts into OpenTraderWorld**: charting and alerting platforms, broker notifications, uptime monitors, scripts, anything that can fire an HTTP request. The payload is received and routed into a module.
...
::: warning The sender has to be able to reach you
A webhook is only useful if the sending service can open a connection to your host. In `local` (and plain LAN) network mode nothing outside can, and the page warns you when the current mode isn't reachable from the internet. Either change the mode in [Settings → Network](/config/network), or point a tunnel (e.g. Cloudflare Tunnel) at the host and keep the app otherwise private.
:::
