...
The **Import** view takes a trade book you keep somewhere else (a broker export, another journal, a spreadsheet) and turns it into journal trades. No per-broker parser: CSV, TSV, JSON and Parquet all go through the same detection.
...
With a [broker account](/config/brokers) granted to the journal, **Pull from a broker** does the same import with no file: pick the account and a period, and the fills come back folded into positions, previewed before anything is written. There is no mapping to check, an API answers typed fields. Re-running a wider period refreshes the positions already imported instead of doubling them.
