...
**PnL is computed on read** and handles partially open positions. Cost basis is switchable between **weighted-average cost** (default) and **FIFO** (useful for tax export) — the choice is applied for real, both in the stats and in the trade form's live PnL preview.
