...
Run the setup script. It asks a few questions, generates strong secrets, writes the config, and starts everything:
...
The first start **builds the images from source** and takes a few minutes; later restarts are instant. (Prebuilt Docker Hub images are planned.)

::: tip Headless servers
The admin is created by the installer over the container's internal network, so you don't need a browser on the server. Note the printed password and sign in from any machine that can reach the app.
:::
