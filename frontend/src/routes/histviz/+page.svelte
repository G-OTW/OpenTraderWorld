<script>
  ...
  // Historical Data Visualization — chart stored OHLCV. Left: dataset picker and
  // managed indicator list, one at a time behind a tab strip (either list can grow
  // unbounded, so stacking both made the lower one unreachable). Right: toolbar
  // (chart type) over an ECharts pane.
  ...
  // Aside tab: 'dataset' first when nothing is charted yet, else straight to indicators.
  let tab = $state(saved.datasetId ? 'indicators' : 'dataset');
  ...
  <aside>
    ...
    <div class="tabs" role="tablist">
      ...
      <button
        ...
        class:active={tab === 'dataset'}
        onclick={() => (tab = 'dataset')}
      >
        {$t('histviz.picker.dataset')}
      </button>
    </div>
