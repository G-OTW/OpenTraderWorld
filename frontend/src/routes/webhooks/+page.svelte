<script>
  ...
  // existing module (RemindMe → notification, Automator → a workflow run). Sender-agnostic:
  ...
  let webhooks = $state([]);
  let targets = $state([]);
  // Workflows a target that needs one can be bound to (Automator).
  let workflows = $state([]);
  ...
  async function reload() {
    ...
    // Same: an empty list only costs the picker its options.
    try {
      workflows = (await automatorApi.list())?.workflows ?? [];
    } catch {
      workflows = [];
    }
  }
  ...
  const workflowOptions = $derived(workflows.map((w) => ({ value: w.id, label: w.name })));
  // A target declaring `needs` is only storable once the object is picked.
  const needsWorkflow = $derived(
    targets.find((x) => x.id === formTarget)?.needs === 'workflow_id'
  );
  const workflowName = (id) => workflows.find((w) => w.id === id)?.name ?? '';
  ...
  function openEdit(wh) {
    ...
    formWorkflow = wh.config?.workflow_id ?? '';
    formError = '';
    modalOpen = true;
  }
  ...
  async function save() {
    ...
    if (needsWorkflow && !formWorkflow) {
      formError = $t('webhooks.err.workflowRequired');
      return;
    }
    saving = true;
    formError = '';
    // The config belongs to the target: switching away from Automator drops the binding.
    const config = needsWorkflow ? { workflow_id: formWorkflow } : {};
    try {
      if (editing) {
        await webhooksApi.update(editing.id, { name: formName, target: formTarget, config });
      } else {
        const r = await webhooksApi.create({ name: formName, target: formTarget, config });
        ...
        <tbody>
          {#each webhooks as wh (wh.id)}
            <tr class:off={!wh.enabled}>
              ...
              <td>
                {targetLabel(wh.target)}
                {#if wh.config?.workflow_id}
                  <span class="hintline">{workflowName(wh.config.workflow_id)}</span>
                {/if}
              </td>
              <td
                class="mono"
  ...
  <div class="form">
    ...
    {#if needsWorkflow}
      {#if workflowOptions.length}
        <Select
          label={$t('webhooks.workflow')}
          options={workflowOptions}
          bind:value={formWorkflow}
          placeholder={$t('webhooks.workflowPlaceholder')}
          hint={$t('webhooks.workflowHint')}
          required
        />
      {:else}
        <p class="hint">{$t('webhooks.noWorkflows')}</p>
      {/if}
    {/if}
...
<style>
  ...
  /* Second line under a target: which workflow the endpoint is bound to. */
  .hintline {
    display: block;
    color: var(--dim);
