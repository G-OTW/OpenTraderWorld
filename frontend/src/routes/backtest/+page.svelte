<script>
  ...
  // ── Strategies ──
  // Loading anything over an edited draft would drop it — ask first.
  const confirmDiscard = () => !dirty || confirm($t('backtest.header.discardConfirm'));
  ...
  // Save = create or update. An unnamed draft needs the modal first (name + tags).
    ...
    <div class="dock-bar">
      ...
      <div class="dock-acts">
        ...
        {:else}
          ...
          {#if runs.length}
            <button class="sm" onclick={clearHistory}>{$t('backtest.page.clearHistory')}</button>
          {/if}
        {/if}
        <button
          class="dock-toggle"
