<script>
  ...
  async function run() {
    ...
    try {
      ...
      collapsed = true;
      // The run was recorded server-side under a generated name — reflect it in the list.
      loadRuns().catch(() => {});
    } catch (e) {
      error = e.message;
    } finally {
  ...
  async function openLiveReport() {
    ...
    goto(`/backtest/report/${result.run_id}?from=normal`);
  }
  ...
  async function clearHistory() {
    if (!confirm($t('backtest.page.clearHistoryConfirm'))) return;
    try {
...
{#snippet historyList()}
  <section class="history">
    ...
    {:else}
      <div class="hbar">
        <span class="muted">{$t('backtest.page.historyHint')}</span>
        <button class="ghost sm" onclick={clearHistory}>{$t('backtest.page.clearHistory')}</button>
      </div>
    {/if}
    {#each runs as r (r.id)}
...
<style>
  ...
  /* Expert mode is the gateway "one button" — neutral tile, no accent fill. */
  .expert {
