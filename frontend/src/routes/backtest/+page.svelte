<script>
  ...
  // The grid engine ladders one instrument: it reads the first dataset and never simulates the
  // rest, so the picker is capped at one and a basket left over from another kind is trimmed
  // on the switch. The API rejects the combination too; this keeps the UI from proposing it.
  const maxDatasets = $derived(settings.kind === 'grid' ? 1 : 8);
  $effect(() => {
    if (datasetIds.length > maxDatasets) datasetIds = datasetIds.slice(0, maxDatasets);
  });
...
<div class="page" bind:this={pageEl}>
  ...
  <section class="work">
    ...
    {:else}
      ...
      <div class="step-body">
        {#if step === 'data'}
          <DataStep {datasets} bind:datasetIds {alignment} {aligning} max={maxDatasets} />
