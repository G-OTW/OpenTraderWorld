<script>
  ...
  // under an auto name, then navigate to the report route.
  async function openLiveReport() {
    ...
    try {
      ...
      goto(`/backtest/report/${id}?from=expert`);
    } catch (e) {
      error = e.message;
    }
  }
  ...
  <div class="layout">
    ...
    <main class="center">
      {#if showResult && result}
        ...
        {#if !chartHidden}
          <div class="chart-box"><ResultChart {bars} trades={result.trades} equity={result.equity} benchmark={result.benchmark ?? []} /></div>
        {/if}
