<script>
  ...
  import { goalsApi, progress, fmtDate, daysLeft, status } from '$lib/modules/goals/api.js';
  ...
  // Deleting a goal takes its KPIs with it and is not undoable, so it goes through
  // ConfirmModal rather than the browser's confirm().
...
<div class="goals">
  ...
  {#if loading}
    <!-- A card grid, so the skeleton is a grid of cards — the page keeps its width and
         rhythm rather than collapsing to a line and snapping back. -->
  ...
  {:else}
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <a class="chip" href="/goals/stats" title={$t('goals.stats.title')}>
          <Icon name="pie-chart" size={12} /> {$t('goals.stats.title')}
        </a>
      </div>
    </div>
    ...
    {#if shown.length === 0}
      <!-- Filtered empty: the fix is to widen the filter, not to add a goal. -->
      <EmptyState icon="filter" compact title={$t('goals.noneForFilter', { filter: statusLabel(filter) })}>
        {#snippet action()}
          <button class="ghost" onclick={() => { filter = 'all'; catFilter = 'all'; }}>{$t('common.clear')}</button>
        {/snippet}
      </EmptyState>
    {:else}
    <div class="grid">
...
<Modal open={!!detail} title={detail?.name ?? ''} size="md" onclose={() => (detail = null)}>
  {#if detail}
    ...
    <div class="detail" class:tinted={!!detail.color} style:--goal-color={detail.color || null}>
...
<ConfirmModal
  ...
  message={pendingDelete ? $t('goals.confirmDelete', { name: pendingDelete.name }) : ''}
...
<style>
  ...
  /* The stats link is a chip like its neighbour, so the anchor has to shed the
     browser's link styling to sit flush with the sort button. */
  .toolbar-right a.chip {
  ...
  /* A skeleton the shape of a goal card: title, sub-line, progress bar. */
  .sk-card {
    display: flex;
  ...
  /* A goal's own colour paints its progress bar and its left edge. The reached/overdue
     edges above still win — status is the more urgent signal. */
  .tinted .bar .fill {
    background: var(--goal-color);
  }
  .card.tinted.open {
    border-left: 1.5px solid var(--goal-color);
  }
  .card-foot {
    display: flex;
    align-items: center;
