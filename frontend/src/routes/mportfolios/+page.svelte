  ...
  <header>
    ...
    {#if tab === 'portfolios'}
      <button class="reload" onclick={refresh} disabled={refreshing}>
        <span class="ico" class:spinning={refreshing}><Icon name="refresh-cw" size={13} /></span>
        {refreshing ? $t('mportfolios.refreshing') : $t('common.refresh')}
      </button>
    {/if}
  </header>
...
<style>
  ...
  /* The icon keeps turning while the fetch runs, so a slow refresh never reads as stuck. */
  .ico {
    display: inline-flex;
  }
  .ico.spinning {
