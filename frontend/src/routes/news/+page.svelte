<script>
  ...
  function timeSince(s) {
    ...
    if (secs < 60) return $t('news.time.justNow');
    const mins = Math.floor(secs / 60);
    if (mins < 60) return $t('news.time.minsAgo', { mins });
    const hrs = Math.floor(mins / 60);
    if (hrs < 24) return $t('news.time.hoursAgo', { hrs });
    const days = Math.floor(hrs / 24);
    return $t('news.time.daysAgo', { days });
  }
  function host(url) {
    try {
