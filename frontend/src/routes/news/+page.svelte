<script>
  ...
  import PickSelect from '$lib/modules/news/PickSelect.svelte';
  ...
  let quotas = $state({}); // feed id → request-quota usage (tracked API sources only)
  let items = $state([]);
  ...
  async function loadFeeds() {
    ...
    // Quota usage rides along with every source refresh; non-fatal if it fails.
    newsApi
      .quotas()
      .then((q) => (quotas = q))
      .catch(() => {});
  ...
  // ── Favourite-strip drag & drop ──
  // `position` is a plain i32 server-side, so a drop renumbers the whole list
  // 0..n-1 and patches only the rows whose index actually moved.
  ...
  async function onDrop(e, target) {
    ...
    const fromIdx = list.findIndex((d) => d.id === from);
    const toIdx = list.findIndex((d) => d.id === target.id);
    if (fromIdx < 0 || toIdx < 0) return;
    const [moved] = list.splice(fromIdx, 1);
    list.splice(toIdx, 0, moved);
    dashboards = list.map((d, i) => ({ ...d, position: i })); // optimistic

    const changed = list.map((d, i) => ({ d, i })).filter(({ d, i }) => d.position !== i);
    try {
      await Promise.all(changed.map(({ d, i }) => newsApi.updateDashboard(d.id, { position: i })));
    } finally {
      await loadDashboards();
    }
  }
  ...
  // Items carry no colour of their own: the dot comes from the source they were
  // fetched by, which is always one of the current dashboard's sources.
  const colorByFeed = $derived(new Map(feeds.map((f) => [f.id, f.color])));
  function feedColor(id) {
    return colorByFeed.get(id) || '';
  }
  ...
  // Request-quota display for tracked API sources: fill percent (null = unlimited,
  // no bar to fill) and a "used / max · period" label.
  function quotaPct(q) {
    if (q.max_requests == null) return null;
    return Math.min(100, Math.round((q.used / Math.max(q.max_requests, 1)) * 100));
  }
  function quotaLabel(q) {
    const per = $t(`common.period.${q.period}`);
    const max = q.max_requests == null ? '∞' : q.max_requests;
    return `${q.used} / ${max} · ${per}`;
  }
</script>
...
<div class="news-module">
  ...
  <aside class="sidebar">
    ...
    <div class="dash-picker" use:clickOutside={() => (showDashMenu = false)}>
      <button class="dash-current" title={selected?.name || ''} onclick={() => (showDashMenu = !showDashMenu)}>
      ...
      <button
        ...
        disabled={refreshingAll || !selectedId}
        title={$t('news.sidebar.refreshDashboard')}
      >{#if refreshingAll}…{:else}<Icon name="refresh-cw" size={13} />{/if}</button>
      {#if selected}
        <button
          class="play"
        ...
        <div class="dash-menu">
          {#each dashboards as d (d.id)}
            <div class="dash-row" class:active={d.id === selectedId}>
              <button class="dash-pick" title={d.name} onclick={() => selectDashboard(d.id)}>
    ...
    <div class="feed-controls">
      <div class="kind-tabs">
        <button class="dir" onclick={newFeed} title={$t('news.sidebar.addSource')}><Icon name="plus" size={13} /></button>
        <PickSelect
          bind:value={kindFilter}
          title={$t('news.sidebar.all')}
          options={[
            { value: '', label: $t('news.sidebar.all') },
            { value: 'rss', label: 'RSS' },
            { value: 'api', label: 'API' }
          ]}
        />
      ...
      <div class="sort-controls">
        <PickSelect
          bind:value={sortBy}
          title={$t('news.sidebar.sortBy')}
          options={[
            { value: 'name', label: $t('news.sidebar.sortName') },
            { value: 'count', label: $t('news.sidebar.sortItems') }
          ]}
        />
        <button
          class="dir"
    ...
    <div class="feed-list">
      {#each filteredFeeds as f (f.id)}
        ...
        <div class="feed">
          <div class="feed-main">
            {#if f.color}<span class="src-dot" style:background={f.color}></span>{/if}
          ...
          {/if}
          {#if f.kind === 'api' && quotas[f.id]}
            {@const qta = quotas[f.id]}
            {@const pct = quotaPct(qta)}
            <div
              class="feed-quota"
              title={$t('news.feed.quotaTitle', { usage: quotaLabel(qta) })}
            >
              {#if pct != null}
                <div class="qbar">
                  <div class="qfill" class:warn={pct >= 70} class:over={pct >= 90} style:width="{pct}%"></div>
                </div>
              {/if}
              <span class="qtext">{quotaLabel(qta)}</span>
            </div>
          {/if}
  ...
  <main class="workarea">
    <!-- Favorited dashboards: always-present strip above the filters. -->
    <div class="fav-strip">
      <button class="fav-add" onclick={newDashboard} title={$t('news.dashboard.newDashboard')}><Icon name="plus" size={14} /></button>
      <div class="fav-scroll">
        {#each favorites as d (d.id)}
          <button
            class="fav-tag"
            class:active={d.id === selectedId}
            class:dragging={d.id === dragId}
            class:drop={d.id === dropId}
            draggable="true"
    ...
    <div class="filters">
      ...
      <PickSelect
        bind:value={fSourceType}
        onchange={applyFilters}
        title={$t('news.filters.allTypes')}
        options={[{ value: '', label: $t('news.filters.allTypes') }, ...sourceTypes.map((s) => ({ value: s, label: s }))]}
      />

      <!-- Multi-select sources: checkboxes in a dropdown. -->
      <div class="src-picker" use:clickOutside={() => (showSourceMenu = false)}>
    ...
    <div class="items" class:line={viewMode === 'line'}>
      {#each items as it (it.id)}
        <article class="item">
          <div class="item-head">
            {#if feedColor(it.feed_id)}<span class="src-dot" style:background={feedColor(it.feed_id)}></span>{/if}
...
<Modal bind:open={showForm} size="lg" title={editingFeed ? $t('news.feed.editSource') : $t('news.feed.addSource')}>
...
<style>
  ...
  /* Per-source identifying colour, on the sidebar row and on each of its items. */
  .src-dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
  ...
  /* The + stays pinned left; only the tags scroll, however many there are. */
  .fav-scroll {
    display: flex;
    align-items: center;
    gap: 6px;
  ...
  /* ── Request-quota bar (tracked API sources) ── */
  .feed-quota {
    display: flex;
    align-items: center;
    gap: 6px;
  ...
  /* Auto-refresh then the view toggle, pinned to the right of the filter row. */
  .link.auto {
    margin-left: auto;
  }
  .src-picker {
  ...
  /* Continuous list: rows separated by hairline filets, no floating cards. */
  .items {
    overflow-y: auto;
    padding: 0;
    display: flex;
