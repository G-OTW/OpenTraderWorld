<script>
  ...
  // Poll jobs while any are active (and the download tab is visible).
  let timer;
  $effect(() => {
    clearInterval(timer);
    const active = jobs.some((j) => j.status === 'queued' || j.status === 'running');
    if (tab === 'download' && active) {
      timer = setInterval(() => loadJobs().catch(() => {}), 1500);
    }
...
<div class="page">
  ...
  {#if error}<p class="err" title={$t('histdata.jobs.clickToCopy')} use:copyLog={error}>{error}</p>{/if}

  <div class="content">
    {#if tab === 'download'}
      <section class="card">
        <h2>{$t('histdata.page.newDownload')}</h2>
        <DownloadForm {providers} onqueued={onQueued} />
      </section>
      <section class="card">
    ...
    {:else}
      <section class="card">
        <ProviderSettings {providers} onchanged={loadProviders} />
      </section>
    {/if}
  </div>
</div>
