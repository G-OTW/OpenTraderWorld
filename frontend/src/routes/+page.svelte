<script>
  ...
  // Full display list: Modules page first, then user pages, default sorted first.
  const display = $derived(
    doc ? pagesForDisplay(doc, installedList, $t) : { pages: [], defaultId: null }
  );
  const activePage = $derived(display.pages.find((p) => p.id === doc?.activePageId) ?? null);
  const layout = $derived(activePage?.layout ?? null);
  // The Modules page is editable like the others; only its page settings (its module set
  // is the installed set) and its deletion stay off limits.
  ...
  $effect(() => {
    ...
    dashboardApi
      ...
      .then((saved) => {
        ...
        // Seed the Modules page on first sight, reconcile it with the installed set after.
        if (ensureModulesPage(doc, installedList, $t)) save();
        openDefault();
      })
      .catch((e) => {
        loadError = e.message;
        doc = normalizeDoc(null);
        ensureModulesPage(doc, installedList, $t);
        openDefault();
      });
  });
  ...
  function openDefault() {
    const { defaultId } = pagesForDisplay(doc, installedList, $t);
  ...
  function openEditPage() {
    if (isBuiltin) return; // the Modules page's module set is the installed set
  ...
  function deleteActivePage() {
    if (isBuiltin) return; // the Modules page can't be deleted
  ...
  >
    {#snippet actions()}
      {#if activePage}
        ...
        <Button variant={editing ? 'primary' : 'secondary'} onclick={toggleEdit} loading={saving}>
        ...
        <div class="row-wrap" animate:flip={{ duration: flipMs }}>
          ...
          {:else}
            ...
            >
              ...
              {:else}
                <!-- Fill the last row so the ruled grid stays rectangular (reference
                     .cell-empty): a --bg cell spanning the remaining columns. -->
                {@const used = row.items.reduce((s, it) => s + (it.span || 0), 0)}
                {#if used < COLS}
                  <div class="cell cell-empty" style:grid-column={`span ${COLS - used}`}></div>
                {/if}
              {/if}
            </div>
          {/if}
...
<style>
  ...
  /* View mode: modules read as one continuous ruled grid (reference .hairline-grid),
     not floating cards. Filets are drawn as outlines on the cells themselves (which fill
     the hairline gaps exactly), NOT as a --border underlay showing through gaps — an
     underlay renders as a grey slab wherever cells don't cover the container.
     Editing mode keeps gaps for DnD affordance. */
  .rows:not(.editing) {
    gap: var(--hairline);
  ...
  /* View mode: filet-separated cells, no radius, hairline gap. Rows collapse to their
     content (grid-auto-rows); each cell outlines itself, so lines only ever appear at
     real cell edges. */
  .rows:not(.editing) .grid {
    gap: var(--hairline);
    min-height: 0;
  ...
  .rows:not(.editing) .cell {
    background: var(--bg);
    outline: var(--hairline) solid var(--border);
  }
  /* Empty trailing cell keeps the ruled grid rectangular. */
  .cell-empty {
    background: var(--bg);
  }
  ...
  /* Reference .module-tile: no border, no radius; separation is the hairline grid.
     Internal rhythm from components.css — icon, then title (margin 14px 0 3px), desc. */
  .tile {
    display: flex;
    flex-direction: column;
  ...
  /* Editing keeps a per-cell filet so gapped, draggable cells stay legible. */
  .tile.edit {
    cursor: grab;
    border: 0.5px solid var(--border);
  }
  /* Icon: 17px, top-left, muted. Gold is reserved for the active nav / promoted. */
  .tile-icon {
    display: inline-flex;
    color: var(--muted);
  }
