<script>
  ...
  import BrokerTaxModal from '$lib/modules/taxcalc/BrokerTaxModal.svelte';
  ...
  // With no profile yet the picker has nothing to offer, so its single entry opens the editor.
  const NEW_PROFILE = '__new';
  let profileOptions = $derived(
    profiles.length
      ? profiles.map((p) => ({ value: p.id, label: p.name }))
      : [{ value: NEW_PROFILE, label: $t('taxcalc.page.addProfile') }]
  );
  function pickProfile(v) {
    if (v === NEW_PROFILE) newProfile();
    else profileId = v;
  }
  ...
  // One dialog, two callers. ConfirmModal exists to replace the browser's confirm(),
  ...
  function removeScenario(id) {
    askConfirm($t('taxcalc.page.confirmDeleteScenario'), async () => {
      try {
  ...
  function removeProfile(id) {
    askConfirm($t('taxcalc.page.confirmDeleteProfile'), async () => {
...
<div class="page">
  <header>
    ...
    <p class="disclaimer">{$t('taxcalc.page.disclaimer')}</p>
  </header>
  ...
  <div class="cols">
        ...
        {#each profiles as p (p.id)}
          <div class="profile {p.id === profileId ? 'active' : ''}">
            ...
            <div class="actions">
              ...
              <button class="link red icon" aria-label={$t('common.delete')} title={$t('common.delete')} onclick={() => removeProfile(p.id)}>
    ...
    <section class="calc">
      <div class="row">
        <div class="fld grow">
          <span class="fld-label">{$t('taxcalc.page.profile')}</span>
          <Dropdown
            value={profileId}
            options={profileOptions}
            placeholder={$t('taxcalc.page.selectProfile')}
            ariaLabel={$t('taxcalc.page.profile')}
            onpick={pickProfile}
          />
        </div>
        <label>
      ...
      <div class="row">
        ...
        <div class="toggles">
          ...
          >
            <span class="source-action-icon"><Icon name="book-open" size={15} strokeWidth={1.8} /></span>
            {$t('taxcalc.page.loadTradingJournal')}
          </button>
          <!-- Same three numbers, read from the account instead of the book. -->
          <button class="source-action" onclick={() => (brokerOpen = true)}>
            <span class="source-action-icon"><Icon name="briefcase" size={15} strokeWidth={1.8} /></span>
            {$t('taxcalc.page.loadFromBroker')}
          </button>
        </div>
      </div>
        ...
        <div class="history">
          ...
          {#each scenarios as s (s.id)}
            <div class="hist-row">
              ...
              <button class="link red icon" aria-label={$t('common.delete')} title={$t('common.delete')} onclick={() => removeScenario(s.id)}>
                <Icon name="trash" size={14} />
              </button>
            </div>
          {/each}
...
<!-- No title: the message is a full question, and taxcalc has no "Delete" string of its
     own — inventing one would mean inventing seven translations. -->
<ConfirmModal
...
<style>
  ...
  /* Full-width row under the title, so it reads as a footnote to the page, not to the subtitle. */
  .disclaimer {
    flex-basis: 100%;
    max-width: 78ch;
  ...
  /* The Dropdown is a button, not a labelable control — the caption is a plain span. */
  .fld-label {
    line-height: 1.4;
  }
  .toggles {
  ...
  .source-action:hover:not(:disabled) {
    background: var(--surface-2);
    border-color: color-mix(in srgb, var(--accent) 38%, var(--border-control));
  }
  ...
  /* Icon-only delete: the label is the aria-label, so the button is just the glyph. */
  .link.icon {
    display: inline-flex;
    align-items: center;
    padding: var(--space-1);
