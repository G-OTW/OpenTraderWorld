<script>
  ...
  import LinkPortfolioModal from '$lib/modules/wealth/LinkPortfolioModal.svelte';
  ...
  // Every category in use, filter-independent — the source for the category picker.
  ...
  // How the time series is drawn: 'line' | 'bars'. Curve view only.
  let chartShape = $state('line');
  // Collapsed category groups in the asset list (Set of category keys).
  let collapsed = $state(new Set());

  let tSearch = $state('');
  // Type/category are multi-select (empty array = no filter) and drive BOTH the table and
  // the breakdown above: one filter for the page, so the chart always describes the rows
  // you are looking at. Search and sort stay table-only (the chart has no text to match).
  let tType = $state([]);
  let tCategory = $state([]);
  ...
  // Server filter, from the page's multi-selects. Comma-joined lists; empty = no filter.
  const filter = $derived({ asset_type: tType.join(','), category: tCategory.join(',') });
  // Category vocabulary for the picker. Derived from the UNFILTERED asset list: taking it
  // from `assets` would shrink the options to whatever is currently selected, leaving no way
  // to widen the filter again.
  const categories = $derived([...new Set(allCategories)].sort((a, b) => a.localeCompare(b)));
  const cur = $derived(settings.display_currency);
  // The headline is a fixed-width rail tile, so a big number (CNY, JPY, IDR) would run off
  // the page. Shrink the glyphs to fit instead of clipping or wrapping: the mono face has a
  // ~0.6em advance, and the tile offers 220px minus its 16px padding on each side.
  const NW_MAX = 28;
  const NW_MIN = 13;
  const nwText = $derived(fmtMoney(bd?.net_worth, cur));
  const nwSize = $derived(
    Math.max(NW_MIN, Math.min(NW_MAX, 188 / Math.max(1, nwText.length * 0.6)))
  );
  ...
  async function reload() {
    ...
    // Refresh the picker vocabulary from the full list — but only when nothing is filtered
    // out, otherwise we would be learning it from a subset. Any add/edit/delete reloads
    // with the filter cleared often enough to keep this current, and a newly typed category
    // is added below on save.
    if (tType.length === 0 && tCategory.length === 0) {
      allCategories = assets.map((a) => a.category).filter(Boolean);
    }
  }
  // Re-fetch when the page filter or chart granularity changes: type/category now scope the
  // breakdown too, so the chart above always matches the rows below.
  $effect(() => {
    void filter;
    void granularity;
    if (!loading) reload();
  ...
  const tableAssets = $derived.by(() => {
    ...
    let rows = assets.filter((a) => {
      if (tType.length && !tType.includes(a.asset_type)) return false;
      if (tCategory.length && !tCategory.includes(a.category || '')) return false;
  ...
  async function saveAsset(payload) {
    ...
    showAsset = false;
    // A category typed here may not be in the vocabulary yet, and `reload` won't learn it
    // while a filter is on (it would be reading a subset).
    if (payload.category) allCategories = [...allCategories, payload.category];
    await reload();
  }
  function openImport() {
    showAsset = false;
  ...
  async function setCurrency(v) {
    settings = await wealthApi.updateSettings({ display_currency: v });
    await reload();
  }
</script>
...
<div class="wealth">
    ...
    <div class="head-right">
      <div class="cur-select">
        ...
        <Dropdown
          value={cur}
          onpick={setCurrency}
          ariaLabel={$t('wealth.page.display')}
  ...
  {:else if view === 'assets'}
    <section class="top">
      <div class="card chart-card">
        <div class="chart-head">
          ...
          <div class="chart-toggles">
            ...
            {#if chartView === 'curve'}
              <div class="toggle" role="group" aria-label={$t('wealth.page.chartShapeGroup')}>
                <button class:active={chartShape === 'line'} onclick={() => (chartShape = 'line')}>{$t('wealth.page.line')}</button>
                <button class:active={chartShape === 'bars'} onclick={() => (chartShape = 'bars')}>{$t('wealth.page.bars')}</button>
              </div>
        ...
        {#if chartView === 'curve'}
          <NetWorthChart points={bd?.points ?? []} currency={cur} {granularity} shape={chartShape} />
        {:else}
      ...
      <div class="rail">
        <div class="stat big">
          ...
          <strong style="font-size: {nwSize}px">{nwText}</strong>
        </div>
        <!-- Owned and owed, before they are netted. A headline that shows only the
             difference is a number the reader cannot check. -->
        {#if bd?.liabilities_value > 0}
          <div class="stat">
            <span class="lbl">{$t('wealth.page.owned')}</span>
            <strong class="up">{fmtMoney(bd.assets_value, cur)}</strong>
          </div>
          <div class="stat">
            <span class="lbl">{$t('wealth.page.owed')}</span>
            <strong class="down">{fmtMoney(bd.liabilities_value, cur)}</strong>
          </div>
        {/if}
        <div class="stat">
        ...
        {/if}
        {#if bd?.stale?.length}
          <!-- A valuation that has aged past what its owner asked for. The failure mode this
               names is silence: a house valued three years ago is wrong, and no tracker says
               so. Names the oldest, because that is the one to look at. -->
          <p class="warn stale">
            <Icon name="clock" size={13} />
            {$t('wealth.page.staleWarning', {
              count: bd.stale.length,
              name: bd.stale[0].name,
              days: bd.stale[0].age_days ?? 0
            })}
          </p>
        {/if}
      </div>
    ...
    <!-- Only the genuinely empty book gets the empty state. `assets` is server-filtered now,
         so a filter that matches nothing must still render the controls — otherwise it hides
         the very buttons needed to clear it. That case falls to `noAssetsMatch` in the table. -->
    {#if assets.length === 0 && tType.length === 0 && tCategory.length === 0}
    ...
    {:else}
      <section class="table-controls">
        ...
        <MultiSelect
          ...
          allLabel={$t('wealth.page.allTypes')}
        />
        <MultiSelect flat bind:value={tCategory} options={categories} allLabel={$t('wealth.page.allCategories')} />
        {#if tSearch || tType.length || tCategory.length}
            ...
            {#each grouped as g (g.key)}
              ...
              {#if !collapsed.has(g.key)}
                {#each g.rows as a (a.id)}
                  <tr>
                    <td class="strong indent" title={a.name}>{a.name}</td>
                    <td title={assetTypeLabel(a.asset_type)}>{assetTypeIcon(a.asset_type)} {assetTypeLabel(a.asset_type)}</td>
                    <td title={a.category || ''}>{a.category || '—'}</td>
                    <td class="num">
...
<LinkPortfolioModal bind:open={showImport} existing={assets} onimported={reload} />
...
<style>
  ...
  /* Net worth is a magnitude, not a result: no green/red. Size comes from `nwSize`
     inline, so the value fits the rail whatever the currency. */
  .stat.big strong {
  ...
  /* Fixed column widths so the actions column never shifts; type/category flex. */
  .c-name { width: 28%; }
  .c-type { width: auto; }
  .c-cat { width: auto; }
  .c-val { width: 150px; }
  .c-date { width: 120px; }
  .c-act { width: 150px; }
  /* Text columns truncate rather than widen the table; the full value is in the title. */
  td.strong,
  td:nth-child(2),
  td:nth-child(3) {
    overflow: hidden;
  ...
  /* stretch (not center) so the search, dropdowns and buttons all share one height
