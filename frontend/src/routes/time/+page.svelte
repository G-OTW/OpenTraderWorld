<script>
  ...
  let bSince = $state('');
  let bUntil = $state('');
  // Chart metric: 'time' (hours) or 'cost' (value at each project's rate, display currency).
  let bMetric = $state('time');
  // Chart coloring: 'single' (one accent color) or 'multi' (one stacked segment per project).
  let bColor = $state('single');
  ...
  function loadPrefs() {
    try {
      ...
      if (p.bMetric === 'time' || p.bMetric === 'cost') bMetric = p.bMetric;
      if (p.bColor === 'single' || p.bColor === 'multi') bColor = p.bColor;
  ...
  $effect(() => {
    const snapshot = { view, bBucket, bProject, bCategory, bSince, bUntil, bMetric, bColor, selectedId };
  ...
  // ── Per-timer ranking (below the totals) ──
  // The breakdown already carries each project's hours and value for the filtered window;
  // the list just re-sorts them. Sorting by cost puts unrated timers last (value 0).
  let bSort = $state('hours'); // 'hours' | 'cost'
  const rankedProjects = $derived(
    [...(bd?.projects ?? [])].sort((a, b) =>
      bSort === 'cost' ? b.value - a.value || b.hours - a.hours : b.hours - a.hours
    )
  );
  const rankTop = $derived(
    rankedProjects.reduce((mx, p) => Math.max(mx, bSort === 'cost' ? p.value : p.hours), 0)
  );

  // Drill-down: the runs behind one timer, over the same window as the breakdown.
  let openProject = $state(null); // project id, or null
  let runs = $state([]);
  ...
  async function toggleRuns(p) {
    ...
    runsLoading = true;
    try {
      const r = await timeApi.listEntries(p.id, 500, { since: bSince, until: bUntil });
      if (openProject === p.id) runs = r;
    } finally {
      runsLoading = false;
    }
  }

  // Changing the filters invalidates whatever run list is open.
  $effect(() => {
    void bFilter;
    openProject = null;
    runs = [];
  ...
  // Display-currency rate implied by the breakdown (value = hours × converted rate), so a
  // run's cost needs no second FX round-trip. 0 for a timer with no rate.
  const openRate = $derived.by(() => {
    const p = rankedProjects.find((x) => x.id === openProject);
    return p && p.hours > 0 ? p.value / p.hours : 0;
  });

  const cur = $derived(state.display_currency);
  async function setCurrency(v) {
    state = await timeApi.updateSettings({ display_currency: v });
...
<div class="time">
  ...
  {#if showResume}
    <div class="resume">
      <span>
        <Icon name="clock" size={14} /> {$t('time.resume.body', { time: fmtDateTime(state.last_seen_at) })}
      </span>
  ...
  {#if loading}
    <!-- Timer cards, so the skeleton is a grid of card shapes. -->
  ...
  {:else if view === 'timers'}
    {#if projects.length === 0}
      <EmptyState icon="timer" title={$t('time.emptyTitle')} description={$t('time.emptyBody')}>
        {#snippet action()}
          <button class="primary" onclick={openAdd}><Icon name="plus" size={15} /> {$t('time.addProject')}</button>
        {/snippet}
      </EmptyState>
  ...
  {:else}
    <section class="filters">
      <Dropdown
        bind:value={bBucket}
        ariaLabel={$t('time.breakdown.byDay')}
        options={[
          { value: 'day', label: $t('time.breakdown.byDay') },
          { value: 'week', label: $t('time.breakdown.byWeek') },
          { value: 'month', label: $t('time.breakdown.byMonth') }
        ]}
      />
      <Dropdown
        bind:value={bProject}
        ariaLabel={$t('time.breakdown.allProjects')}
        options={[
          { value: '', label: $t('time.breakdown.allProjects') },
          ...projects.map((p) => ({ value: p.id, label: p.name }))
        ]}
      />
      <Dropdown
        bind:value={bCategory}
        ariaLabel={$t('time.breakdown.allCategories')}
        options={[
          { value: '', label: $t('time.breakdown.allCategories') },
    ...
    <section class="bd">
      <div class="card chart-card">
        <div class="chart-head">
          <h3>
            {bMetric === 'cost' ? $t('time.breakdown.trackedCost') : $t('time.breakdown.trackedHours')}
            {#if bMetric === 'cost'}<span class="cur">({cur})</span>{/if}
          </h3>
          <div class="chart-toggles">
            <div class="seg" role="group" aria-label={$t('time.breakdown.chartMetric')}>
              <button class:active={bMetric === 'time'} onclick={() => (bMetric = 'time')}>{$t('time.breakdown.metricTime')}</button>
              <button class:active={bMetric === 'cost'} onclick={() => (bMetric = 'cost')}>{$t('time.breakdown.metricCost')}</button>
            </div>
            <div class="seg" role="group" aria-label={$t('time.breakdown.chartColoring')}>
              <button class:active={bColor === 'single'} onclick={() => (bColor = 'single')}>{$t('time.breakdown.overall')}</button>
              <button class:active={bColor === 'multi'} onclick={() => (bColor = 'multi')}>{$t('time.breakdown.perProject')}</button>
            </div>
          </div>
        </div>
        <BucketChart
      ...
      <div class="card rank-card">
        <div class="chart-head">
          <h3>{$t('time.breakdown.byTimer')}</h3>
          <div class="seg" role="group" aria-label={$t('time.breakdown.rankBy')}>
            <button class:active={bSort === 'hours'} onclick={() => (bSort = 'hours')}>{$t('time.breakdown.totalHours')}</button>
            <button class:active={bSort === 'cost'} onclick={() => (bSort = 'cost')}>{$t('time.breakdown.totalCost')}</button>
          </div>
        </div>

        {#if rankedProjects.length === 0}
          <p class="muted">{$t('time.chart.empty')}</p>
        {:else}
          <table class="tbl rank">
            <thead>
              <tr>
                <th>{$t('time.breakdown.timer')}</th>
                <th class="num">{$t('time.breakdown.totalHours')}</th>
                <th class="num">{$t('time.breakdown.totalCost')} <span class="cur">({cur})</span></th>
                <th class="num">{$t('time.breakdown.share')}</th>
              </tr>
            </thead>
            <tbody>
              {#each rankedProjects as p (p.id)}
                <tr class="rank-row" class:open={openProject === p.id}>
                  <td>
                    <button class="name" onclick={() => toggleRuns(p)} aria-expanded={openProject === p.id}>
                      <Icon name={openProject === p.id ? 'chevron-down' : 'chevron-right'} size={13} />
                      <span class="dot" style:background={p.color || 'var(--accent)'}></span>
                      {p.name}
                    </button>
                  </td>
                  <td class="num">{fmtHours(p.hours)}</td>
                  <td class="num">{fmtMoney(p.value, cur)}</td>
                  <td class="num">
                    <span class="share">
                      <span
                        class="share-fill"
                        style:width="{rankTop > 0 ? ((bSort === 'cost' ? p.value : p.hours) / rankTop) * 100 : 0}%"
                        ...
                        <table class="tbl runs">
                          <thead>
                            <tr>
                              ...
                              <th>{$t('time.details.end')}</th>
                              <th class="num">{$t('time.details.duration')}</th>
                              <th class="num">{$t('time.details.cost')}</th>
                              <th>{$t('time.details.note')}</th>
                            </tr>
                          </thead>
                          <tbody>
                            {#each runs as r (r.id)}
                              <tr>
                                ...
                                <td>{r.ended_at ? fmtDateTime(r.ended_at) : $t('time.details.running')}</td>
                                <td class="num">{fmtDuration(entrySeconds(r))}</td>
                                <td class="num">{fmtMoney((entrySeconds(r) / 3600) * openRate, cur)}</td>
...
<style>
  ...
  /* Per-timer ranking: a row per timer, expanding into its runs. */
  .rank-card h3 {
    font-size: var(--text-base);
    font-weight: var(--fw-medium);
  }
  ...
  /* Proportion of the ranked metric, relative to the biggest timer. */
  .share {
    display: inline-block;
    width: 84px;
    height: 6px;
