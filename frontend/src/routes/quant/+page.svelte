<script>
  ...
  import { quantApi, fmtPct, dsLabel, CONFIDENCE_LEVELS } from '$lib/modules/quant/api.js';
  ...
  const initialTab = (() => {
    try {
      ...
      return ['single', 'portfolio', 'size', 'kelly'].includes(t) ? t : 'single';
    } catch {
      return 'single';
    }
  })();
...
<div class="page">
  ...
  {#if !datasets.length && tab !== 'kelly' && tab !== 'size'}
    <p class="hint">
  ...
  {#if tab === 'single'}
    ...
    {#if single}
      <div class="head">
        ...
        <span class="sub">{$t('quant.page.periodsCount', { count: single.result.periods.toLocaleString() })}</span>
      </div>
      ...
      <main class="port-main">
        {#if port}
          <div class="head">
            ...
            <span class="sub">{$t('quant.page.alignedPeriodsCount', { count: port.periods.toLocaleString() })}</span>
          </div>
              ...
              <div class="picked">
                ...
                <span class="muted small">
                  {$t('quant.page.pickedWeightsSummary', {
                    ret: fmtPct(pickedWeights.ret, 1),
                    vol: fmtPct(pickedWeights.vol, 1),
