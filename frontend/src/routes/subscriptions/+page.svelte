<script>
  ...
  async function del(s) {
    if (!confirm($t('subscriptions.list.confirmDelete', { name: s.name }))) return;
...
<div class="subs">
  ...
  {:else}
    ...
    {#if subs.length === 0}
      <div class="empty">{activeFilters ? $t('subscriptions.page.emptyFiltered') : $t('subscriptions.page.emptyNone')}</div>
    {:else}
      <div class="table-wrap">
        <table class="tbl">
          <thead>
            <tr>
              <th class="sortable" onclick={() => sortBy('name')}>{$t('subscriptions.form.name')}{sortArrow('name')}</th>
              <th class="sortable" onclick={() => sortBy('platform')}>{$t('subscriptions.form.platform')}{sortArrow('platform')}</th>
              <th class="sortable" onclick={() => sortBy('category')}>{$t('subscriptions.form.category')}{sortArrow('category')}</th>
              <th class="num sortable" onclick={() => sortBy('price')}>{$t('subscriptions.form.price')}{sortArrow('price')}</th>
              <th class="sortable" onclick={() => sortBy('frequency')}>{$t('subscriptions.form.frequency')}{sortArrow('frequency')}</th>
              <th class="num sortable" onclick={() => sortBy('monthly')}>{$t('subscriptions.page.monthlyEq')}{sortArrow('monthly')}</th>
              <th class="sortable" onclick={() => sortBy('next')}>{$t('subscriptions.page.nextBilling')}{sortArrow('next')}</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {#each tableSubs as s (s.id)}
              <tr class:inactive={!s.active}>
                ...
                <td class="row-actions">
                  <button class="icon" title={$t('subscriptions.page.edit')} onclick={() => openEdit(s)}><Icon name="pencil" size={14} /></button>
                  <button class="icon" title={$t('subscriptions.page.delete')} onclick={() => del(s)}><Icon name="trash" size={14} /></button>
                </td>
              </tr>
            {/each}
