<script>
  ...
  // Rates are loaded once; switching the display currency re-derives from the same map.
  ...
  onMount(async () => {
    [settings, suggestions] = await Promise.all([
      subsApi.getSettings(),
      subsApi.suggestions(),
      fx.load()
    ]);
  ...
  async function setCurrency(v) {
    settings = await subsApi.updateSettings({ display_currency: v });
    await reload();
  }
  ...
  // Each sub's monthly-equivalent in its own currency (the stored reference amount).
  const monthlyOf = (s) => s.price * monthlyFactor(s.frequency);

  // A billing reminder, seeded from the row: it fires on the next billing date and then
  // repeats on the sub's own cadence. `quarterly` has no reminder equivalent, so it falls
  // back to a one-shot rather than silently firing on the wrong months.
  const REMINDER_FREQ = { weekly: 'weekly', monthly: 'monthly', yearly: 'yearly' };
  function reminderDefaults(s) {
    const due = nextBillingDate(s);
    // The amount actually charged is the price in the sub's OWN currency, not the
    // display-currency conversion shown in the table.
    return {
      name: $t('subscriptions.page.reminderName', {
        name: s.name,
        amount: fmtMoney(s.price, s.currency)
      }),
      kind: 'custom',
      frequency: REMINDER_FREQ[s.frequency] ?? 'once',
      start_date: due ? dateKey(due) : ''
    };
  }

  // Display-currency views. The sub's own currency stays the reference; these are the
  // converted amounts shown in the table. null => no rate, so callers show the native value.
  const inDisplay = (amount, from) => fx.to(amount, from, cur);
  /** Show a converted amount only for foreign rows that actually have a rate. */
  const showsConverted = (s, amount) =>
    s.currency !== cur && inDisplay(amount, s.currency) != null;
    ...
    <div class="title">
      ...
      <div class="cur-select">
        ...
        <Dropdown
          value={cur}
          onpick={setCurrency}
          ariaLabel={$t('subscriptions.page.display')}
  ...
  {:else}
    ...
    <section class="filters">
      ...
      <div class="band">
        <Dropdown
          bind:value={fBand}
          ariaLabel={$t('subscriptions.page.priceBand')}
          options={[
            { value: '', label: $t('subscriptions.page.anyPrice') },
            ...PRICE_BANDS.map((b) => ({ value: b.id, label: b.label }))
          ]}
        />
      </div>
      {#if activeFilters > 0}
          ...
          <tbody>
            {#each tableSubs as s (s.id)}
              <tr class:inactive={!s.active}>
                ...
                <td>{s.category || '—'}</td>
                <td class="num">
                  {#if showsConverted(s, s.price)}
                    {fmtMoney(inDisplay(s.price, s.currency), cur)}<span class="native">{fmtMoney(s.price, s.currency)}</span>
                  {:else}
                ...
                <td class="num">
                  {#if showsConverted(s, monthlyOf(s))}
                    {fmtMoney(inDisplay(monthlyOf(s), s.currency), cur)}<span class="native">{fmtMoney(monthlyOf(s), s.currency)}</span>
                  {:else}
                    {fmtMoney(monthlyOf(s), s.currency)}
                  {/if}
                </td>
                <td class="num next-cell">
                  <span class="next-wrap">
                    {s.active ? fmtDate(nextBillingDate(s)) : '—'}
                    {#if s.active && nextBillingDate(s)}
                      <QuickReminderButton
                        variant="row"
                        title={$t('subscriptions.page.remindBilling')}
                        defaults={reminderDefaults(s)}
                      />
                    {/if}
                  </span>
                </td>
                <td class="row-actions">
                  ...
                  {#if s.url}
                    <!-- A real <a>, not a button: middle-click/copy-link must work. -->
                    <a class="icon" href={s.url} target="_blank" rel="noreferrer" aria-label={$t('subscriptions.page.openLink')} title={$t('subscriptions.page.openLink')}><Icon name="external-link" size={14} /></a>
                  {/if}
...
<style>
  ...
  /* Date + bell on one baseline, right-aligned like the rest of the numeric column.
     The bell only shows on row hover so the column stays a clean date column. */
  .next-wrap {
    display: inline-flex;
    align-items: center;
    gap: var(--space-1);
  ...
  .next-wrap :global(button.icon) {
    width: 22px;
    height: 22px;
    opacity: 0;
    transition: opacity var(--dur-fast) var(--ease);
  }
  tr:hover .next-wrap :global(button.icon),
  .next-wrap :global(button.icon:focus-visible) {
    opacity: 1;
  }
  .strong {
    font-weight: var(--fw-medium);
