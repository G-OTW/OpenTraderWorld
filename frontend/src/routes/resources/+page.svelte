<script>
  ...
  // selects "All" or a single category; the toolbar holds search, a gallery/card/list
  // display toggle, and an A–Z sort toggle. All view/filter state is persisted per-browser.
  import { onMount } from 'svelte';
  import { resourcesApi, linkHost, thumbFallback } from '$lib/modules/resources/api.js';
  ...
  let search = $state('');
  const DISPLAYS = ['gallery', 'card', 'list'];
  let display = $state('card'); // 'gallery' (big thumbnails) | 'card' | 'list'
  ...
  function loadPrefs() {
    try {
      ...
      if (DISPLAYS.includes(p.display)) display = p.display;
  ...
  // Two distinct destructive actions (a resource, or a category *and all its
  // resources*) share one dialog: `pending` carries the message plus the work to run.
  let confirmOpen = $state(false);
  let pending = $state(null); // { title, message, run }

  function askConfirm(title, message, run) {
    pending = { title, message, run };
    confirmOpen = true;
  }
  ...
  function removeResource(r) {
    askConfirm(
      $t('resources.page.deleteResource'),
      $t('resources.page.confirmDeleteResource', { name: r.name }),
      async () => {
  ...
  function removeCat(c) {
    askConfirm(
      $t('resources.page.deleteCategory'),
      $t('resources.page.confirmDeleteCategory', { name: c.name }),
      async () => {
...
<div class="page">
  <header class="top">
    ...
    <div class="tools">
      ...
      <div class="seg" role="group" aria-label={$t('resources.page.display')}>
        <button class:active={display === 'gallery'} onclick={() => (display = 'gallery')}><Icon name="image" size={13} /> {$t('resources.page.gallery')}</button>
  ...
  {#if loading}
    <!-- Skeleton the card area; the rail is chrome that fills in with the data. -->
  ...
  {:else}
    <div class="body">
      ...
      <section class="content">
        {#if !filtered.length}
          {#if search}
            <EmptyState icon="search" compact title={$t('resources.page.noneMatchSearch')}>
              {#snippet action()}
                <button class="ghost" onclick={() => (search = '')}>{$t('common.clear')}</button>
              {/snippet}
            </EmptyState>
          {:else}
            <EmptyState icon="book" title={$t('resources.page.noneYet')}>
              {#snippet action()}
                <button class="primary" onclick={openAdd}>{$t('resources.page.addResource')}</button>
              {/snippet}
            </EmptyState>
        ...
        {:else if display === 'gallery'}
          <div class="gallery">
            {#each filtered as r (r.id)}
              {@const fb = thumbFallback(r.name)}
              <article class="gcard">
                <!-- The whole picture is the link when there is one; without a link it is
                     just the tile, so it must not look clickable. -->
                <svelte:element
                  this={r.link ? 'a' : 'div'}
                  class="gthumb"
                ...
                >
                  ...
                  {:else}
                    <span class="ph" style:--h={fb.hue}>{fb.initials}</span>
                  {/if}
                </svelte:element>
                <div class="gbody">
                  <div class="card-head">
...
<style>
  ...
  /* Gallery: the picture carries the item, so the tiles are wider than the cards and the
     text sits under a fixed 16:9 crop — a ragged grid of mismatched images reads as noise. */
  .gallery {
    display: grid;
  ...
  /* No thumbnail: initials over a colour derived from the name, so the tile still has an
     identity instead of a grey hole. */
  .ph {
    width: 100%;
    height: 100%;
    display: grid;
