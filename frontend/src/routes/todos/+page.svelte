<script>
  ...
  import Timeline from '$lib/modules/todos/Timeline.svelte';
  ...
  // Timeline selection: null | { level: 'year'|'month'|'day', y, m?, d? }
  let range = $state(null);

  // Timeline visibility, remembered across visits. Shown unless explicitly hidden.
  const TIMELINE_KEY = 'otw.todos.timeline';
  let showTimeline = $state(true);
  (function loadTimelineOpen() {
    try {
      showTimeline = localStorage.getItem(TIMELINE_KEY) !== '0';
    } catch {
      /* ignore */
    }
  })();
  function toggleTimeline() {
    ...
    // Hiding the rail drops its filter, otherwise the list stays narrowed by a
    // selection the user can no longer see or clear.
    if (!showTimeline) range = null;
    try {
      localStorage.setItem(TIMELINE_KEY, showTimeline ? '1' : '0');
    } catch {
      /* ignore */
    }
  }
  ...
  // Timeline range. An undated task is out as soon as a period is picked — it belongs to
  // no point on the axis, so showing it under "March" would be a lie.
  function matchesRange(t) {
    if (range === null) return true;
    if (!t.due_date) return false;
    const [y, m, d] = t.due_date.split('-').map(Number);
    if (y !== range.y) return false;
    if (range.level === 'year') return true;
    if (m !== range.m) return false;
    return range.level === 'month' || d === range.d;
  }
  ...
  const open = $derived(
    todos.filter((t) => !t.done && matchesCat(t) && matchesOverdue(t) && matchesRange(t)).sort(byDue)
  );
  const done = $derived(
    todos.filter((t) => t.done && matchesCat(t) && matchesOverdue(t) && matchesRange(t)).sort(byDue)
  );
  ...
  // Ids of tasks whose note is unfolded. A Set in a $state rune must be reassigned to
  // trigger, so toggling builds a new one.
...
<div class="todos">
  <header class="head">
    ...
    <div class="head-actions">
      <button
        class="icon tl-toggle"
        class:on={showTimeline}
        onclick={toggleTimeline}
        title={showTimeline ? $t('todos.timeline.hide') : $t('todos.timeline.show')}
        aria-label={showTimeline ? $t('todos.timeline.hide') : $t('todos.timeline.show')}
  ...
  {:else}
    ...
    <ul class="list">
      {#each open as td (td.id)}
        ...
        <li class="item" data-due={ds}>
          ...
          {#if td.details}
            <!-- With a note the body is the disclosure control; without one it stays inert. -->
            <button
              class="body as-button"
              onclick={() => toggleNote(td.id)}
              aria-expanded={expanded.has(td.id)}
              aria-controls="note-{td.id}"
            >
              <div class="row1">
              ...
              <span class="meta">
                ...
                <span class="note-hint">
                  <Icon name={expanded.has(td.id) ? 'chevron-down' : 'chevron-right'} size={12} />
                  {$t('todos.note')}
                </span>
              </span>
              {#if expanded.has(td.id)}
                <p class="details" id="note-{td.id}">{td.details}</p>
              {/if}
            </button>
          {:else}
            <div class="body">
    ...
    {#if open.length === 0 && done.length === 0}
      <p class="no-match">{$t('todos.noMatch')}</p>
    {/if}
      </div>

      {#if showTimeline}
        <Timeline {todos} bind:range />
      {/if}
    </div>
  {/if}
</div>
...
<style>
  ...
  /* Task column + timeline rail. The rail is fixed-width, the list takes the rest. */
  .split {
    display: flex;
  ...
  /* Base button.icon styling is global; only the pressed state is local. */
  .tl-toggle.on {
    color: var(--accent);
  ...
  /* Inside .meta the row already carries the offset. */
  .meta .cat-tag {
    margin-top: 0;
  }
  .sk-item {
  ...
  /* The disclosure variant must be visually indistinguishable from the plain body. */
  .body.as-button {
    display: block;
    padding: 0;
    background: transparent;
  ...
  .body.as-button:hover .note-hint {
    color: var(--text);
  }
  .row1 {
    display: flex;
