<script>
  ...
  // Catalog snapshot check: the dataset has its own release cycle, so the header offers a
  // one-click update when the publisher has a newer snapshot than the installed one.
  let snapshot = $state(null); // last /findb/update-check payload, null until checked
  ...
  onMount(async () => {
    ...
    loading = false;
    // Cheap: the server caches the publisher's answer for 6 h.
    if (meta.installed && !meta.importing) checkUpdate(false);
  });
  onDestroy(() => {
  ...
  async function install() {
    try {
      ...
      // Refused (demo sandbox) or already running: say so rather than reject silently.
      checkFailed = true;
    }
    await refreshStatus();
  }

  // `force` = the user asked, so bypass the server-side cache. A silent check on mount that
  // fails leaves the plain "check for updates" link: only an explicit click reports trouble.
  async function checkUpdate(force = true) {
    checking = true;
    checkFailed = false;
    try {
      snapshot = await findbApi.updateCheck(force);
    } catch {
      checkFailed = force;
    } finally {
      checking = false;
  ...
  {:else}
    <header class="bar">
      ...
      <div class="cat">
        ...
        <span class="snap muted">{$t('findb.header.snapshot', { version: meta.version })}</span>
        {#if meta.importing}
          <span class="upd muted">{$t('findb.update.importing')}</span>
        {:else if snapshot?.update_available}
          <Button variant="primary" size="sm" icon="download" onclick={install}>
            {$t('findb.update.available', { version: snapshot.latest })}
          </Button>
        {:else if checking}
          <span class="upd muted">{$t('findb.update.checking')}</span>
        {:else if checkFailed || (snapshot && !snapshot.latest)}
          <button class="link" onclick={() => checkUpdate()}>{$t('findb.update.unavailable')}</button>
        {:else if snapshot}
          <button class="link" onclick={() => checkUpdate()}>{$t('findb.update.upToDate')}</button>
        {:else}
          <button class="link" onclick={() => checkUpdate()}>{$t('findb.update.check')}</button>
        {/if}
      </div>
    </header>
...
<style>
  ...
  /* Catalog identity on the right of the tab bar: how many rows, which snapshot, and
     whatever the update check has to say — one line, the button only when there is one. */
  .cat {
    display: flex;
    align-items: center;
    gap: var(--space-3);
