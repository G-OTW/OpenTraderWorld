<script>
  ...
  import RoutinesModal from '$lib/modules/journal/RoutinesModal.svelte';
  ...
  // Routines admin (calendar only): the daily habits painted on the month grid.
  // The counter is what tells the calendar to re-read them after an edit.
  let routinesOpen = $state(false);
  let routinesVersion = $state(0);
  ...
  <main class="content">
    <header class="content-head">
      ...
      <div class="head-actions">
        ...
        {/if}
        <!-- Routines are a per-book habit painted on the calendar, so the button
             only belongs on that view and only once a category is picked. -->
        {#if scoped && view === 'calendar'}
          <button class="btn" onclick={() => (routinesOpen = true)}>
            <Icon name="check-square" size={15} />
            {$t('journal.routines.button')}
          </button>
        {/if}
...
<RoutinesModal
  bind:open={routinesOpen}
  categoryId={categoryId}
  categoryName={currentCat?.name ?? ''}
  onchanged={() => (routinesVersion += 1)}
/>
