<script>
  ...
  import { remindApi, freqLabel, kindLabel, weekdaysLabel, fmtTime, fmtDate, KINDS } from '$lib/modules/remindme/api.js';
    ...
    <div class="controls">
      ...
      <div class="type-sel">
        <Dropdown
          ...
          title={$t('remindme.filterByType')}
          ariaLabel={$t('remindme.filterByType')}
          options={[
            { value: 'all', label: $t('remindme.allTypes') },
                  ...
                  {#each g.items as r (r.id)}
                    <tr class:inactive={!r.active}>
                      <td class="strong">
                        ...
                        {#if r.url}
                          <a
                            ...
                            title={r.link_label || r.url}
                          >
                            <Icon name="external-link" size={12} />
                            {r.link_label || $t('remindme.openLink')}
                          </a>
                        {/if}
                      </td>
                      ...
                      <td>
                        ...
                        {#if r.frequency === 'weekly' && r.weekdays}<span class="days">{weekdaysLabel(r.weekdays, $t)}</span>{/if}
                      </td>
...
<style>
  ...
  /* Dropdown brings its own control look; the wrapper only sets the filter's width. */
  .type-sel {
    min-width: 140px;
  }
  .month {
