<script>
  ...
  // Both are capped at RAIL_MAX and scroll, so the rail can't grow past what it can
  // show, and both are arranged from the pencil in the section header.
  //
  ...
  const RAIL_MAX = 8;
  const TOOLS_DEFAULT = ['backtest', 'quant', 'histdata', 'automator', 'webhooks', 'prompt-store'];
  const PRESET_ICONS = {
  ...
  // section default.
  function railSection(candidates, saved, fallbackIds) {
  ...
  const toolEntries = $derived(railSection(toolCandidates, railDoc?.rail?.tools, TOOLS_DEFAULT));
...
<style>
  ...
  /* A section never grows past what the rail can show: it scrolls instead. */
  .sidebar-scroll {
    max-height: 232px;
    overflow-y: auto;
