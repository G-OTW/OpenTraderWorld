...
export function chartColors() {
  ...
  const series = Array.from({ length: SERIES_COUNT }, (_, i) =>
    read(`--chart-${i + 1}`, ['#356fd6', '#15866a', '#c94b54', '#74808b', '#b87516', '#8a6cb1', '#4c91a8', '#8a9a62'][i])
  );
  ...
  return {
    ...
    diverge: [
      read('--diverge-neg', '#c94b54'),
      read('--diverge-mid', '#efede7'),
      read('--diverge-pos', '#15866a')
    ],
    accent: read('--accent', '#356fd6'),
    accentContrast: read('--accent-contrast', '#ffffff'),
    greenContrast: read('--green-contrast', '#ffffff'),
    muted: read('--muted', '#59616b'),
    dim: read('--dim', '#7b838d'),
    faint: read('--faint', '#969da4'),
    border: read('--border', '#f3f4f5'),
    borderControl: read('--border-control', '#d4d7dd'),
    gridLine: read('--grid-line', '#e3e0d9'),
    surface: read('--surface', '#fdfdfe'),
    surface2: read('--surface-2', '#f3f4f6'),
    bg: read('--bg', '#f7f8f9'),
    mono: read('--mono', "ui-monospace, 'SF Mono', Menlo, monospace"),
    text: read('--text', '#17191c'),
    green: read('--green', '#15866a'),
    red: read('--red', '#c94b54'),
    amber: read('--amber', '#b87516')
  };
}

/**
 * Institutional equity-curve line style, ready to spread into an ECharts line
 * series (or read piecemeal for a uPlot/SVG chart). Accent 1.5px stroke, hollow
 * data points (bg-filled, 1px accent ring, r=2), NO area fill, NO gradient.
 *
 *   const c = $derived(chartColors());
