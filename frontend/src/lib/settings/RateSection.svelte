  ...
  <div class="head">
    ...
    <div class="toolbar">
      <div class="win">
        ...
        <Dropdown
          value={String(days)}
          onpick={(v) => setWindow(Number(v))}
          ariaLabel={$t('settings.rate.window')}
          options={[
            { value: '1', label: $t('settings.rate.today') },
            { value: '7', label: $t('settings.rate.days', { count: 7 }) },
            { value: '30', label: $t('settings.rate.days', { count: 30 }) }
          ]}
        />
      </div>
