<script>
  ...
  let search = $state('');
  let sortDir = $state(1); // clicking the Vault header flips A-Z / Z-A

  // Matches the vault name and its item names, so searching "apikey" finds its owner.
    ...
    const out = q
      ? vaults.filter(
          (v) =>
            ...
            (v.items ?? []).some((i) => `${v.name}.${i.name}`.toLowerCase().includes(q))
        )
      : [...vaults];
...
<div class="section">
  ...
  {:else if !shown.length}
    <EmptyState icon="search" compact title={$t('vault.noMatch')} />
  {:else}
    <div class="tablewrap">
      <table>
        <thead>
