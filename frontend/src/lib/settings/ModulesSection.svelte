<script>
  ...
  // Registry order is load order; the list reads better alphabetically.
...
<div class="section">
  ...
  {#if $installedIds === null}
    <!-- A row is an icon, a two-line label, and an action. Six of them, so the list keeps
         its height while the installed set loads. -->
    <!-- Reusing .ico/.label rather than skeleton-only classes: the bones then inherit the
         row's real geometry, which is the point of a skeleton over a spinner. -->
    <ul class="list" aria-busy="true">
      {#each Array.from({ length: 6 }, (_, i) => i) as i (i)}
        <li>
          <span class="ico"><Skeleton circle size="17px" /></span>
          <span class="label">
            <Skeleton height="0.9rem" width="35%" />
            <Skeleton height="0.75rem" width="70%" />
          </span>
          <Skeleton height="1.9rem" width="76px" />
        </li>
      {/each}
    </ul>
  {:else}
    <ul class="list">
      {#each sortedModules as m (m.id)}
