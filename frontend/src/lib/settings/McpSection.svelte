<script>
  ...
  const shown = $derived.by(() => {
    ...
    const out = q ? tokens.filter((t) => t.name.toLowerCase().includes(q)) : [...tokens];
  ...
  let formName = $state('');
  let formPerms = $state({});
  // Expiry as a days-from-now choice: '' = leave as it is (edit only), '0' = never.
  let formExpiry = $state('0');
  // Whether this token may be reached from outside the app (a chat binding). Off unless
  // asked for: it is a separate decision from what the token may touch.
  let formExternal = $state(false);
  let formError = $state('');
  ...
  async function save() {
    ...
    formError = '';
    try {
      // '' means "don't touch the expiry", so the field is left out of the payload.
      const expiry = formExpiry === '' ? {} : { expires_in_days: Number(formExpiry) };
      if (editing) {
        await settingsApi.updateMcpToken(editing.id, {
          name: formName,
        ...
        const r = await settingsApi.createMcpToken({
          name: formName,
          permissions: formPerms,
          external: formExternal,
          ...expiry,
  ...
  const fmtExpiry = (tok) =>
    tok.expires_at ? fmtDate(tok.expires_at) : $t('settings.mcp.expiryNever');
...
<div class="section">
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>{$t('settings.mcp.colExpires')}</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
  ...
  {:else if !shown.length}
    <EmptyState icon="search" compact title={$t('settings.mcp.noMatch')} />
  {:else}
    <div class="tablewrap">
      <table>
        <thead>
        ...
        <tbody>
          {#each shown as tok (tok.id)}
            <tr>
              <td class="strong">
                {tok.name}
                {#if tok.external}
                  <Badge tone="accent" title={$t('settings.mcp.externalHint')}>
                    {$t('settings.mcp.externalTag')}
                  </Badge>
                {/if}
              </td>
              ...
              <td class:expired={isExpired(tok)}>
                {fmtExpiry(tok)}{#if isExpired(tok)} · {$t('settings.mcp.expired')}{/if}
              </td>
              <td class="actions">
...
<Modal bind:open={modalOpen} title={editing ? $t('settings.mcp.editTitle') : $t('settings.mcp.newToken')} size="md">
  <div class="form">
    ...
    <label class="field">
      <span>{$t('settings.mcp.expiry')}</span>
      <Dropdown
        value={formExpiry}
        onpick={(v) => (formExpiry = v)}
        ariaLabel={$t('settings.mcp.expiry')}
        options={[
          ...(editing ? [{ value: '', label: $t('settings.mcp.expiryKeep') }] : []),
    ...
    <p class="muted small">{$t('settings.mcp.expiryHint')}</p>

    <label class="check">
      <input type="checkbox" bind:checked={formExternal} />
      <span>{$t('settings.mcp.external')}</span>
    </label>
    <p class="muted small">{$t('settings.mcp.externalHint')}</p>
...
<style>
  ...
  /* An expired token is not an error, it is a token that did its job: amber, not red. */
  td.expired {
    color: var(--amber);
  }
