<script>
  ...
  // Each token carries per-module permissions: absent = no access, 'r' = read, 'rw' = read+write.
  ...
  function permSummary(perms) {
    ...
    const rw = vals.filter((v) => v === 'rw').length;
    return $t('settings.mcp.permSummary', { read: vals.length - rw, write: rw });
  }
...
<div class="section">
  ...
  {:else if !tokens.length}
    <p class="muted small">{$t('settings.mcp.noTokens')}</p>
  {:else}
    <div class="tablewrap">
      <table>
        <thead>
