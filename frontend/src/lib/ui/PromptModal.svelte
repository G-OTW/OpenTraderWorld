<script>
  ...
  import SymbolHelp from './SymbolHelp.svelte';
  import { t } from '$lib/i18n';

  // A small form modal. `fields` is an array of:
  //   { key, label, placeholder?, type?, value?, required?, options?, symbolHelp? }
  // `symbolHelp` (a provider key, or '' for the whole table) puts the shared symbol-format
  // helper next to the label: the field takes a symbol spelled the provider's way.
...
<Modal bind:open {title} onclose={oncancel}>
  ...
  >
    {#each fields as f, i (f.key)}
      ...
      {:else}
        <label class="field">
          {#if f.label || f.symbolHelp != null}
            <span class="lbl">
              {f.label ?? ''}
              {#if f.symbolHelp != null}<SymbolHelp provider={f.symbolHelp} />{/if}
            </span>
          {/if}
          {#if f.type === 'select'}
            <Dropdown
