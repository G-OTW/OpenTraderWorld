          ...
          {#if step.kind === 'ind'}
            ...
            <span class="src-wrap">
              ...
              <div class="src">
                <Dropdown
                  bind:value={step.src}
                  ariaLabel={$t('backtest.ind.of')}
                  options={sourcesBefore(i).map((s) => ({
                    value: s,
                    label: s,
                    disabled: !PRICE_TOKENS.includes(s) && !isChainableIndicator(step.indicator)
                  }))}
                />
              </div>
            </span>
...
<style>
  ...
  .controls input { font-size: var(--text-xs); min-width: 0; }
