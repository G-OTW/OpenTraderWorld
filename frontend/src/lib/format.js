 ...
 *   LOCALE Dates format in the language the user picked *in the app* (`intlLocale`,
 *          mirrored from the i18n store) — the browser's locale is not it, and
 *          "Sep 5, 2026" rendering as "5 sept. 2026" inside an English UI was the
 *          bug. Numbers/money still follow the browser locale (separators are a
 *          regional habit, not a UI language). Never pin a literal locale.
 */
import { derived } from 'svelte/store';
import { locale, LOCALES, t as tr } from '$lib/i18n';

/**
 * Active UI language as a BCP-47 tag, kept in sync with the i18n store. This module is
 * not a component, so it subscribes once at import rather than reading `$locale`.
 */
let intlLocale = 'en';
locale.subscribe((code) => {
  intlLocale = LOCALES.find((l) => l.code === code)?.html ?? code;
});
...
/**
 * Compact money in the caller's own currency, for a cramped chart axis: €260K, ¥1.2M,
 * CHF 4K. fmtCompactMoney is USD by construction; an axis under a EUR series needs the
 * series' currency or it states the wrong one.
 */
export function fmtCompactValue(n, ccy = 'USD') {
  if (isNil(n)) return EM_DASH;
  ...
  const mag =
    abs >= 1e9
      ? `${(abs / 1e9).toFixed(1)}B`
      : abs >= 1e6
        ? `${(abs / 1e6).toFixed(abs >= 1e7 ? 0 : 1)}M`
        : abs >= 1e3
          ? `${(abs / 1e3).toFixed(abs >= 1e4 ? 0 : 1)}K`
          : abs.toFixed(abs >= 100 || abs === 0 ? 0 : 2);
  return `${sign}${sym}${mag}`;
}
...
/** Token counts, compact: 940 → "940", 12_400 → "12.4k", 3_120_000 → "3.1M".
 *  Decimal thousands, not binary: a token is a count, not a size. */
...
/**
 * Localized relative time, ready to render: `{$ago(iso)}` -> "3m ago".
 *
 * `relativeTime()` deliberately returns { key, params } instead of text, so that the
 * format helpers stay free of translations. Rendering that object directly prints
 * "[object Object]", which is exactly what a dozen call sites did; this store is the
 * one-step form they should have been using.
 */
export const ago = derived(tr, ($t) => (v) => {
