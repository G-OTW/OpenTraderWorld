...
<div class="trend">
  ...
  {:else}
    <svg viewBox="0 0 {W} {H}" role="img" aria-label={$t('mindset.trendChart.trendLabel', { label })}>
      ...
      {/each}
      {#if points.length > 1}
        <polyline points={line} fill="none" stroke="var(--chart-1)" stroke-width="1.5" vector-effect="non-scaling-stroke" />
      {/if}
      {#each points as p (p.date)}
        <circle cx={p.x} cy={p.y} r="2" fill="var(--bg)" stroke="var(--chart-1)" stroke-width="1" vector-effect="non-scaling-stroke">
