...
export const findbApi = {
  ...
  /**
   * Is a newer catalog snapshot published? Returns
   * { current, installed, latest, built_at, size, upstream_commit, update_available }.
   * `latest` is null when the publisher is unreachable or the archive is a local file.
   * `force` skips the server's 6 h cache (the explicit "check now" button).
   */
  updateCheck: (force = false) => req(`/findb/update-check${force ? '?force=1' : ''}`),
