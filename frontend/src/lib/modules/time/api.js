...
export const timeApi = {
  ...
  // `window` optionally narrows to { since, until } (YYYY-MM-DD, on the run's start).
  listEntries: (id, limit = 50, window = {}) =>
    req(`/time/projects/${id}/entries${qs({ limit, ...window })}`).then((r) => r.entries),
