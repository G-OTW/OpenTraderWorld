...
export const resourcesApi = {
  ...
  /** Ask the server for a link's social-card image. Resolves to a stored URL, or null
   *  when the page declares no usable image. */
  preview: (link) =>
    req('/resources/preview', { method: 'POST', body: JSON.stringify({ link }) }).then(
      (r) => r.thumb_url ?? null
    )
};

/** Stand-in for a resource with no thumbnail: its initials over a colour derived from the
 *  name, so the same resource always draws the same tile. */
export function thumbFallback(name = '') {
