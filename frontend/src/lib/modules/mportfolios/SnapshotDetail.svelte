<script>
  ...
  let takenLabel = $derived(snapshot?.taken_at ? fmtDateTime(snapshot.taken_at) : '');
</script>

<Modal {open} size="xl" title={snapshot?.name ?? $t('mportfolios.detail.snapshotTitle')} onclose={close}>
  {#if loading}
