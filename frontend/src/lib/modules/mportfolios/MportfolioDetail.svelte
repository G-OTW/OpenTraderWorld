...
<Modal {open} size="xl" title={portfolio?.name ?? $t('mportfolios.detail.title')} onclose={close}>
  {#if loading}
...
<style>
  ...
  td {
    ...
    /* One line per holding: the wrap scrolls sideways rather than stacking company names. */
    white-space: nowrap;
  }
