<script>
  ...
  // variant: 'icon' (header bell) | 'inline' (text button inside a form) | 'row' (square
  //   icon sized for a table row's actions, on the shared button.icon style).
  // defaults: any other ReminderForm field to seed — frequency, start_date, details, …
  //   (a caller that knows the schedule, e.g. a subscription's next billing date).
  ...
  // Feed the form a one-item list so its linked-item picker shows the pre-linked
  // goal/todo by name.
  ...
  // `defaults` wins over the name/link seeds, so a caller can override any field.
  const initial = $derived(
    prelinked || effDefault || defaults
      ? { name: effDefault, kind, linked_id: effId, ...(defaults ?? {}) }
      : null
  );
...
{:else if variant === 'row'}
  <button type="button" class="icon" title={effTitle} aria-label={effTitle} disabled={busy} onclick={openModal}><Icon name="bell" size={14} /></button>
{:else}
...
<style>
  /* Square on the shared control height, so it lines up with the .btn it sits next to
