<script>
  ...
  async function remove(ch) {
    if (!confirm($t('remindme.channels.confirmDelete', { name: ch.name }))) return;
...
<style>
  ...
  .hint { color: var(--green); font-style: normal; font-size: 0.72rem; }
  .help { color: var(--muted); font-size: 0.72rem; }
  .field input {
    background: var(--surface);
    border: 1px solid var(--border);
    color: var(--text);
  ...
  .err { color: var(--red); font-size: 0.8rem; margin: 0; }
  .form-actions {
    display: flex;
