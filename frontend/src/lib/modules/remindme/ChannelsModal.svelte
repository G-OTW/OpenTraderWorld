<script>
  ...
  let secretVaultItem = $state(null); // vault item reference — the only way to set a secret
  ...
  function startEdit(ch) {
    draftId = ch.id;
    secretVaultItem = ch.secret_vault_item ?? null;
    error = null;
    draft = {
      kind: ch.kind,
      name: ch.name,
  ...
  async function save() {
    if (!draft) return;
    error = null;
    // A brand-new channel needs a vault-plugged secret; editing may leave it blank to
    // keep the old one.
    if (draftId === null && !secretVaultItem) {
    ...
    try {
      ...
      // Only send the reference when one was picked (blank on edit = untouched).
      if (secretVaultItem) payload.secret_vault_item = secretVaultItem;
  ...
  {:else if draft}
    ...
    <div class="form">
      ...
      <div class="field">
        ...
        <VaultPicker bind:vaultItemId={secretVaultItem} hasSecret={!!draft.has_secret} />
...
<style>
  ...
  .form-title { font-weight: var(--fw-medium); }
  .field {
    display: flex;
    flex-direction: column;
    gap: 4px;
  ...
  .help { color: var(--dim); font-size: var(--text-xs); }
  .field input {
    background: var(--surface);
    border: 0.5px solid var(--border-control);
    color: var(--text);
