...
<div class="templates">
  <div class="toolbar">
    ...
    <Button onclick={openPrebuilt}>{$t('journal.templates.startFromPrebuilt')}</Button>
...
<Modal bind:open={showEditor} title={editing ? $t('journal.templates.editTitle') : $t('journal.templates.newTitle')}>
  <div class="editor">
    ...
    <div class="field">
      ...
      <Dropdown
        bind:value={defaultFeeScheduleId}
        ariaLabel={$t('journal.templates.defaultFeeSchedule')}
        options={[
          { value: '', label: $t('journal.templates.none') },
      ...
      {#each fields as f, i (f.key)}
        <div class="frow">
          ...
          {:else}
            <Dropdown
              bind:value={f.type}
              ariaLabel={f.label || $t('journal.templates.labelPlaceholder')}
              options={CUSTOM_FIELD_TYPES.map((ct) => ({ value: ct.id, label: ct.label }))}
            />
            {#if f.type === 'select'}
              <input
                class="choices"
