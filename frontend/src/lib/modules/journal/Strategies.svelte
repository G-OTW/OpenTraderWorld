<script>
  ...
  let capNote = $state('');
  // Events are dated: a beginning stack, refill or withdrawal can be logged on any past
  // date (defaults to today). The date drives the equity curve and the FX rate used.
  let capDate = $state(today());
  ...
  // 'YYYY-MM-DD' → ISO at noon local, so the event keeps its date in any timezone.
  ...
  <section class="card">
    ...
    <div class="cap-head">
      <div class="field">
        ...
        <Dropdown
          bind:value={capCategory}
          ariaLabel={$t('journal.strategies.capital.category')}
    ...
    <div class="add-row">
      <Dropdown
        bind:value={capKind}
        ariaLabel={$t('journal.strategies.capital.kind.initial')}
        options={[
          { value: 'initial', label: $t('journal.strategies.capital.kind.initial') },
          { value: 'refill', label: $t('journal.strategies.capital.kind.refill') },
          { value: 'withdrawal', label: $t('journal.strategies.capital.kind.withdrawal') }
        ]}
      />
      <input type="number" step="any" placeholder={$t('journal.strategies.capital.amount')} bind:value={capAmount} />
      <Dropdown
        bind:value={capCurrency}
        title={$t('journal.strategies.capital.currency')}
        ariaLabel={$t('journal.strategies.capital.currency')}
      ...
      <input
        ...
        class="date-in"
        bind:value={capDate}
        title={$t('journal.strategies.capital.date')}
        aria-label={$t('journal.strategies.capital.date')}
      />
...
<style>
  ...
  /* A date is fixed-width — it shouldn't grow with the row. */
  .add-row input.date-in {
    flex: 0 0 auto;
    min-width: 0;
  }
  .cap-head {
