<script>
  ...
  async function saveDisplayCurrency(next) {
  ...
  <section class="card">
    ...
    <div class="field inline">
      ...
      <Dropdown
        ...
        onpick={saveDisplayCurrency}
        ariaLabel={$t('journal.fees.displayCurrency.label')}
  ...
  <section class="card">
    ...
    <div class="add-grid">
      ...
      <div class="field">
        ...
        <Dropdown
          bind:value={amountKind}
          ariaLabel={$t('journal.fees.schedules.kind')}
          options={FEE_AMOUNT_KINDS.map((k) => ({ value: k.id, label: k.label }))}
        />
      </div>
      <div class="field">
        ...
        <Dropdown
          bind:value={per}
          ariaLabel={$t('journal.fees.schedules.charged')}
          options={FEE_PER.map((p) => ({ value: p.id, label: p.label }))}
        />
      </div>
      <div class="field" class:dim={amountKind === 'pct'}>
        <span>{$t('journal.fees.schedules.currency')}</span>
        <Dropdown
          bind:value={currency}
          disabled={amountKind === 'pct'}
          ariaLabel={$t('journal.fees.schedules.currency')}
