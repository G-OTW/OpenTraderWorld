...
<style>
  ...
  /* Level 1: the table rests on the page, so it takes a border and no shadow. */
  .table-wrap {
    overflow-x: auto;
  ...
  /* Long/short is a direction, not a status, so it keeps its own pill rather than
     borrowing Badge's success/danger tones. Green and red here are conventional
     (a long profits when price rises), not semantic — the word is what carries it. */
  .side {
