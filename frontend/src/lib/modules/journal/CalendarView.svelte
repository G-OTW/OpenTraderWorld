<script>
  ...
  let {
    categoryId = '',
    // Bumped by the page when a routine is created or deleted, so the dots follow.
    routinesVersion = 0,
    displayCurrency = 'USD',
    // 'YYYY-MM-DD' to open on that day's month — set when coming back from its trades.
    initialDay = '',
  ...
  (function loadCursor() {
    // Coming back from a day's trade list wins over the persisted month: the user is
    // returning to a specific place, not reopening the calendar.
    if (initialDay) {
      const [y, m] = initialDay.split('-').map(Number);
      if (y && m >= 1 && m <= 12) {
        cursor = { year: y, month: m - 1 };
        return;
      }
    }
    try {
  ...
  // ── Routines ──
  // Which routines this book runs is a journal link; whether one was performed is the
  // Trading routines module's own tick, and its recurrence decides which days it is
  // even owed on. So the server answers one question per month: what was due each day,
  // and what was done. Nothing about routines is derived here.
  let statusByDate = $state(new Map()); // 'YYYY-MM-DD' -> [{link_id, routine_id, name, done}]

  $effect(() => {
    routinesVersion; // re-read when the page attaches or detaches a routine
    ...
    journalApi
      ...
      .then((days) => {
        const m = new Map();
        for (const d of days ?? []) m.set(d.date, d.routines);
        statusByDate = m;
      })
      .catch(() => (statusByDate = new Map()));
  });

  const hasRoutines = $derived(statusByDate.size > 0);

  // The dot is about the day that was traded: a routine skipped on a day with no trade
  // is not a miss. With a trade, all done is green, none done is red, and a mix is
  // amber. A day owing no routine gets no dot at all.
  function dotState(date, hasTrade) {
    const rows = statusByDate.get(date) ?? [];
    if (rows.length === 0) return null;
    const done = rows.filter((r) => r.done).length;
    const tone = !hasTrade ? 'none' : done === rows.length ? 'done' : done === 0 ? 'miss' : 'partial';
    return { tone, done, total: rows.length, hasTrade, rows };
  }

  // A routine's own mark inside the hover card follows the same reading.
  const rowTone = (row, hasTrade) => (row.done ? 'done' : hasTrade ? 'miss' : 'none');

  // ── Day panel ──
  // Read-only: the tick belongs to the Trading routines module, and duplicating it here
  // would give the same habit two truths.
  let dayOpen = $state(false);
  let dayDate = $state('');
  const dayRows = $derived(dayDate ? (statusByDate.get(dayDate) ?? []) : []);
  const dayTraded = $derived(dayDate ? !!byDate.get(dayDate) : false);
...
<div class="cal">
  ...
  {:else}
      ...
      {#each grid as week}
        <div class="row" role="row">
          {#each week.cells as c}
            {#if c}
              <!-- The routine dot is its own control, so the cell button holds only the
                   day's trades and the two clicks never fight over the same target. -->
              <div class="cellwrap" role="gridcell">
                <button
                  class="cell"
                ...
                {#if dotState(c.date, !!c.bucket)}
                  {@const st = dotState(c.date, !!c.bucket)}
                  <button
                    class="rdot {st.tone}"
                    onclick={() => openDay(c.date)}
                    aria-label={$t('journal.routines.dayTitle', { date: c.date })}
                  ></button>
                  <!-- Hover reads the day routine by routine: the cell dot is only
                       their summary, and several routines share one dot. -->
                  <div class="rcard" aria-hidden="true">
                    <span class="rhead">
                      {$t('journal.routines.dotTitle', { done: st.done, total: st.total })}
                    </span>
                    {#each st.rows as row (row.link_id)}
                      <span class="rrow">
                        <span class="dot {rowTone(row, st.hasTrade)}"></span>{row.name}
                      </span>
                    {/each}
                  </div>
                {/if}
    ...
    <p class="legend">
      ...
      {#if hasRoutines}
        <span class="dot none"></span>{$t('journal.routines.legend.none')}
        <span class="dot miss"></span>{$t('journal.routines.legend.miss')}
        <span class="dot partial"></span>{$t('journal.routines.legend.partial')}
        <span class="dot done"></span>{$t('journal.routines.legend.done')}
      {/if}
...
<!-- One day's routines: tick what was performed. Nothing else edits a check. -->
<Modal bind:open={dayOpen} title={$t('journal.routines.dayTitle', { date: dayDate })} size="sm">
  <div class="dayp">
    {#if dayRows.length === 0}
      <p class="muted">{$t('journal.routines.noneToday')}</p>
    {:else}
      {#each dayRows as row (row.link_id)}
        <span class="drow">
          <span class="dot {rowTone(row, dayTraded)}"></span>
          <span class="dname">{row.name}</span>
          <span class="dstate">
            {row.done ? $t('journal.routines.done') : $t('journal.routines.notDone')}
          </span>
        </span>
      {/each}
      <p class="muted">{$t('journal.routines.tickHint')}</p>
    {/if}
  </div>
</Modal>
...
<style>
  ...
  /* The wrapper is the grid item so the routine dot can sit over the cell without
     nesting a button inside one. */
  .cellwrap {
    position: relative;
    display: flex;
    background: var(--bg);
  ...
  /* The hover card is drawn from the cell, not from a title attribute: it has to name
     each routine and show its own state. It sits above the neighbouring cells and never
     takes the pointer, so moving onto it does not steal the dot's hover. */
  .rcard {
    position: absolute;
    right: 2px;
    bottom: 18px;
