<script>
  ...
  let {
    ...
    tags = [], // discipline tags (mistakes / rules / setups)
    onsubmit = () => {},
    oncancel = () => {}
  } = $props();
  ...
  function blank() {
    const base = {
      ...
      exits: [],
      brackets: [],
      // The plan, not the result: the stop feeds the risk line below and the R-multiple
      // the analytics screen reports.
      stop_price: null,
      tags: []
    };
    if (initial) {
      return {
        ...
        stop_price: initial.stop_price ?? null,
        tags: initial.tags ?? []
      };
    }
    return base;
  ...
  function toggleTag(id) {
    t.tags = t.tags.includes(id) ? t.tags.filter((x) => x !== id) : [...t.tags, id];
  }
...
// ── Risk line ──
  // One sentence, live, before the trade is committed: what the stop costs if it fills,
  // and what share of the account's stack that is. The stack comes from the category's
  // capital events; only events in the trade's own currency count toward the percentage,
  // so a multi-currency book never quotes a converted number it hasn't converted.
  let capitalEvents = $state([]);
  $effect(() => {
    ...
    journalApi
      .listCapital(id)
      .then((e) => {
        if (!stale) capitalEvents = e;
      })
      .catch(() => {
        if (!stale) capitalEvents = [];
      });
    return () => {
      stale = true;
    };
  ...
  const stack = $derived(
    capitalEvents
      .filter((e) => e.currency === t.currency)
      .reduce((sum, e) => sum + (e.kind === 'withdrawal' ? -e.amount : e.amount), 0)
  );

  // The stop the risk is measured against: the explicit field, else the first SL bracket
  // (advanced mode) — the same precedence the backend applies.
  const planStop = $derived.by(() => {
    const explicit = Number(t.stop_price);
    if (Number.isFinite(explicit) && explicit > 0) return explicit;
    const sl = t.brackets.find((br) => br.kind === 'sl' && Number(br.price) > 0);
    return sl ? Number(sl.price) : null;
  });
  // The first take-profit, when there is one: it turns the sentence into a ratio.
  const planTarget = $derived.by(() => {
    const tp = t.brackets.find((br) => br.kind === 'tp' && Number(br.price) > 0);
    return tp ? Number(tp.price) : null;
  });

  const risk = $derived.by(() => {
    const entry = preview?.avg;
    const stop = planStop;
    if (!entry || !stop) return null;
    const qty = feeBasisQty;
    const mult = Number(t.multiplier) || 1;
    const amount = Math.abs(entry - stop) * qty * mult;
    if (!(amount > 0)) return null;
    // A stop on the wrong side of the entry is a typo, not a plan: say so instead of
    // reporting a loss the trade cannot take.
    const wrongSide = t.side === 'short' ? stop < entry : stop > entry;
    const reward =
      planTarget != null ? (Math.abs(planTarget - entry) * qty * mult) / amount : null;
    return {
      amount,
      pct: stack > 0 ? (amount / stack) * 100 : null,
      stack,
      reward,
      wrongSide
    };
...
>
  ...
  <div class="row">
    <div class="field">
      ...
      <Dropdown
        bind:value={t.category_id}
        ariaLabel={$tr('journal.tradeForm.category')}
    ...
    <div class="field">
      ...
      <Dropdown
        bind:value={t.strategy_id}
        ariaLabel={$tr('journal.tradeForm.strategy')}
        options={[{ value: null, label: '—' }, ...strategies.map((s) => ({ value: s.id, label: s.name }))]}
      />
    </div>
  </div>
  ...
  <div class="grid">
    {#if showReserved('ticker')}
      <label class="field">
        <span>
          {$tr('journal.tradeForm.ticker')}
          <!-- The ticker is what market data is fetched with, so its shape is the
               provider's, not the broker statement's. -->
          <SymbolHelp />
        </span>
        <input bind:value={t.ticker} list="sg-tickers" autocomplete="off" />
      </label>
    {/if}
    {#if showReserved('asset_class')}
      <div class="field">
        ...
        <Dropdown
          bind:value={t.asset_class}
          ariaLabel={$tr('journal.tradeForm.assetClass')}
    ...
    {#if showReserved('side')}
      <div class="field">
        ...
        <Dropdown
          bind:value={t.side}
          ariaLabel={$tr('journal.tradeForm.side')}
          options={[
            { value: 'long', label: $tr('journal.side.long') },
            { value: 'short', label: $tr('journal.side.short') }
          ]}
        />
      </div>
    {/if}
    {#if showReserved('currency')}
      <div class="field">
        ...
        <Dropdown
          bind:value={t.currency}
          ariaLabel={$tr('journal.tradeForm.currency')}
    ...
    {#if showReserved('unit_type')}
      <div class="field">
        ...
        <Dropdown
          bind:value={t.unit_type}
          ariaLabel={$tr('journal.tradeForm.unitType')}
    ...
    {#if feeSchedules.length > 0}
      <div class="field">
        ...
        <Dropdown
          value={t.fee_schedule_id ?? ''}
          onpick={applyFeeSchedule}
          ariaLabel={$tr('journal.tradeForm.feeSchedule')}
          options={[
            { value: '', label: $tr('journal.templates.none') },
            ...feeSchedules.map((s) => ({ value: s.id, label: s.name }))
          ]}
        />
      </div>
    {/if}
    ...
    {#if showReserved('signal_name')}
      <label class="field">
        ...
        {#if selectedStrategy?.signals?.length}
          <Dropdown
            bind:value={t.signal_name}
            ariaLabel={$tr('journal.tradeForm.signalName')}
            options={[
              { value: '', label: '—' },
              ...selectedStrategy.signals.map((sig) => ({ value: sig, label: sig }))
            ]}
          />
        {:else}
    ...
    {#each customFields as f (f.key)}
      <label class="field" class:wide={f.type === 'textarea'}>
        ...
        {:else if f.type === 'select'}
          <Dropdown
            bind:value={t.fields[f.key]}
            ariaLabel={f.label}
            options={[
              { value: '', label: '—' },
              ...(f.options?.choices ?? []).map((c) => ({ value: c, label: c }))
            ]}
          />
        {:else}
  ...
  {#if t.advanced}
    <div class="legs">
      ...
      <section class="leg-group">
        ...
        {#each t.brackets as b, i (b.id)}
          <div class="leg-row bracket">
            <Dropdown
              bind:value={b.kind}
              ariaLabel={$tr('journal.tradeForm.brackets.title')}
              options={[
                { value: 'sl', label: $tr('journal.tradeForm.brackets.sl') },
                { value: 'tp', label: $tr('journal.tradeForm.brackets.tp') }
              ]}
            />
      ...
      <div class="field cost-basis">
        <span>{$tr('journal.tradeForm.costBasis.label')}</span>
        <Dropdown
          bind:value={t.cost_basis_method}
          ariaLabel={$tr('journal.tradeForm.costBasis.label')}
          options={[
            { value: 'avg', label: $tr('journal.tradeForm.costBasis.avg') },
            { value: 'fifo', label: $tr('journal.tradeForm.costBasis.fifo') }
          ]}
        />
      </div>
    </div>
  ...
  <!-- The plan and the discipline: what the trade risks, and which rules it honoured or
       broke. Always shown — a template describes the trade's columns, not its plan. -->
  <section class="plan">
    <div class="plan-row">
      <label class="field">
        <span>{$tr('journal.tradeForm.stopPrice')}</span>
        <input
          type="number"
          step="any"
          bind:value={t.stop_price}
          placeholder={$tr('journal.tradeForm.stopPlaceholder')}
        />
      </label>
      {#if risk}
        <p class="risk" class:wrong={risk.wrongSide}>
          {#if risk.wrongSide}
            {$tr('journal.tradeForm.risk.wrongSide', { side: $tr(`journal.side.${t.side}`) })}
          {:else if risk.pct != null}
            {$tr('journal.tradeForm.risk.withStack', {
              amount: fmtMoney(risk.amount, t.currency),
              pct: fmtPct(risk.pct),
              stack: fmtMoney(risk.stack, t.currency)
            })}
          {:else}
            {$tr('journal.tradeForm.risk.plain', { amount: fmtMoney(risk.amount, t.currency) })}
          {/if}
          {#if risk.reward != null && !risk.wrongSide}
            <span class="reward">{$tr('journal.tradeForm.risk.reward', { r: risk.reward.toFixed(2) })}</span>
          {/if}
        </p>
      {:else}
        <p class="risk empty-hint">{$tr('journal.tradeForm.risk.none')}</p>
      {/if}
    </div>

    {#if tags.length > 0}
      <div class="field wide">
        <span>{$tr('journal.tradeForm.tags')}</span>
        <div class="chips">
          {#each tags as tag (tag.id)}
            <button
              type="button"
              class="chip {tag.kind}"
              class:on={t.tags.includes(tag.id)}
...
<style>
  /* Plan block: the stop, the sentence it produces, and the discipline chips. */
  .plan {
    display: flex;
  ...
  /* One frame per chip; the kind tints the border only once the chip is on, so an
     unselected rail stays quiet. */
  .chip {
  ...
  .leg-row.bracket {
    grid-template-columns: 86px minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1.6fr) auto auto;
  }
  .leg-row input,
  .leg-row :global(.dd) {
    min-width: 0;
