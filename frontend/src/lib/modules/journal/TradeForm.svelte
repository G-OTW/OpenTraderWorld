<script>
  ...
  import { ASSET_CLASSES, CURRENCIES, UNIT_TYPES, computeFee, fmtMoney, roundDp } from './api.js';
  ...
  // ── Live PnL preview (mirrors the backend avg-cost math) ──
  const preview = $derived.by(() => {
    ...
    const gross = exits.reduce((s, l) => s + (Number(l.price) - avg) * Number(l.qty) * mult * dir, 0);
    const legFees =
...
>
    ...
    <div class="legs">
      ...
      {#if preview}
        <div class="pnl-preview">
          <span>{$tr('journal.tradeForm.preview.avgEntry')} <strong>{preview.avg?.toFixed(4) ?? '—'}</strong></span>
          {#if preview.openQty > 0}<span class="open">{$tr('journal.tradeForm.preview.openQty', { qty: preview.openQty })}</span>{/if}
          {#if preview.net != null}
            <span>{$tr('journal.tradeForm.preview.netPnl')} <strong class={preview.net >= 0 ? 'pos' : 'neg'}>{preview.net.toFixed(2)}</strong></span>
          {:else}
  ...
  {#if showReserved('images')}
    <div class="field wide">
      ...
      <div class="images">
        {#each t.images as img, i}
          <div class="thumb">
            <img src={img} alt={$tr('journal.tradeForm.images.alt')} />
            <button type="button" class="rm" onclick={() => removeImage(i)}><Icon name="x" size={13} /></button>
          </div>
        {/each}
  ...
  <div class="actions">
    ...
    <button type="submit" class="primary">{initial ? $tr('journal.tradeForm.saveTrade') : $tr('journal.tradeForm.logTrade')}</button>
  </div>
</form>
