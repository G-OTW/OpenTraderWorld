<script>
  // The analytics screen: what the raw breakdown cannot say. Four questions, four tabs.
  //
...
<div class="analytics">
  ...
  {:else if an && an.closed_count === 0 && tab !== 'compare'}
    <EmptyState
      icon="bar-chart"
  ...
  {:else if an}
    ...
    {:else if tab === 'scatter'}
      <ScatterPanel points={an.points} currency={displayCurrency} />
    {:else if tab === 'groups'}
      <section class="card">
