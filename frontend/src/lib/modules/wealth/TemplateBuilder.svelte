<script>
  ...
  function submit() {
    if (!name.trim()) {
      alert($t('wealth.templates.nameRequired'));
      return;
    }
...
<div class="builder">
  <div class="row">
    <label class="field"><span>{$t('wealth.templates.name')}</span><input bind:value={name} placeholder={$t('wealth.templates.namePlaceholder')} /></label>
    <label class="field">
