<script>
  ...
  function build(r, p) {
    ...
    const titles = [
      { text: $t(isVol ? 'quant.seasonality.byMonthWeekdayVol' : 'quant.seasonality.byMonthWeekday'), left: 90, top: 4, textStyle: { fontSize: 12, color: titleColor } },
      { text: $t('quant.seasonality.byMonth'), left: 90, top: 290, textStyle: { fontSize: 12, color: titleColor } },
      { text: $t('quant.seasonality.byWeekday'), left: 90, top: 364, textStyle: { fontSize: 12, color: titleColor } }
    ];
    if (hasHour) titles.push({ text: $t('quant.seasonality.byHour'), left: 90, top: 438, textStyle: { fontSize: 12, color: titleColor } });

    const axisLbl = { color: p.muted, fontSize: 11 };
