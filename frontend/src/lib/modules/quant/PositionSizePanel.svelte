...
<div class="wrap">
    ...
    {#if result}
      <div class="out">
        ...
        <div class="stats">
          ...
          <div><span>{$t('quant.size.riskOfStack')}</span><b>{fmtPct(result.risk_fraction, 2)}</b></div>
          <div><span>{$t('quant.size.exposure')}</span><b>{fmtPct(result.exposure_fraction, 1)}</b></div>
  ...
  <section class="asset">
    ...
    {#if sigError}<p class="err" title={$t('quant.common.clickToCopy')} use:copyLog={sigError}>{sigError}</p>{/if}
      ...
      <div class="sig-head">
        ...
        <span class="muted small">
          {$t('quant.size.signalsSummary', {
            hv: fmtPct(sig.signals.hv_period, 2),
      ...
      <div class="sugg">
        {#each sig.signals.suggestions as s (s.key)}
          <button class="pill" onclick={() => applyStop(s)} title={$t('quant.size.setAsStopTitle')}>
            ...
            <span class="pill-val">
              {$t('quant.size.stopSuggestion', { stop: fmtNum(s.stop_price, 4), pct: fmtPct(s.distance_pct, 2) })}
            </span>
          </button>
        {/each}
      </div>
