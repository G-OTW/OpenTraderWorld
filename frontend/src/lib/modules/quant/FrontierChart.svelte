<script>
  ...
  function build(f, pal) {
    const { muted, dim, gridLine, border, green, greenContrast, accent, surface, text, mono } = pal;
    ...
    return {
      animation: false,
      grid: { left: 56, right: 20, top: 32, bottom: 70 },
      tooltip: {
        formatter: (p) =>
          p.seriesName === 'cloud'
      ...
      yAxis: {
        ...
        nameGap: 14,
        nameTextStyle: { color: dim, fontFamily: mono, fontSize: 9, align: 'left' },
        axisLabel: { color: dim, fontFamily: mono, fontSize: 10 },
        axisLine: { lineStyle: { color: border } },
        axisTick: { show: false },
        splitLine: { lineStyle: { color: gridLine, width: 0.5 } }
      ...
      visualMap: {
        ...
        textStyle: { color: dim, fontFamily: mono, fontSize: 9 },
      ...
      series: [
        { name: 'cloud', type: 'scatter', symbolSize: 5, data: cloud, itemStyle: { opacity: 0.55 } },
        {
          ...
          label: { show: true, formatter: '★', color: greenContrast, fontSize: 11 }
        },
        {
