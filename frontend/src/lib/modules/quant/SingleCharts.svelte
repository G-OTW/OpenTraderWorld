<script>
  ...
  function build(r) {
    ...
    return {
      ...
      title: [
        { text: $t('quant.charts.drawdownPct'), left: 56, top: 4, textStyle: { fontSize: 12, color: muted } },
        { text: $t('quant.charts.returnDistributionPct'), left: 56, top: 214, textStyle: { fontSize: 12, color: muted } }
      ],
      tooltip: {
        trigger: 'axis',
      ...
      xAxis: [
        { type: 'category', data: ddX, gridIndex: 0, axisLabel: { show: false }, axisLine: { lineStyle: { color: border } } },
        {
          type: 'value',
          gridIndex: 1,
      ...
      yAxis: [
        { type: 'value', gridIndex: 0, max: 0, axisLabel: { color: muted }, splitLine: { lineStyle: { color: border, opacity: 0.3 } } },
        { type: 'value', gridIndex: 1, axisLabel: { color: muted }, splitLine: { lineStyle: { color: border, opacity: 0.3 } } }
      ],
      series: [
        {
          type: 'line',
          ...
          yAxisIndex: 0,
          data: ddY,
          areaStyle: { color: red, opacity: 0.25 },
  ...
  $effect(() => {
    if (chart && result) chart.setOption(build(result), true);
  });
</script>
