...
<div class="filters">
  ...
  <button class="sizesort" class:on={!!fSizeSort} onclick={cycleSizeSort} title={$t('histdata.datasets.sortBySize')}>
    {sizeSortLabel}
  </button>
  {#if anyFilter}
          ...
          <div class="sets">
            {#each tk.sets as d (d.id)}
              <div class="set" class:err={d.status === 'error'}>
                ...
                <span class="num">{$t('histdata.datasets.barsCount', { count: d.bar_count.toLocaleString() })}</span>
