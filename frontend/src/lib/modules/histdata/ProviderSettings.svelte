<script>
  ...
  let { connectors = [], providers = [], onchanged } = $props();

  // Remember which connector was open across refresh (per-browser).
  const SEL_KEY = 'otw.histdata.connector.v1';
  ...
  // ── Secrets: draft values per "connectorId:secretName" ──
  let drafts = $state({});
  async function saveSecret(id, name) {
    const key = `${id}:${name}`;
    const value = drafts[key];
    if (!value) return;
    await histdataApi.setSecret(id, name, value);
    drafts[key] = '';
    onchanged?.();
  }
      ...
      {#each connectors as c (c.id)}
        <li>
          ...
          >
            ...
            {#if c.paid}<span class="tag paid">{$t('histdata.providers.paid')}</span>{/if}
          </button>
        </li>
      {/each}
    </ul>
        ...
        <label class="fld">
          ...
          <select bind:value={addProvider} onchange={onAddProviderChange}>
            ...
            {#each providers as p (p.provider)}
              <option value={p.provider}>{p.label}{p.paid ? ` (${$t('histdata.providers.paid')})` : ''}</option>
            {/each}
          </select>
        </label>
        <label class="fld">
...
<style>
  ...
  /* Black ink, not a token: --amber and --green are mid-lightness in *both* themes, so
     one dark ink clears 4.5:1 on either. --text would invert to near-white on dark. */
  .paid {
