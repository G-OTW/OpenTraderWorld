<script>
  ...
  // Cross-rate factor from → to for the given tax year. Uses the year's last day, or the
  // last known rate when that date is still in the future (current year) / fell on a
  // non-trading day — hence the as-of lookup. Rates are USD-based (quote missing ⇒ USD
  // itself, factor 1). Returns null if a needed rate is missing.
  async function fxFactor(from, to, year) {
    if (from === to) return 1;
    const rows = await journalApi.fxRatesAsOf(`${year}-12-31`);
    const byQuote = Object.fromEntries((rows ?? []).map((x) => [x.quote, x.rate]));
    const r = (ccy) => (ccy === 'USD' ? 1 : byQuote[ccy]);
    const rf = r(from);
    const rt = r(to);
...
<Modal bind:open title={$t('taxcalc.loadJournal.title')} size="md">
  ...
  <div class="row">
    <div class="grow fld">
      <span>{$t('taxcalc.loadJournal.category')}</span>
      <Dropdown
        bind:value={categoryId}
        ariaLabel={$t('taxcalc.loadJournal.category')}
        options={[
          { value: '', label: $t('taxcalc.loadJournal.allCategories') },
