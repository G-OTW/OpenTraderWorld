<script>
  ...
  // Trailing stop. Two extra steps over SL/TP: `activate_pct` holds the trail off the book until
  // the trade is that far in profit, `breakeven_pct` moves the stop to the entry. A distance is a
  // percent of the extreme or a raw price distance (points, pips, currency) and nothing else: a
  // distance that reads an indicator is an indicator, so an ATR trail is an exit condition.
  // Percent fields are shown as percentages and stored as fractions.
  // An older strategy may carry the ATR kind the engine no longer accepts: show it as a percent
  // so the segment has a lit button, and the first edit writes a kind the run will take.
  const trKind = $derived(side.trailing_stop?.kind === 'abs' ? 'abs' : 'pct');
  const trOn = $derived(side.trailing_stop != null);
  let trVal = $state(0);
  let trAct = $state(0);
  let trBe = $state(0);
  $effect(() => {
    ...
    if (!tr) return;
    trVal = trKind === 'pct' ? (tr.value ?? 0) * 100 : (tr.value ?? 0);
    trAct = (tr.activate_pct ?? 0) * 100;
    trBe = (tr.breakeven_pct ?? 0) * 100;
  });

  function writeTrail(kind = trKind, value = trVal, act = trAct, be = trBe) {
    const v = Number(value) || 0;
    side.trailing_stop = {
      kind,
      // A percent trails a fraction of the extreme; abs carries its number as typed.
      value: kind === 'pct' ? v / 100 : v,
      activate_pct: (Number(act) || 0) / 100,
      breakeven_pct: (Number(be) || 0) / 100
    };
  }
  function toggleTrail(on) {
    if (!on) {
      side.trailing_stop = null;
      return;
    }
    if (!(Number(trVal) > 0)) trVal = 2; // 2% or 2 points
    writeTrail();
  }
</script>
...
<fieldset class="side" class:short={!isLong} {disabled}>
  ...
  <div class="risk">
    ...
    {#if slOn || tpOn}
      <p class="hint span">{$t('backtest.side.stopHint')}</p>
    {/if}
  </div>

  <div class="stop-field trail" class:off={!trOn}>
    <div class="stop-head">
      <label class="stop-toggle">
        <input type="checkbox" checked={trOn} onchange={(e) => toggleTrail(e.target.checked)} />
        <span>{$t('backtest.side.trailingStop')}</span>
      </label>
      <div class="kind-seg">
        <button type="button" class:on={trKind === 'pct'} disabled={!trOn} onclick={() => writeTrail('pct')}>%</button>
        <button type="button" class:on={trKind === 'abs'} disabled={!trOn} onclick={() => writeTrail('abs')}>abs</button>
      </div>
    </div>
    <div class="trail-row">
      <label class="mini">
        <span>{$t('backtest.side.trailDistance')}</span>
        <span class="unit">
          <input type="number" min="0" step="0.1" bind:value={trVal} disabled={!trOn}
            onchange={() => writeTrail()} />
          <span class="u">{trKind === 'pct' ? '%' : 'pts'}</span>
        </span>
      </label>
      <label class="mini">
        <span>{$t('backtest.side.trailActivate')}</span>
        <span class="unit">
          <input type="number" min="0" step="0.1" bind:value={trAct} disabled={!trOn}
            onchange={() => writeTrail()} title={$t('backtest.side.trailActivateHint')} />
          <span class="u">%</span>
        </span>
      </label>
      <label class="mini">
        <span>{$t('backtest.side.trailBreakeven')}</span>
        <span class="unit">
          <input type="number" min="0" step="0.1" bind:value={trBe} disabled={!trOn}
            onchange={() => writeTrail()} title={$t('backtest.side.trailBreakevenHint')} />
          <span class="u">%</span>
        </span>
      </label>
    </div>
    {#if trOn}
      <p class="hint">{$t('backtest.side.trailHint')}</p>
    {/if}
  </div>
...
<style>
  .side {
    ...
    border-left: 2px solid color-mix(in srgb, var(--side-color) 55%, transparent);
    border-radius: 0;
    padding: var(--space-3);
    display: flex;
  ...
  /* The trail carries more knobs than SL/TP, so it takes its own full-width row and wraps its
     fields rather than squeezing into the two-column risk grid. */
  .trail-row {
    display: grid;
