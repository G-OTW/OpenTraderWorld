...
<div class="group">
  ...
  {#each group.conditions as c, i (c)}
    ...
    {/if}
    <div class="cond">
      <OperandEditor bind:operand={c.left} {customIndicators} {positions} />
      <div class="op">
        <Dropdown
          value={c.op}
          onpick={(v) => setOp(c, v)}
          ariaLabel={$t('backtest.signalGroup.rule')}
          options={OPS.map((o) => ({ value: o.id, label: o.label }))}
        />
      </div>
      {#if opIsBinary(c.op) && c.right}
        <OperandEditor bind:operand={c.right} {customIndicators} {positions} />
      {/if}
