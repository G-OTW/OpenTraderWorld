<script>
  ...
  // A single dataset has nothing to align, so it reports the same facts as a period: window,
  // bar count, grain, warm-up.
  let { alignment = null, loading = false } = $props();

  const fmtTs = (ts) => (ts ? String(ts).slice(0, 10) : '—');
  const one = $derived(alignment?.assets?.length === 1 ? alignment.assets[0] : null);
</script>
...
{:else if alignment}
  <div class="banner">
    <div class="hdr">
      ...
      <span class="title">{$t(one ? 'backtest.align.titleOne' : 'backtest.align.title')}</span>
      <span class="spacer"></span>
      <span class="stat">
        {one
          ? $t('backtest.align.barsN', { n: one.bars.toLocaleString() })
    ...
    <div class="facts">
      <span class="fact">
        {one
          ? $t('backtest.align.window', { from: fmtTs(one.first_ts), to: fmtTs(one.last_ts) })
      ...
      {#if alignment.grain}
        <span class="fact">{$t('backtest.align.grain', { tf: alignment.grain })}</span>
      {/if}
        ...
        <tbody>
          {#each alignment.assets as a (a.ticker)}
            <tr>
              ...
              <td class="r num" class:warn={a.inactive_bars > 0}>
                {a.inactive_bars.toLocaleString()}
              </td>
            </tr>
          {/each}
        </tbody>
