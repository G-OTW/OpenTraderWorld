<script>
  ...
  let { bars = null, trades = [], equity = [] } = $props();
  ...
  function build(b, p) {
    ...
    return {
      ...
      series: [
        ...
        {
          // Same starting capital passively holding the asset — the benchmark to beat.
          name: $t('backtest.chart.buyHold'),
          type: 'line',
          data: b.c[0] > 0 && equity.length ? b.c.map((c) => (equity[0].equity * c) / b.c[0]) : [],
          xAxisIndex: 1,
          yAxisIndex: 1,
          showSymbol: false,
