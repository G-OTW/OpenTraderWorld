<script>
  ...
  function build(b, p, tr, range) {
    ...
    if (!tooDense) {
      ...
      for (const t of tr) {
        ...
        // A trailing stop is a stop: it draws on the same level line as the fixed one.
        if (t.exit_reason === 'stop_loss' || t.exit_reason === 'trailing_stop')
    ...
    return {
      ...
      xAxis: [
        { type: 'category', data: b.ts, gridIndex: 0, axisLabel: { show: false }, axisLine: { lineStyle: { color: p.border } }, axisTick: { show: false } },
        { type: 'category', data: b.ts, gridIndex: 1, axisLabel: { color: p.dim, fontFamily: p.mono, fontSize: 10 }, axisLine: { lineStyle: { color: p.border } }, axisTick: { show: false } }
      ],
      yAxis: [
        { scale: true, gridIndex: 0, axisLabel: { color: p.dim, fontFamily: p.mono, fontSize: 10 }, axisLine: { lineStyle: { color: p.border } }, axisTick: { show: false }, splitLine: { lineStyle: { color: p.gridLine, width: 0.5 } } },
        { scale: true, gridIndex: 1, name: $t('backtest.chart.equity'), nameTextStyle: { color: p.dim, fontFamily: p.mono, fontSize: 9 }, axisLabel: { color: p.dim, fontFamily: p.mono, fontSize: 10 }, axisLine: { lineStyle: { color: p.border } }, axisTick: { show: false }, splitLine: { lineStyle: { color: p.gridLine, width: 0.5 } } }
      ],
      axisPointer: { link: [{ xAxisIndex: 'all' }] },
      tooltip: { trigger: 'axis', axisPointer: { type: 'cross' }, backgroundColor: p.surface, borderColor: p.border, textStyle: { color: p.text, fontFamily: p.mono, fontSize: 11 } },
      dataZoom: [
        { type: 'inside', xAxisIndex: [0, 1] },
        { type: 'slider', xAxisIndex: [0, 1], bottom: 4, height: 14, textStyle: { color: p.dim, fontFamily: p.mono } }
      ],
      series: [
        {
        ...
        { name: $t('backtest.chart.exit'), type: 'scatter', data: exits, xAxisIndex: 0, yAxisIndex: 0, symbol: 'circle', symbolSize: 7, z: 5 },
        {
          ...
          symbol: 'circle',
          symbolSize: 4,
          itemStyle: { color: p.bg, borderColor: p.accent, borderWidth: 1 },
  ...
  function palette() {
    return {
      green: cssVar('--green', '#7fb894'),
      red: cssVar('--red', '#c9776b'),
      accent: cssVar('--accent', '#c9a45c'),
      text: cssVar('--text', '#edebe6'),
      muted: cssVar('--muted', '#8b8e96'),
      dim: cssVar('--dim', '#6e7178'),
      faint: cssVar('--faint', '#4a4d54'),
      border: cssVar('--border', '#24262c'),
      gridLine: cssVar('--grid-line', '#1b1d22'),
      surface: cssVar('--surface', '#0c0d10'),
      bg: cssVar('--bg', '#0c0d10'),
      mono: cssVar('--mono', "ui-monospace, 'SF Mono', Menlo, monospace")
    };
  }
</script>
