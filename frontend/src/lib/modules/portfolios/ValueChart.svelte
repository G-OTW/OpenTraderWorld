<script>
  ...
  function periodKey(dateStr, g) {
    ...
    // `d` is local midnight, so it must be serialized from its local parts. Going through
    // toISOString() shifted it back a day east of Greenwich, keying the bucket to a Sunday.
...
<div class="wrap">
  <!-- The granularity switch only re-buckets an existing series; with nothing plotted it is
       a control that does nothing, so it stays out of the empty state. -->
  {#if points.length >= 2}
    <div class="head">
      <div class="grans">
    ...
    >
      ...
      <!-- equity curve: gold, no fill under it -->
      <polyline points={line} fill="none" class="curve" vector-effect="non-scaling-stroke" />

      <!-- crosshair -->
      {#if cross}
        <line x1={cross.x} y1={T} x2={cross.x} y2={T + plotH} class="cross" />
        <line x1={L} y1={cross.y} x2={L + plotW} y2={cross.y} class="cross" />
        <circle cx={cross.x} cy={cross.y} r="2.5" class="dot" />
  ...
  {:else}
    <EmptyState
      icon="trending-up"
      title={$t('portfolios.valueChart.emptyTitle')}
      description={$t('portfolios.valueChart.notEnoughHistory')}
      compact
    />
  {/if}
</div>
