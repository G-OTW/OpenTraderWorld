<script>
  ...
  // Identifying colour: same palette as the journal categories / editor chips.
  // '' = none, and the dot falls back to the neutral kind colour.
  ...
  async function save() {
    ...
    try {
      ...
      let saved;
      if (editing) {
        await newsApi.updateFeed(feed.id, { name, config, enabled, interval_secs, color });
        saved = { ...feed, name, config, enabled, interval_secs, color };
      } else if (dashboardId) {
        ...
        const res = await newsApi.addDashboardSource(dashboardId, { name, kind, config, interval_secs, color });
        if (res.duplicate) {
          duplicate = res.duplicate;
          saving = false;
          return; // resolved by reuseExisting() / createCopy()
        }
        saved = res.feed ?? { id: res.feed_id, name, kind, config, interval_secs, color };
      } else {
        saved = await newsApi.createFeed({ name, kind, config, interval_secs, color });
      }
      onsaved(saved);
    } catch (e) {
      error = e.message;
  ...
  async function createCopy() {
    ...
    try {
      const config = buildConfig();
      const interval_secs = Number(intervalSecs);
      const res = await newsApi.addDashboardSource(dashboardId, { name, kind, config, interval_secs, color, force: true });
      duplicate = null;
      onsaved(res.feed ?? { id: res.feed_id, name, kind, config, interval_secs, color });
    } catch (e) {
      error = e.message;
    } finally {
      saving = false;
...
<div class="feed-form">
  ...
  <div class="row">
    ...
    {:else}
      <Dropdown
        bind:value={kind}
        ariaLabel={$t('news.feedForm.type')}
        options={[
          { value: 'rss', label: $t('news.feedForm.typeRss') },
          { value: 'api', label: $t('news.feedForm.typeApi') }
        ]}
      />
    {/if}
  </div>
  ...
  <div class="row">
    <span>{$t('news.feedForm.color')}</span>
    <div class="colors">
      <button
        class="swatch none"
        class:selected={!color}
        title={$t('news.feedForm.colorNone')}
        aria-label={$t('news.feedForm.colorNone')}
        onclick={() => (color = '')}
      ><Icon name="x" size={12} /></button>
      {#each COLOR_SWATCHES as hex}
        <button
          class="swatch"
        ...
        ></button>
      {/each}
      <label class="swatch custom" title={$t('news.feedForm.colorCustom')}>
        <input type="color" value={color || '#7d8a99'} oninput={(e) => (color = e.target.value)} />
  ...
  {:else}
    ...
    <div class="row">
      ...
      <Dropdown
        ...
        ariaLabel={$t('news.feedForm.method')}
        options={[
          { value: 'GET', label: 'GET' },
      ...
      {#if rlTrack}
        <div class="rl-limit">
          ...
          <div class="rl-period">
            <Dropdown
              bind:value={rlPeriod}
              ariaLabel={$t('news.feedForm.per')}
              options={['minute', 'hour', 'day', 'week', 'month'].map((p) => ({
                value: p,
                label: $t(`common.period.${p}`)
              }))}
            />
          </div>
        </div>
...
<style>
  ...
  /* Colour picker: fixed palette + a tile hiding a native colour input. */
  .colors {
    display: flex;
    align-items: center;
    gap: 6px;
