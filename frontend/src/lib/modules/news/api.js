...
export const newsApi = {
  ...
  /** Request-quota usage of tracked API feeds, keyed by feed id:
   *  { "<id>": { max_requests, period, used, resets_at } } */
  quotas: () => req('/feeds/quotas').then((r) => r.quotas),
  ...
  // [{ name, vault_item_id }] — vault_item_id set when plugged from the vault.
  listSecretMeta: (id) => req(`/feeds/${id}/secrets`).then((r) => r.secrets ?? []),
  setSecret: (id, name, value, vaultItemId = null) =>
    req(`/feeds/${id}/secrets`, {
      method: 'POST',
