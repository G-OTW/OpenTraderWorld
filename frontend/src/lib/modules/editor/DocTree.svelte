<script>
  ...
  // Colour flags a row can carry — mirrors FLAGS in core/otw-core/src/documents.rs.
  const FLAG_COLORS = {
    red: '#c9776b',
    orange: '#d08c4f',
    yellow: '#c9a45c',
  ...
  // Row whose flag palette is open.
  let flagMenuId = $state(null);

  function pickFlag(id, color) {
    flagMenuId = null;
    onflag(id, color);
  }

  // Close the palette on any click outside it.
  $effect(() => {
    if (!flagMenuId) return;
    const onDoc = (e) => {
      if (!e.target.closest('.flag-wrap')) flagMenuId = null;
    };
  ...
  const ICONS = {
    ...
    flag: 'M4 14V2.6h8L10 5.6l2 3H4'
  };

  /** Show the native tooltip only when the label is actually ellipsised. */
  function tooltipIfTruncated(node, text) {
    const sync = () => {
      const full = node.dataset.full ?? '';
      if (node.scrollWidth > node.clientWidth) node.title = full;
      else node.removeAttribute('title');
    };
    node.dataset.full = text;
    node.addEventListener('mouseenter', sync);
    return {
      update(next) {
        node.dataset.full = next;
      },
      destroy() {
        node.removeEventListener('mouseenter', sync);
      }
    };
  }
...
{#snippet node(d, depth)}
  ...
  <div
    ...
    class:menu-open={flagMenuId === d.id}
  ...
  >
    ...
    {:else}
      <button class="label" onclick={() => (isFolder ? toggle(d.id) : onselect(d.id))} ondblclick={() => startRename(d)}>
        {#if d.flag}
          <span class="flag-mark" style="background:{FLAG_COLORS[d.flag]}" aria-hidden="true"></span>
        {/if}
        {@render icon(isFolder ? 'folder' : d.kind === 'database' ? 'database' : 'page', 'ico')}
        <span class="title" use:tooltipIfTruncated={d.title || $t('editor.docTree.untitled')}>
          {d.title || $t('editor.docTree.untitled')}
        </span>
      </button>
    {/if}
    ...
    <span class="actions">
      ...
      <span class="flag-wrap">
        <button
          class="act"
          class:on={!!d.flag}
          style={d.flag ? `color:${FLAG_COLORS[d.flag]}` : ''}
          title={$t('editor.docTree.flag')}
          onclick={() => (flagMenuId = flagMenuId === d.id ? null : d.id)}
        >
          {@render icon('flag', '')}
        </button>
        {#if flagMenuId === d.id}
          <div class="flag-menu" role="menu">
            {#each Object.entries(FLAG_COLORS) as [name, hex]}
              <button
                class="flag-dot"
                style="background:{hex}"
                title={name}
                aria-label={name}
                onclick={() => pickFlag(d.id, name)}
              ></button>
            {/each}
            <button class="flag-dot none" title={$t('editor.docTree.noFlag')} aria-label={$t('editor.docTree.noFlag')} onclick={() => pickFlag(d.id, null)}>
              <Icon name="x" size={11} />
            </button>
          </div>
        {/if}
...
<style>
  ...
  /* Colour dot at the head of the row — always visible, never truncated away */
  .flag-mark {
    flex-shrink: 0;
    width: 7px;
    height: 7px;
  ...
  .row:hover .actions,
  .row.menu-open .actions {
    display: flex;
  }

  /* Colour-flag palette, opened from the row's flag button */
  .flag-wrap {
    position: relative;
    display: inline-flex;
  }
