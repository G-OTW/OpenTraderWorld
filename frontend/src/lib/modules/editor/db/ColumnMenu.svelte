...
<div class="colhead">
  <button class="colname" bind:this={trigger} onclick={() => (open = !open)} title={$t('editor.columnMenu.editColumn')}>
  ...
  {#if open}
    ...
    <div class="panel" use:anchored={{ trigger }}>
      <div class="name-field">
        ...
        />
        <span class="name-ico" aria-hidden="true"><Icon name="pencil" size={13} /></span>
      </div>
      ...
      {#if isSelect}
        ...
        {#each choicesOf(col) as c (c.id)}
          <!-- The input carries the option's colour itself; a separate preview
               chip only stole width and made both truncate. -->
          <div class="choice">
            <div class="choice-top">
              <input
                class="field sm opt-chip {c.color}"
                value={c.name}
...
<style>
  ...
  /* Level 2: it floats over the table, so it carries the shadow and drops the border.
     Fixed + anchored by hand — the scrolling table would clip an absolute panel. */
  .panel {
    position: fixed;
  ...
  /* Rename input with a hint icon parked on its right edge */
  .name-field {
    position: relative;
    display: flex;
  }
  ...
  /* The rename input wears the option colour (opt-chip class) but stays a field. */
  .choice-top .field {
    min-width: 0;
    border: 1px solid var(--border-control);
    border-radius: var(--radius);
