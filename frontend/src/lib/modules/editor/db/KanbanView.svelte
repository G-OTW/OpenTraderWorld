<script>
  ...
  function buildLanes(col, allRows) {
    ...
    // Unassigned first: that is the inbox cards start in, not an afterthought.
    return [empty, ...lanes];
  }
...
{:else}
  <div class="kanban-bar">
    <span class="pick">
      ...
      <Dropdown
        value={groupCol?.id}
        onpick={onGroupColChange}
        ariaLabel={$t('editor.kanbanView.groupBy')}
        options={selectCols.map((c) => ({ value: c.id, label: c.name || $t('editor.docTree.untitled') }))}
      />
    </span>
  </div>
