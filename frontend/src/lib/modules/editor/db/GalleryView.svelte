...
<div class="gallery-bar">
  <span class="pick">
    ...
    <Dropdown
      value={coverCol?.id ?? ''}
      onpick={onCoverColChange}
      ariaLabel={$t('editor.galleryView.coverImageLabel')}
      options={[
        { value: '', label: $t('editor.galleryView.none') },
        ...urlCols.map((c) => ({ value: c.id, label: c.name || $t('editor.docTree.untitled') }))
      ]}
    />
  </span>
  {#if urlCols.length === 0}
