<script>
  ...
  import { drawTool, makeDrawing, translate, moveAnchor, FIB_LEVELS } from './drawings.js';
  import { priceDigitsOf, makePriceFmt, makeAxisFmt } from './format.js';
  import { QUICK_COLORS } from './quicktest.js';
  ...
  function onPointerDown(e) {
    ...
    drag = {
      ...
      y0: e.clientY,
      win0: { ...win },
      // The plot drag needs the extent too: it pans the price scale as well as time.
      ext0: zone === 'yaxis' || zone === 'plot' ? priceExtent() : null,
      vert: false,
      moved: false
    };
  ...
  function onPointerMove(e) {
    ...
    if (drag.zone === 'plot') {
      ...
      // Dragging also moves the price scale, as in every market tool: pull the chart down and
      // the window shows higher prices. A vertical drag is what switches the scale to manual
      // (the "auto" button is the way back), so a purely horizontal pan leaves it auto-fitting.
      if (drag.ext0 && (drag.vert || Math.abs(dy) > 2)) {
        drag.vert = true;
        const ext = drag.ext0;
        if (priceLogNow()) {
          const lo = Math.log(Math.max(ext.min, Number.EPSILON));
          const hi = Math.log(Math.max(ext.max, Number.EPSILON * 2));
          const d = ((hi - lo) * dy) / r.height;
          yMan = { min: Math.exp(lo + d), max: Math.exp(hi + d) };
        } else {
          const d = ((ext.max - ext.min) * dy) / r.height;
          yMan = { min: ext.min + d, max: ext.max + d };
        }
      }
  ...
  function xPix(idx) {
    const r = rect(0);
    if (!r) return null;
    const span = win.e - win.s + 1;
    return r.x + ((idx - win.s + 0.5) / span) * r.width;
  }
  function xIdx(px) {
    const r = rect(0);
    if (!r) return 0;
    const span = win.e - win.s + 1;
    return win.s + ((px - r.x) / r.width) * span - 0.5;
  }
  ...
  const shapes = $derived.by(() => {
    ...
    const list = draft ? [...drawings, { ...draft, id: '__draft' }] : drawings;
  ...
  function onDrawDown(e) {
    ...
    if (drawTool(tool).points === 1) {
      const d = makeDrawing(tool, p, null, tool === 'text' ? { text: '' } : {});
      commit(d);
      updateDrawings();
      if (d.tool === 'text') {
  ...
  function onDrawUp(e) {
    if (draft) {
      ...
      if (far) commit(makeDrawing(d.tool, d.a, d.b));
      else tool = 'cursor';
      updateDrawings();
    }
  ...
  const fmtSize = (v) => Number(Number(v).toFixed(6)).toString();
  ...
  const markShapes = $derived.by(() => {
    void paintTick;
    const r = clip;
    if (!r || !view || !marks.length) return [];
    const out = [];
    for (const m of marks) {
      ...
      out.push({
        ...
        y: base + (up ? 11 : -3),
        label: fmtSize(m.qty),
        title: `${m.side} ${fmtSize(m.qty)} @ ${fmtPrice(m.y)}`
      });
    }
    return out;
  });
...
>
    ...
    <div class="quick-menu" style:left="{quickMenu.left}px" style:top="{quickMenu.top}px">
      <div class="qrow">
        <button type="button" class="qbtn long" onclick={() => quickSubmit('long')}>
          <Icon name="arrow-up" size={12} /> {$t('histviz.quick.long')}
        </button>
        <button type="button" class="qbtn short" onclick={() => quickSubmit('short')}>
          <Icon name="arrow-down" size={12} /> {$t('histviz.quick.short')}
        </button>
      </div>
      <label class="qqty">
        {$t('histviz.quick.qty')}
  ...
  {#if view}
    <div class="nav">
      ...
      <!-- In fullscreen the page already puts a close button in this corner. -->
      {#if !fullscreen}
        <button
          onclick={() => onfullscreen?.()}
          title={$t('histviz.chart.fullscreen')}
