<script>
  ...
  const palette = $derived.by(() => {
    const up = settings.upColor || cssVar('--green', '#26a69a');
    const down = settings.downColor || cssVar('--red', '#ef5350');
    const accent = settings.lineColor || cssVar('--accent', '#4a90d9');
    return {
      up,
      down,
      green: up,
  ...
  function buildOption(v, p) {
    ...
    // so we pin min/max to the data extent (small margin) — otherwise it spans from a low
    // power of 10 up past the data and squashes everything into a flat sliver.
    const priceLog = settings.scale === 'log' && type !== 'renko' && v.l.every((x) => x > 0);
    let logMin = 0;
    let logMax = 0;
    if (priceLog) {
      // Fit the axis to the *visible* window (from the current zoom), so a zoomed-in slice
      // fills the pane instead of being compressed against the full-history extent.
      ...
      logMin = lo / 1.01;
      logMax = hi * 1.01;
      logRange = { min: logMin, max: logMax };
    }
    ...
    const xAxes = grids.map((_, i) => ({
      ...
      axisLabel: { show: i === grids.length - 1, color: p.muted, fontSize: 10 },
      axisTick: { show: false },
      ...
      splitLine: { show: i === 0 && showV, lineStyle: { color: p.border, opacity: 0.3 } }
    }));
    const yAxes = grids.map((_, i) => {
      ...
      const axis = {
        ...
        splitLine: { show: i === 0 && showH, lineStyle: { color: p.border, opacity: 0.3 } }
      };
      if (isLog) {
    ...
    return {
      ...
      dataZoom: [
        ...
        { type: 'slider', xAxisIndex: allX, bottom: 4, height: 16, start: zoomPct.start, end: zoomPct.end, textStyle: { color: p.muted } }
      ],
      series
    };
  }
  ...
  $effect(() => {
    ...
    // Only the log axis needs to refit on zoom; touch it only then to avoid a feedback loop
    // (setOption → datazoom → setOption) on the far more common linear scale.
    if (settings.scale === 'log') void [zoomPct.start, zoomPct.end];
    void palette;
    if (settings.scale !== 'log') logRange = { min: 0, max: 0 };
    chart.setOption(buildOption(view, palette), { notMerge: true });
...
<style>
  ...
  .ico.candle {
    background: linear-gradient(to right, var(--c) 0 50%, var(--c2) 50% 100%);
    border-radius: 1px;
  }
  .ico.bar {
    background: linear-gradient(to right, var(--c) 0 50%, var(--c2) 50% 100%);
