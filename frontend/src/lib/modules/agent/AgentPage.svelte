<script>
  ...
  import UsageModal from '$lib/modules/agent/UsageModal.svelte';
  import { agentApi, runStream, foldThread } from '$lib/modules/agent/api.js';

  let conversations = $state([]);
  let activeId = $state(null);
  let messages = $state([]); // [{ id, role, text, thinking, tools }]
  let usage = $state({ input_tokens: 0, output_tokens: 0, cache_write_tokens: 0, cache_read_tokens: 0 });
  ...
  async function openConversation(id) {
    ...
    try {
      ...
      messages = foldThread(data.messages);
      usage = data.usage ?? { input_tokens: 0, output_tokens: 0, cache_write_tokens: 0, cache_read_tokens: 0 };
      await scrollBottom(true);
    } catch (e) {
      error = e.message;
    }
  ...
  // Cached prompt tokens count toward what the conversation moved, not toward what it cost:
  // a cache read is billed at a fraction of an input token. They are shown apart for that
  // reason, and only once there are any.
  let cachedTokens = $derived((usage.cache_read_tokens || 0) + (usage.cache_write_tokens || 0));
  let totalTokens = $derived(
    (usage.input_tokens || 0) + (usage.output_tokens || 0) + cachedTokens
  );
  let tokTitle = $derived(
    $t('agent.chat.tokTitle', {
      in: (usage.input_tokens || 0).toLocaleString(),
      out: (usage.output_tokens || 0).toLocaleString()
    }) +
      (usage.cache_read_tokens
        ? ` · ${$t('agent.chat.tokCached', { cached: usage.cache_read_tokens.toLocaleString() })}`
        : '')
  );
</script>
  ...
  <section class="chat">
    <header class="chat-head">
      ...
      {#if activeId && totalTokens > 0}
        <span class="tok" title={tokTitle}>
          <Icon name="zap" size={12} />
          {fmtTokens(totalTokens)} tok
        </span>
      {/if}

      <!-- Usage across every conversation, not just this one: the badge above answers "what
           did this chat cost", this answers "what am I spending". -->
      <button
        class="icon"
        title={$t('agent.usage.title')}
        aria-label={$t('agent.usage.title')}
...
<!-- Mounted only while open: it fetches on mount, so this is also what refreshes it. -->
{#if showUsage}
  <UsageModal bind:open={showUsage} />
{/if}
