<script>
  ...
  async function refreshConfig() {
    try {
      ...
      // The draft box edits THIS conversation's override, so it starts empty ("inherit")
      // rather than pre-filled with the default agent's model.
      modelDraft = activeConv?.model ?? '';
    } catch {
      agentCfg = null;
      providers = [];
    }
  ...
  /** Switch this conversation's provider. Empty = back to inheriting the agent's. */
  async function pickProvider(e) {
    const id = e.target.value || null;
    await saveConvModel({ provider_id: id });
    loadModels();
  }

  /** Save the typed/picked model for this conversation (empty = inherit). */
  async function saveModel(value = modelDraft) {
    modelDraft = value;
    const m = modelDraft.trim();
    if (m === (activeConv?.model ?? '').trim()) return;
    await saveConvModel({ model: m });
  }

  /** Drop this conversation's override and follow the agent settings again. */
  async function clearModelOverride() {
    modelDraft = '';
    await saveConvModel({ provider_id: null, model: '' });
    loadModels();
  }
  ...
  async function loadModels() {
    ...
    if (!pid || !activeProvider?.has_key) return;
    loadingModels = true;
    try {
      pickModels = await agentApi.listProviderModels(pid);
    } catch (e) {
      ...
      {#if providers.length}
        <div class="pick-wrap">
          ...
          {#if pickOpen}
            ...
            <div class="pick-pop">
              <p class="pick-scope">{$t('agent.pick.thisChatOnly')}</p>
              <label class="pick-lbl" for="pick-provider">{$t('agent.set.provider')}</label>
              <select id="pick-provider" value={activeConv?.provider_id ?? ''} onchange={pickProvider}>
                <option value="">{$t('agent.pick.inherit')}</option>
              ...
              {/if}
              {#if modelOverridden}
                <button class="pick-reset" onclick={clearModelOverride}>
                  {$t('agent.pick.useDefault')}
                </button>
              {/if}
            </div>
          {/if}
