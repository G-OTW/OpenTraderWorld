      ...
      {#each memories as m (m.id)}
        <li>
          <div class="main">
            <div class="slug-row">
              <code>{m.slug}</code>
              <!-- Which persona wrote it. Memory is one shared store, so a fact recorded by
                   the day trader must not read as a standing truth for the PM. -->
              {#if m.agent_name}
                <Badge tone="accent">{m.agent_name}</Badge>
              {:else if m.kind === 'agent'}
                <Badge tone="accent">{$t('agent.mem.byAgent')}</Badge>
              {/if}
            </div>
