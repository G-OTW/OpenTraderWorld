...
export const agentApi = {
  ...
  // Skills (name-keyed, md instructions). The list carries `used_by` per skill — one catalog
  // shared by every persona, so an edit is never local to one of them.
  /** Token + request usage over a window, cut by day/hour, persona, model and provider.
   *  `tz` is negated from getTimezoneOffset so the server buckets on the user's own days. */
  usage: (days = 30, bucket = 'day') =>
    req(
      `/agent/usage?days=${days}&bucket=${bucket}&tz=${-new Date().getTimezoneOffset()}`
    ),

  listSkills: () => req('/agent/skills'),
  ...
  // Personas. Each entry is { agent, effective_skills } — the shelf the agent will actually
  // get (its allowlist intersected with the enabled catalog), not the one it declares.
  listAgents: () => req('/agent/agents').then((r) => r.agents),
  createAgent: (a) => req('/agent/agents', { method: 'POST', body: JSON.stringify(a) }).then((r) => r.agent),
  updateOneAgent: (id, a) =>
    req(`/agent/agents/${id}`, { method: 'PUT', body: JSON.stringify(a) }).then((r) => r.agent),
  /** Delete a persona. Its conversations move to the default assistant, they are not lost. */
  deleteAgent: (id) => req(`/agent/agents/${id}`, { method: 'DELETE' }),
  cloneAgent: (id) => req(`/agent/agents/${id}/clone`, { method: 'POST' }).then((r) => r.agent),
  /** Restore a built-in persona's shipped prompt, name and skill bodies. */
  resetAgent: (id) => req(`/agent/agents/${id}/reset`, { method: 'POST' }),

  /** Download URL for a persona, a skill, or the whole shelf (used by an <a download>). */
  shelfExportUrl: ({ agent, skill } = {}) => {
    if (agent) return `/api/agent/shelf/export?agent=${agent}`;
    if (skill) return `/api/agent/shelf/export?skill=${skill}`;
    return '/api/agent/shelf/export';
  },
  importShelf: (bundle) =>
    req('/agent/shelf/import', { method: 'POST', body: JSON.stringify(bundle) }),

  /** Answer a pending write (R5). The run is parked in its tool loop waiting for this. */
  confirmWrite: (conversationId, toolUseId, approve) =>
    req(`/agent/conversations/${conversationId}/confirm`, {
      method: 'POST',
      body: JSON.stringify({ tool_use_id: toolUseId, approve })
    }),

  // Conversations
  listConversations: () => req('/agent/conversations').then((r) => r.conversations),
  /** Open a conversation, optionally with a persona (omitted → the default agent). The type
   *  check is deliberate: passed as a bare event handler this would otherwise ship a click
   *  event as the id and 400. */
  createConversation: (agentId) =>
    req('/agent/conversations', {
      method: 'POST',
      body: JSON.stringify(typeof agentId === 'string' ? { agent_id: agentId } : {})
    }).then((r) => r.conversation),
  getConversation: (id) => req(`/agent/conversations/${id}`),
  /** Patch a conversation: { title }, { mcp_token_id } (null = chat only), { agent_id }. */
 ...
 */
export async function runStream(conversationId, message, handlers, signal, extra = {}) {
  ...
  const dispatch = () => {
    ...
    switch (event) {
      ...
      case 'tool':
        ...
        break;
      case 'confirm':
        // A write is parked server-side until the user answers. The run is still open: the
        // next frame for this tool arrives once a decision is posted (or it times out).
        handlers.onConfirm?.(parsed);
        break;
      case 'done':
