...
{#if role === 'system'}
  <!-- A note about the conversation, not a turn in it: persona handovers land here. Never
       sent to a provider (the server drops system rows when assembling the request). -->
  <div class="marker"><span>{text}</span></div>
{:else}
...
<style>
  /* Handover marker: a rule across the thread with the note centred on it, so the switch
     reads as a boundary between two stretches rather than as something someone said. */
  .marker {
    display: flex;
    align-items: center;
    gap: var(--space-2);
