<script>
  ...
  import PersonaManager from '$lib/modules/agent/PersonaManager.svelte';
  ...
  // Mirrors the backend default (agent/provider.rs): a reply budget that covers a reasoning
  // model's thinking plus the tool call after it. Lower and replies get cut mid tool call.
  ...
  function normalizeAgent(a) {
    a.params = a.params ?? {};
    if (a.params.max_tokens == null) a.params.max_tokens = DEFAULT_MAX_TOKENS;
  ...
  function onKindChange(kind) {
    if (kind === 'anthropic') form.base_url = '';
  }
  ...
  async function saveAgent() {
    ...
    try {
      ...
      params.max_tokens = Number(agent.params?.max_tokens) || DEFAULT_MAX_TOKENS;
...
<div class="settings">
  <div class="tabs" role="tablist">
    ...
    <button role="tab" aria-selected={tab === 'personas'} class:active={tab === 'personas'} onclick={() => (tab = 'personas')}>{$t('agent.set.tabPersonas')}</button>
  </div>
  ...
  {:else}
    ...
    <section>
      ...
      {#if editing}
        <div class="prov-form">
          <Select label={$t('agent.set.type')} options={KIND_OPTS} bind:value={form.kind} onpick={onKindChange} />
