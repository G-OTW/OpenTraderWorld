...
#
# Driven from another script (server-setup.sh, CI): set OTW_NONINTERACTIVE=1 and export
# the answers, one environment variable per question. Nothing is read from the terminal;
# a question whose variable is unset takes its own default.
#
#   NET_CHOICE            1-4, the network mode
#   OTW_DOMAIN            public domain          (mode 4)
#   DUCK_DOMAIN/DUCK_TOKEN/LAN_IP                (mode 3)
#   OTW_HTTP_PORT         http port              (modes 1-2)
#   ADMIN_USER            admin username         (default admin)
#   OTW_ADMIN_PASSWORD    admin password         (default: freshly generated)
#   OTW_LOG               log level
#   OVERWRITE/CLEAN_VOLS/WIPE_VOL/START_NOW      y or N answers to the confirmations
set -euo pipefail

# Everything this script writes holds or borders on secrets (.env, dns.env), so nothing it
# creates is readable by other accounts on the host. Set once, at the top, rather than next
# to a single heredoc: a umask only applies to files created after it.
umask 077
...
# Take every answer from the environment instead of a terminal (see the header).
NONINTERACTIVE="${OTW_NONINTERACTIVE:-0}"
...
prompt_default() {
  ...
  # Driven from another script: the answer is already in the environment under the
  # question's own variable name, and an unset one falls back to the default. This is
  # the only path that never touches a terminal, so an orchestrator cannot hang here.
  if [[ "$NONINTERACTIVE" == "1" ]]; then
    local __pre="${!__var-}"
    printf -v "$__var" '%s' "${__pre:-$__def}"
    info "$__q -> ${!__var:-<empty>}"
    return 0
  fi
...
# Poll the app until it reports an admin account exists. Bounded: on a slow first boot the
# compile/migration step dominates, and a miss only means the password line stays in .env.
wait_for_admin() {
  local url="${APP_URL%/}/api/setup/status" i
  for i in $(seq 1 60); do
    if command -v curl >/dev/null 2>&1 &&
       curl -fsS --max-time 3 "$url" 2>/dev/null | grep -q '"configured":[[:space:]]*true'; then
      return 0
    fi
    sleep 2
  done
  info "Could not confirm the admin account was created; leaving OTW_ADMIN_PASSWORD in .env."
  info "Once you have signed in, blank that line yourself."
  return 1
}

# Blank OTW_ADMIN_PASSWORD in .env, in place, preserving the file's mode.
retire_bootstrap_password() {
  local tmp
  tmp="$(mktemp "${ENV_FILE}.XXXXXX")"
  sed 's/^OTW_ADMIN_PASSWORD=.*/OTW_ADMIN_PASSWORD=/' "$ENV_FILE" > "$tmp"
  chmod 600 "$tmp"
  mv "$tmp" "$ENV_FILE"
  info "Bootstrap password removed from .env (the account now exists; the line is a no-op)."
  info "Restart the stack when convenient to drop it from the container environment too."
}
...
# Pre-set by an orchestrator that has to print the password on its own final card;
# generated here otherwise, which is the normal interactive path.
ADMIN_PASS="${OTW_ADMIN_PASSWORD:-$(gen_secret)}"
...
# The umask covers a file this run created. A .env left by an earlier install keeps whatever
# mode it had, so set it explicitly: this file holds the database password, the master key
# that seals every stored credential, and the bootstrap admin password.
chmod 600 "$ENV_FILE"
...
case "$START_NOW" in
  ...
  *)
    ...
    echo
    # Core creates the admin on first boot from OTW_ADMIN_USER/PASSWORD in .env.
    ...
    echo
    # Now retire it. Once the account exists the value is dead weight that lives in a file
    # and in the container's environment, where `docker inspect` publishes it to anyone who
    # can talk to the daemon. Blanked rather than deleted so the line still documents the
    # headless install path, and the bootstrap is a no-op on an empty value.
    wait_for_admin && retire_bootstrap_password
    bold "Up. Open ${APP_URL} and sign in."
    case "$NET_CHOICE" in
      2) info "If the browser turns the address into https:// and fails to connect, its HTTPS-only"
         info "mode is caching a redirect: open ${APP_URL} in a private window (Chrome incognito,"
         info "Firefox private) to check, then allow HTTP for that address, or pick mode 3 (LAN + HTTPS)." ;;
