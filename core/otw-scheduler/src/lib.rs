...
impl Scheduler {
    ...
    async fn poll_one(&self, feed: &otw_store::feeds::Feed) -> u64 {
        let mut secrets =
            match otw_store::feeds::load_secrets(&self.pool, &self.cipher, feed.id).await {
                Ok(s) => s,
                Err(e) => {
                    let _ = otw_store::feeds::mark_error(&self.pool, feed.id, &format!("secrets: {e}"))
                        .await;
                    return 0;
                }
            };
        // Inline `{{vault.item}}` placeholders in the config resolve straight from the
        // vault — no per-feed secret needed. Keyed by "vault.item" so `substitute` matches.
        if let Err(e) = self.resolve_vault_refs(feed, &mut secrets).await {
            let _ = otw_store::feeds::mark_error(&self.pool, feed.id, &format!("vault: {e}")).await;
            return 0;
        }
    ...
    /// Scan the feed config for `{{vault.item}}` placeholders and resolve each straight
    /// from the vault into `secrets` (keyed by the placeholder body, e.g. "AlphaVantage.apikey").
    /// An unknown vault/item is a hard error so the feed shows why it can't authenticate.
    async fn resolve_vault_refs(
        &self,
        feed: &otw_store::feeds::Feed,
        secrets: &mut std::collections::HashMap<String, String>,
    ) -> anyhow::Result<()> {
        for key in scan_vault_refs(&feed.config) {
            if secrets.contains_key(&key) {
                continue;
            }
            let (vault, item) = key.split_once('.').expect("scan yields dotted keys");
            match otw_store::vault::open_by_names(&self.pool, &self.cipher, vault, item).await? {
                Some(value) => {
                    secrets.insert(key, value);
                }
                None => anyhow::bail!("unknown vault reference {{{{{key}}}}}"),
            }
        }
        Ok(())
    }
...
/// Collect distinct `{{name.item}}` placeholder bodies appearing anywhere in a feed's
/// config JSON (url, headers, query, …). Only tokens with a dot are vault refs; the
/// `{{secret:...}}` form has a colon, not a dot, so it's ignored here.
fn scan_vault_refs(config: &serde_json::Value) -> Vec<String> {
    let mut out = Vec::new();
    let mut seen = std::collections::HashSet::new();
    collect_strings(config, &mut |s| {
        let mut i = 0;
        while let Some(open) = s[i..].find("{{") {
            ...
            // Vault ref = "vault.item": exactly one dot, no colon, both sides non-empty.
            if !body.contains(':') {
                if let Some((v, it)) = body.split_once('.') {
                    if !v.is_empty() && !it.is_empty() && !it.contains('.') && seen.insert(body.to_string())
                    {
...
/// Walk a JSON value, calling `f` on every string leaf.
fn collect_strings(v: &serde_json::Value, f: &mut impl FnMut(&str)) {
    match v {
        serde_json::Value::String(s) => f(s),
        serde_json::Value::Array(a) => a.iter().for_each(|x| collect_strings(x, f)),
        serde_json::Value::Object(o) => o.values().for_each(|x| collect_strings(x, f)),
        _ => {}
    }
}
