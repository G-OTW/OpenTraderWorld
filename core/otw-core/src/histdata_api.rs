...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        // Per-dataset chart layout: indicators, drawings and plot style, so a stored
        // instrument reopens exactly as it was left.
        .route(
            "/api/histviz/layouts/{id}",
            get(get_layout).put(set_layout),
        )
...
// ── Per-dataset chart layout ───────────────────────────────────────────────────

/// GET /api/histviz/layouts/{id} — the saved layout for a dataset, or `null`.
async fn get_layout(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<Value>, ApiError> {
    let layout = store::get_layout(&state.pool, id).await?;
    Ok(Json(layout.unwrap_or(Value::Null)))
}

/// PUT /api/histviz/layouts/{id} — replace it. The client owns the schema; we round-trip
/// the JSON object and only refuse a dataset that does not exist.
async fn set_layout(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(body): Json<Value>,
    ...
    if store::get_dataset(&state.pool, id).await?.is_none() {
        return Err(ApiError::not_found("dataset"));
    }
    store::set_layout(&state.pool, id, &body).await?;
    Ok(Json(body))
}
