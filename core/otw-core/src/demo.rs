...
const RULES: &[(&str, Rule)] = &[
    ...
    // Account broker: a broker account is somebody's real money. Nothing to demo, and the
    // credentials it would ask for belong in the vault, which is closed here too.
    ("/api/brokers", Rule::Deny),
    ...
    // Pulling a broker account reaches outside the box with a credential and bulk-writes
    // the journal, so it is denied like every import that is not a file the visitor dropped.
    ("/api/journal/import/broker", Rule::Deny),
    ...
    // External connectors (mail/Telegram/Slack/Discord…) are out, wholesale, and so is
    // the return path: a public sandbox must not hold a bot that drives it.
    ("/api/control", Rule::Deny),
    ...
    // Reading a real broker account with a real key. The rest of the tax module is a
    // calculator over numbers the user typed, and stays open.
    ("/api/taxcalc/broker", Rule::Deny),
    ...
    // Inbound endpoints: the list and each endpoint's received events are browsable, so
    // the module page and the dashboard's webhook card show the seeded traffic. Every
    // write stays closed: creating one mints a public token and a public URL, and the
    // public receiving route (`/api/hooks`) is denied above.
    ("/api/webhooks", Rule::ReadOnly),
];
...
fn allowed(method: &Method, path: &str) -> bool {
    ...
    // A portfolio's broker import reaches a real account with a real key and writes a
    // ledger from what it answers, under an otherwise writable prefix (the portfolio id
    // sits mid-path, so no prefix rule fits).
    if matches_prefix(path, "/api/portfolios") && path.contains("/import/broker") {
        return false;
    }
...
pub fn live_slot() -> Option<LiveSlot> {
    ...
    loop {
        ...
        match LIVE_OPEN.compare_exchange_weak(open, open + 1, Ordering::Relaxed, Ordering::Relaxed)
        {
...
async fn repin(pool: &PgPool, cipher: &SecretCipher) -> anyhow::Result<()> {
    ...
    let key = match agent_store::provider_key(pool, cipher, prow.id)
        .await?
        .filter(|k| !k.is_empty())
    {
        Some(k) => k,
...
mod tests {
    ...
    #[test]
    fn taking_the_data_out_or_loading_it_in_is_shut() {
        // The sandbox is public and resets on a timer: handing out a zip of the whole
        // demo database, or letting a visitor replace it, are both out of scope.
        assert!(!allowed(&Method::GET, "/api/settings/data/export"));
        assert!(!allowed(&Method::POST, "/api/settings/data/import"));
        assert!(!allowed(&Method::POST, "/api/settings/data/preview"));
    }
    ...
    fn per_ip_budget_is_isolated() {
        ...
        assert!(
            q.allow("b"),
            "`b` must be unaffected by `a` draining its slice"
        );
    }
    ...
    fn global_ceiling_still_caps_many_ips() {
        ...
        assert!(
            !q.allow("d"),
            "global ceiling reached despite `d` being fresh"
        );
    }
