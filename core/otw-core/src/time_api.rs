...
struct EntriesQuery {
    ...
    limit: i64,
    /// Optional `YYYY-MM-DD` window on the run's start (matches the breakdown filters).
    since: Option<String>,
    until: Option<String>,
}
...
async fn list_entries(
    ...
    let parse = |s: &Option<String>| {
        s.as_deref()
            .filter(|v| !v.is_empty())
            .and_then(|v| otw_store::journal_fx::parse_date(v).ok())
    };
    let entries = time_tracker::list_entries(
        &state.pool,
        id,
        q.limit.clamp(1, 500),
        parse(&q.since),
...
#[derive(Deserialize, schemars::JsonSchema)]
pub(crate) struct CreateEntryBody {
    started_at: String,
