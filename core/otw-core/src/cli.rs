...
//!
//! A self-hosted single-user install has no mailbox to send a reset link to, and the second
//! factor is on the device that just went missing — there is no channel the server can trust
//! more than the credential it just lost. So the recovery path is not a route at all: it is an argv branch that connects to the database,
...
pub enum Command {
    ...
    ListUsers,
    /// Recovery for a lost authenticator. The second factor is the one credential the host
    /// CLI has to be able to remove: a phone that is gone cannot be asked for a code, and
    /// there is no e-mail channel to fall back on. The argument for letting the shell do it
    /// is the same as for `reset-password` — whoever can run this already holds
    /// `DATABASE_URL` and could clear the column by hand.
...
pub fn from_args() -> Option<Command> {
    ...
    match args.next()?.as_str() {
        ...
        "disable-totp" => {
            ...
            match username {
                Some(username) => Some(Command::DisableTotp { username }),
                None => usage("disable-totp needs a username"),
            }
        }
        _ => None,
    }
...
fn usage(problem: &str) -> ! {
    ...
    eprintln!("  disable-totp <username>             remove the second factor (lost authenticator)");
    std::process::exit(2);
}
...
pub async fn run(cmd: Command, database_url: &str) -> anyhow::Result<()> {
    ...
    match cmd {
        ...
        Command::DisableTotp { username } => disable_totp(&pool, &username).await,
    }
}

/// Remove an account's second factor and its stored secret, and revoke every session so a
/// cookie opened while the factor was on cannot outlive it.
async fn disable_totp(pool: &PgPool, username: &str) -> anyhow::Result<()> {
    ...
    if !user.totp_enabled {
        println!("'{}' has no second factor enabled; nothing to do.", user.username);
        return Ok(());
    }
    otw_store::disable_totp(pool, user.id).await?;
    otw_store::revoke_all_sessions(pool, user.id).await?;
    ...
    if let Err(e) = otw_store::logs::insert(
        ...
        "otw_core::cli",
        &format!(
            "two-factor authentication removed from the host CLI for account '{}'; all \
             sessions revoked",
            user.username
        ),
    )
    .await
    {
        eprintln!("warning: the change succeeded but could not be written to the app log: {e:#}");
    }

    println!("Two-factor authentication removed for '{}'.", user.username);
    println!("Every existing session was revoked. Sign in with the password, then enrol again");
    println!("from Settings if you still want a second factor.");
    Ok(())
}
...
async fn reset_password(pool: &PgPool, username: &str, from_stdin: bool) -> anyhow::Result<()> {
    ...
    let (password, generated) = if from_stdin {
        ...
        // Held to the same policy the app enforces: a password that arrives through a pipe
        // is still the password that answers the internet.
