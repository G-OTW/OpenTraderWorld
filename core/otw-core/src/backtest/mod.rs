...
pub struct Funding {
    ...
    /// Funding interval in hours (default 8, the common perp cadence). Informational — the
    /// engine accrues the rate continuously per bar, which matches the interval total.
    ...
    for r in 0..m {
        ...
        if funding_on && r > 0 {
            ...
            if dt > 0.0 {
                ...
                let rate = s.funding.annual_rate_pct / 100.0 * (dt / year_secs);
                for a in &assets {
                    if let Some(p) = &a.pos {
