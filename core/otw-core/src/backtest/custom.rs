...
/// Cross-language parity fixture: evaluates `scripts/indicator-parity/cases.json` with the real
/// engine and writes the series next to it, for the JS port (`frontend/src/lib/indicators/`) to
/// diff against. Ignored by default — it writes files and is only meaningful with the companion
/// script. Run it via `scripts/indicator-parity/run.sh`.
#[cfg(test)]
mod parity {
    use super::*;

    /// Deterministic OHLCV, generated by the same integer LCG as the JS side so both languages
    /// evaluate bit-identical inputs.
    fn synth(n: usize, seed: u64) -> (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>) {
        ...
        for _ in 0..n {
            let open = close;
            close = close * (1.0 + (next() - 0.5) * 0.02);
            let high = close.max(open) * (1.0 + next() * 0.005);
            let low = close.min(open) * (1.0 - next() * 0.005);
            let vol = 1000.0 + (next() * 5000.0).floor();
            o.push(open);
            h.push(high);
            l.push(low);
            c.push(close);
    ...
    #[test]
    #[ignore = "writes the parity fixture; run via scripts/indicator-parity/run.sh"]
    fn dump_series() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../scripts/indicator-parity");
        let cases: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(dir.join("cases.json")).unwrap()).unwrap();
        let n = cases["bars"]["n"].as_u64().unwrap() as usize;
        let seed = cases["bars"]["seed"].as_u64().unwrap();
        let (o, h, l, c, v) = synth(n, seed);
        let b = Bars { ticker: "", ts: &[], open: &o, high: &h, low: &l, close: &c, volume: &v };

        let mut out = serde_json::Map::new();
        // Every built-in, read straight from the bars (the `resolve_builtin` path).
        let builtins: Vec<serde_json::Value> = cases["builtins"]
            .as_array()
            .unwrap()
            .iter()
            .map(|spec| {
                let num = |k: &str| spec.get(k).and_then(|x| x.as_f64()).unwrap_or(0.0);
                serde_json::to_value(super::super::resolve_builtin(
                    spec["indicator"].as_str().unwrap(),
                    num("period") as usize,
                    num("fast") as usize,
                    num("slow") as usize,
                    num("mult"),
                    num("signal_period") as usize,
                    &b,
                ))
                .unwrap()
            })
            .collect();
        // Whole graphs (transform nodes, chained indicators, null propagation).
        let defs: Vec<serde_json::Value> = cases["defs"]
            .as_array()
            .unwrap()
            .iter()
            .map(|d| {
                let def: CustomIndicatorDef = serde_json::from_value(d.clone()).unwrap();
                serde_json::to_value(eval(&def, &b)).unwrap()
            })
            .collect();
        out.insert("builtins".into(), builtins.into());
        out.insert("defs".into(), defs.into());
        std::fs::write(dir.join("rust_out.json"), serde_json::to_string(&out).unwrap()).unwrap();
    }
}
