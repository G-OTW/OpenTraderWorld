...
/// "Trail 2% (from +1%, BE at +0.50%)": the trailing stop with whichever steps it configures.
fn trail_text(side: &Value) -> Option<String> {
    let t = side.get("trailing_stop").filter(|v| v.is_object())?;
    let v = t.get("value").and_then(Value::as_f64).filter(|v| *v > 0.0)?;
    let dist = match t.get("kind").and_then(Value::as_str) {
        Some("abs") => fmt(v, 4),
        _ => format!("{}%", fmt(v * 100.0, 2)),
    };
    let pct = |key: &str| t.get(key).and_then(Value::as_f64).filter(|x| *x > 0.0);
    let mut steps = Vec::new();
    if let Some(a) = pct("activate_pct") {
        steps.push(format!("from +{}%", fmt(a * 100.0, 2)));
    }
    if let Some(be) = pct("breakeven_pct") {
        steps.push(format!("BE at +{}%", fmt(be * 100.0, 2)));
    }
    Some(if steps.is_empty() {
        format!("Trail {dist}")
    } else {
        format!("Trail {dist} ({})", steps.join(", "))
    })
}
