...
fn sma_opt(src: &[Option<f64>], period: usize) -> Vec<Option<f64>> {
...
fn ema_opt(src: &[Option<f64>], period: usize) -> Vec<Option<f64>> {
    let mut out = vec![None; src.len()];
    if period == 0 {
        return out;
    }
...
fn wma_opt(src: &[Option<f64>], period: usize) -> Vec<Option<f64>> {
    let mut out = vec![None; src.len()];
    if period == 0 {
        return out;
    }
