...
async fn set_network(
    ...
    // This decides what the world can reach. Moving an install from localhost to `web` is
    // a one-request change of blast radius, so it costs a password.
    ...
    crate::security_events::alert(
        &state,
        crate::security_events::NETWORK_MODE,
        crate::security_events::NETWORK_MODE,
        "Network exposure changed",
        &format!(
            "Mode: {mode}\nPort: {}\nDomain: {}\n\nThis decides what can reach the instance. \
             It takes effect at the next `docker compose up`.",
            input.port,
            if domain.is_empty() { "none" } else { domain },
        ),
    );
    Ok(Json(json!({
        "ok": true,
...
///
/// The token goes through the query builder, never through a formatted URL string:
/// `reqwest::Error`'s Display appends the request URL (`… for url (…)`), so a formatted
/// one would put the token in any error we log — and `app_logs` is readable from Settings
/// and lands in every backup. Nothing here logs the error value for the same reason.
async fn update_duckdns(
    ...
    let body = http
        ...
        .query(&[("domains", sub), ("token", token), ("ip", ip)])
        .send()
        .await
        .and_then(|r| r.error_for_status())
        .map_err(|e| {
            let why = match e.status() {
            ...
            tracing::error!("duckdns update failed: {why}");
