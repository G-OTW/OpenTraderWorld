...
//!   - retries **pending** pairs whose currency the API serves (e.g. a trade logged before
//!     the backfill window, or a past fetch failure) — Frankfurter serves any historical
//!     date back to ECB coverage, so most of these resolve without manual entry;
//!   - resolves pending pairs the API will never serve (USDT, USDC, any currency off the
//!     majors list) from a stored histdata series, when a market for them exists;
...
//! A pair no market can price stays pending for the user to fill in manually; each is
//! attempted at most once per process run to avoid hammering the sources.
//!
...
pub fn spawn(pool: PgPool) {
    tokio::spawn(async move {
        ...
        // Same idea for the histdata pass, which is keyed per currency: EUR resolving from
        // a series says nothing about whether USDT can.
        let mut derived: HashSet<(Date, String)> = HashSet::new();
        ...
        loop {
            ...
            if let Err(e) = retry_histdata(&pool, &mut derived).await {
                tracing::error!("fx histdata resolve failed: {e:#}");
            }
    ...
    while day <= yesterday {
        if is_business_day(day) && !journal_fx::has_full_coverage(pool, day).await? {
            ...
            match fx::fetch_rates(day, recent).await {
                ...
                Ok(None) => mark_majors_pending(pool, day, "no source returned rates").await?,
                Err(e) => {
                    mark_majors_pending(pool, day, "fetch error").await?;
...
/// File a pending task for each major the date is actually missing. A currency already
/// stored for that day is not pending, whatever the fetch just did.
async fn mark_majors_pending(pool: &PgPool, day: Date, reason: &str) -> anyhow::Result<()> {
    for quote in journal_fx::missing_quotes_on(pool, day, &journal_fx::FX_QUOTES).await? {
        journal_fx::mark_pending(pool, day, &quote, reason).await?;
    }
    Ok(())
}

/// Try to resolve pending pairs from the online source. A pending date can be a
/// weekend/holiday (a trade's date), so the fetch targets the last business day on or
/// before it — once a rate exists at-or-before the date, the breakdown's carry-forward
/// read works and the pending task can be cleared.
///
/// Only the majors come through here: the source quotes those and nothing else, and asking
/// it for USDT would burn a request to be told no every tick. Everything else is the
/// histdata pass's business.
async fn retry_pending(pool: &PgPool, attempted: &mut HashSet<Date>) -> anyhow::Result<()> {
    ...
    for p in journal_fx::list_pending(pool).await? {
        ...
        if !journal_fx::FX_QUOTES.contains(&p.quote.as_str()) {
            continue; // not a currency this source knows
        }
        let d = p.pending_date;
        // Resolved since the task was created (by an earlier tick, the histdata pass or a
        // manual entry): the question is whether the breakdown can price it now, which is
        // the same carry-forward read the breakdown itself does.
        if journal_fx::usd_rate_on(pool, &p.quote, d).await?.is_some() {
            journal_fx::clear_pending(pool, d, &p.quote).await?;
            resolved += 1;
            continue;
        }
        ...
        match fx::fetch_rates(target, recent).await {
            ...
            Ok(None) => {} // still uncoverable — falls through to histdata, then manual
            Err(e) => tracing::warn!("fx pending retry failed for {target}: {e:#}"),
        }
        tokio::time::sleep(Duration::from_millis(250)).await;
    }
    if resolved > 0 {
        tracing::info!("fx pending retry resolved {resolved} pair(s)");
    }
    Ok(())
}

/// Price whatever the online source could not, off a stored histdata series.
///
/// This is what makes a Binance book converge: USDT is never on the majors list and never
/// will be, but `USDCUSDT` is a real market the user may already have downloaded, and the
/// rate is read off its daily close instead of being asked of an API that does not quote
/// it. A currency with no stored market is left alone and stays pending for manual entry —
/// this pass never invents a number, and it never fetches.
async fn retry_histdata(pool: &PgPool, derived: &mut HashSet<(Date, String)>) -> anyhow::Result<()> {
    let mut resolved = 0u32;
    for p in journal_fx::list_pending(pool).await? {
        let d = p.pending_date;
        if journal_fx::usd_rate_on(pool, &p.quote, d).await?.is_some() {
        ...
        if !derived.insert((d, p.quote.clone())) {
            continue; // no market for it last time, and nothing downloads between ticks
        }
        match fx_histdata::resolve(pool, &p.quote, d).await {
            // `resolve` stores the rate, and storing it clears this exact task.
            Ok(Some(r)) => {
                tracing::info!("fx {} on {d} derived from {} (histdata)", p.quote, r.ticker);
                resolved += 1;
            }
            Ok(None) => {} // no market stored for this currency — manual entry only
            Err(e) => tracing::warn!("fx histdata resolve failed for {} on {d}: {e:#}", p.quote),
        }
    }
    if resolved > 0 {
        tracing::info!("fx histdata pass resolved {resolved} pair(s)");
    }
    Ok(())
}
