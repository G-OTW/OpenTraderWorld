...
//!
//! Channels are configured once and shared by every producer (reminders, watchlist price
//! alerts, mailbox, inbound webhooks). A channel is only sent to when the user has
//! explicitly enabled it *and* granted it to the producing module. Each attempt's outcome
//! is recorded so the UI can show per-channel status. Failures never interrupt firing:
//! the in-app notification is already written by the time we get here.
...
/// Push one fired notification to the user's enabled channels. Errors are logged and
/// recorded per channel, never propagated.
///
/// `module` is the **producer** — one of [`crate::notif_channels_api::NOTIF_MODULES`] —
/// and is the outer filter: a channel the user did not grant to it is never sent to, even
/// if a caller names its id explicitly. That grant is the whole point of the shared
/// channel list, so it is enforced here rather than at each call site.
///
/// `only` is then the producer's own destination choice: `None` (or an empty slice) means
/// every granted, enabled channel — what reminders do — while a non-empty slice narrows
/// further. That is how a watchlist alert can go to Telegram alone without the user
/// disabling their email channel for everything else. Ids that no longer exist, are
/// disabled, or lost the grant are simply skipped.
///
/// Returns how many channels the message actually reached, so a caller holding an outbox can
/// tell "delivered" from "nowhere to deliver to".
pub async fn dispatch(
    ...
    let channels = match notif_channels::load_enabled(pool, cipher, module).await {
        Ok(c) => c,
        Err(e) => {
    ...
    let channels: Vec<ChannelWithSecret> = match only {
        Some(ids) if !ids.is_empty() => {
            channels.into_iter().filter(|c| ids.contains(&c.id)).collect()
        }
        _ => channels,
    };
    // The link is part of the message: a channel has no in-app view to click through to.
    let body = if notif.url.is_empty() {
        notif.details.clone()
    } else if notif.details.trim().is_empty() {
        notif.url.clone()
    } else {
        format!("{}\n\n{}", notif.details, notif.url)
    };
    // Counted, not assumed: a caller that keeps an outbox needs to know whether the message
    // actually left, and "no channel granted" is a delivery of zero, not a delivery.
    let mut sent = 0usize;
    for ch in channels {
        let res = send_one(http, &ch, &notif.name, &body).await;
...
/// Cap a message to a channel's hard limit (Discord rejects >2000 chars, Telegram >4096)
/// so a long webhook payload degrades to a truncated notification instead of a failed one.
...
async fn send_telegram(
    ...
    let text = cap(plain(title, body), 4096);
    let resp = http
        .post(&url)
...
async fn send_slack(
    ...
    let text = cap(plain(title, body), 40_000);
...
async fn send_discord(
    ...
    let text = cap(plain(title, body), 2000);
