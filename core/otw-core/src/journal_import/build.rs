...
fn build_roundtrip(grid: &Grid, r: &Reader, mapping: &Mapping, ctx: &BuildCtx) -> BuildResult {
    ...
    for (i, row) in grid.rows.iter().enumerate() {
        ...
        let input = TradeInput {
            ...
            exits: vec![],
            brackets: vec![],
            // A broker statement carries no plan: no stop, no discipline tag.
            stop_price: None,
            tags: vec![],
        };
...
/// One fill on the way to a position. A file row builds one, and so does a broker
/// execution, and the folding below is the same either way.
pub struct Fill {
    /// Where it came from: a 1-based line in the source file, or the index of the
    /// execution in what the broker returned.
    pub line: usize,
    pub at: Option<OffsetDateTime>,
    pub ticker: String,
    pub side: String, // "long" = buy, "short" = sell
    pub price: f64,
    pub qty: f64,
    pub fees: f64,
    pub hash_key: String,
...
fn build_executions(grid: &Grid, r: &Reader, mapping: &Mapping, ctx: &BuildCtx) -> BuildResult {
    ...
    let folded = fold_fills(fills, mapping, false);
    errors.extend(folded.errors);
    BuildResult { trades: folded.trades, errors }
}

/// Fold fills into positions: per instrument, chronologically, a fill either extends the
/// open position or closes it, and a position closes into a trade the moment its quantity
/// reaches zero. Over-closing flips the position, except on a **cash account**, where
/// there is nothing to flip into.
///
/// `spot_only` is that distinction. On a spot venue a sell with nothing open is the sale of
/// a holding bought before the window, not a short: inventing the short would file a trade
/// the user never took, so the fill is reported as an error naming the fix (widen the
/// period) and left out. A margin or futures account keeps the flip, which is a real trade
/// there.
pub fn fold_fills(mut fills: Vec<Fill>, mapping: &Mapping, spot_only: bool) -> BuildResult {
    let mut errors = Vec::new();
    // Group per instrument, chronologically. Fills without a date keep source order.
    fills.sort_by(|a, b| {
        a.ticker
            .to_lowercase()
    ...
    for fill in fills {
        ...
        while remaining > 1e-9 {
            match open.as_mut() {
                None if spot_only && fill.side == "short" => {
                    errors.push(RowError {
                        row: fill.line,
                        message: format!(
                            "sold {} {} with nothing open. This is a holding bought \
                             before the period, not a short: widen the period to cover \
                             its purchase",
                            crate::journal_import::fmt_qty(remaining),
                            fill.ticker
                        ),
                        raw: format!("{} {} @ {}", fill.side, fill.ticker, fill.price),
                    });
                    remaining = 0.0;
                }
                None => {
