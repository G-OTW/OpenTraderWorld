...
/// Returns `Ok(None)` when no source could supply a usable, full set of rates.
pub async fn fetch_rates(date: Date, allow_backup: bool) -> Result<Option<FetchedRates>> {
    ...
    match fetch_frankfurter(&client, date).await {
        ...
        Ok(rates) => {
            ...
            // Partial Frankfurter data is still better than nothing if it's complete-ish;
            // but we prefer a full set, so fall through to the backup when allowed.
            if !allow_backup && !rates.is_empty() {
                return Ok(Some(FetchedRates { source: "frankfurter", rates }));
            }
        }
