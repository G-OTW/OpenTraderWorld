...
impl Default for Conventions {
    fn default() -> Self {
        Self {
            qty_sign_is_side: false,
            fees_abs: true,
        }
    }
}

/// Every ledger kind a row can be. Mirrors `otw_store::portfolios::OP_KINDS`; kept here so
/// the mapping layer can validate a default without reaching into the store.
pub const KINDS: &[&str] = &[
...
pub struct Defaults {
    /// What a row with no readable kind is: "" (an error, the default) or any of [`KINDS`].
    /// A file of nothing but purchases, or a dividend export with no kind column at all,
    /// says so once here instead of having a direction invented per row.
    #[serde(default)]
    pub side: String,
}
...
pub(crate) fn sym_key(symbol: &str) -> String {
...
pub struct Stats {
    ...
    pub buys: usize,
    pub sells: usize,
    /// Rows that move cash and no units: income, deposits, withdrawals, fees, taxes.
    pub cash_rows: usize,
...
fn resolve(symbol: &str, mapping: &Mapping, assets: &[store::Asset]) -> Resolution {
    resolve_choice(symbol, mapping.symbols.get(&sym_key(symbol)), assets)
}

/// The same question without a file behind it: what is this symbol, in this portfolio?
/// The broker import asks it of a balance sheet rather than of a grid, and the answer has
/// to be the same one, so both go through here.
pub(crate) fn resolve_choice(
    symbol: &str,
    choice: Option<&SymbolChoice>,
    assets: &[store::Asset],
) -> Resolution {
    ...
    match choice {
        Some(SymbolChoice::Skip) => Resolution {
            state: "skip",
...
pub async fn analyze(
    ...
    let known = istore::existing_hashes(pool, portfolio_id, &hashes)
        .await
        .unwrap_or_default();
    ...
    for op in &built.ops {
        ...
        if let Some(existing) = symbols.iter_mut().find(|s| sym_key(&s.source) == key) {
            ...
            if existing.currency_clash.is_none() {
                existing.currency_clash =
                    clash(op.currency.as_deref(), existing.currency.as_deref());
            }
            continue;
        }
        let r = resolutions
    ...
    for (i, op) in built.ops.iter().enumerate() {
        ...
        // What the row moves in cash. A buy pays its fee on top, everything else nets it out.
        ...
        if !duplicate && !skipped && !unresolved {
            ...
            match op.side {
                "buy" => e.0 += amount,
                "sell" => e.1 += amount,
                // Cash in is a credit, cash out a debit: a fee row is not a sale.
                "deposit" | "dividend" | "interest" | "coupon" => e.1 += amount,
                _ => e.0 += amount,
            }
        }
...
pub async fn commit(
    ...
    let pf = store::get_portfolio(pool, portfolio_id)
        .await?
        .ok_or_else(|| anyhow::anyhow!("portfolio not found"))?;
    let pf_currency = pf.currency.clone();
    let assets = store::list_assets(pool, portfolio_id).await?;
    ...
    for op in &built.ops {
        // A cash row naming no symbol belongs to the portfolio and needs no resolution:
        // asking the user which asset their deposit is would be asking a question with no
        // answer. A cash row that *does* name one still resolves, so a dividend lands on
        // the line that paid it.
        let asset_id = if op.symbol.is_empty() {
            None
        } else {
            let Some(r) = resolutions.get(&sym_key(&op.symbol)) else {
                continue;
            };
            if !r.writes() {
                skipped += 1;
        ...
        // Only a row with no asset needs stamping: an asset row inherits its asset's
        // currency by leaving the column NULL, which is what every hand-typed row means.
        let currency = asset_id.is_none().then(|| {
            op.currency
                .clone()
                .unwrap_or_else(|| pf_currency.to_string())
        });
    ...
    for (key, target) in &mapping.columns {
        ...
        let Some(header) = grid.headers.get(idx) else {
            continue;
        };
...
mod tests {
        ...
        let mut mapping = Mapping {
            ...
            columns: (0..grid.width())
                .map(|i| {
                    (
                        i.to_string(),
                        s.columns.get(&i).cloned().unwrap_or("ignore".into()),
                    )
                })
                .collect(),
            ..Default::default()
    ...
    /// A dividend is a ledger row of its own: it books as income against the line that
    /// paid it, with the amount in `price` and a quantity of 1.
    #[test]
    fn a_dividend_books_as_income_not_as_a_buy() {
        let file = concat!(
            "Date,Symbol,Type,Quantity,Price\n",
            "2026-03-04,VT,Buy,10,118.42\n",
            "2026-03-31,VT,Dividend,0,24.80\n",
        );
        let (_, built) = run(file);
        assert_eq!(built.errors.len(), 0);
        assert_eq!(built.ops.len(), 2);
        assert_eq!(built.ops[1].side, "dividend");
        assert_eq!(built.ops[1].quantity, 1.0);
        assert_eq!(built.ops[1].price, 24.80);
    }

    /// Cash that names no line still books: a deposit belongs to the portfolio, and asking
    /// which asset it is would be asking a question with no answer.
    #[test]
    fn a_deposit_needs_no_symbol() {
        let file = concat!(
            "Date,Symbol,Type,Quantity,Amount\n",
            "2026-01-02,,Deposit,0,5000\n",
            "2026-02-01,,Fee,0,4.90\n",
        );
        let (_, built) = run(file);
        assert_eq!(built.errors.len(), 0);
        assert_eq!(built.ops.len(), 2);
        assert_eq!(built.ops[0].side, "deposit");
        assert!(built.ops[0].symbol.is_empty());
        assert_eq!(built.ops[1].side, "fee");
        assert_eq!(built.ops[1].price, 4.90);
    }

    /// A row that states a kind but no money is still an error: an amount cannot be guessed.
    #[test]
    fn a_cash_row_with_no_amount_is_reported() {
        let file = concat!(
            "Date,Symbol,Type,Quantity,Price\n",
            "2026-03-31,VT,Dividend,0,0\n",
        );
        let (_, built) = run(file);
        assert_eq!(built.ops.len(), 0);
        assert_eq!(built.errors.len(), 1);
        assert_eq!(built.errors[0].row, 2);
    }

    /// A trade still needs its line. The kind vocabulary must not make the symbol optional
    /// for a buy.
    #[test]
    fn a_trade_still_needs_a_symbol() {
        let file = concat!(
            "Date,Symbol,Type,Quantity,Price\n",
            "2026-03-04,,Buy,10,118.42\n"
        );
        let (_, built) = run(file);
        assert_eq!(built.ops.len(), 0);
        assert_eq!(built.errors.len(), 1);
    ...
    fn known_symbols_resolve_themselves_and_unknown_ones_wait() {
        let asset = store::Asset {
            ...
            hist_currency: "USD".into(),
        };
        let mut mapping = Mapping::default();
        assert_eq!(resolve("vt", &mapping, &[asset.clone()]).state, "matched");
        assert_eq!(
            resolve("AAPL", &mapping, &[asset.clone()]).state,
            "unresolved"
        );
