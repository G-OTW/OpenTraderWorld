...
/// `security` is the one producer that is not a module: it is the app itself reporting a
/// sign-in from a source this account has never used. It is listed here so a channel can be
/// granted to it like any other, and so an existing wildcard grant already covers it.
pub const NOTIF_MODULES: &[&str] = &[
    "remindme", "watchlists", "mailbox", "webhooks", "histdata", "histviz", "automator",
    "journal", "backtest", "portfolios", "security",
];
