...
async fn get_feed(
    ...
    let secrets: Vec<Value> = feeds::list_secret_meta(&s.pool, id)
        .await?
        .into_iter()
        .map(|(name, vault_item_id)| json!({ "name": name, "vault_item_id": vault_item_id }))
        .collect();
    Ok(Json(json!({ "feed": feed, "secret_names": secret_names, "secrets": secrets })))
}

#[derive(Deserialize, schemars::JsonSchema)]
pub(crate) struct CreateFeed {
    #[serde(default)]
    name: String,
    kind: String,
    ...
    interval_secs: i32,
    /// Identifying colour `#rrggbb`; absent or empty = none.
    color: Option<String>,
}
...
/// Accept `#rrggbb` only (the picker never produces anything else); an empty
/// string means "no colour" and reaches the store as `Some("")`, which clears it.
fn clean_color(c: Option<&str>) -> Result<Option<&str>, ApiError> {
...
async fn create_feed(
    ...
    let color = clean_color(body.color.as_deref())?.filter(|c| !c.is_empty());
    let feed =
        feeds::create_feed(&s.pool, &body.name, &body.kind, &body.config, interval, color).await?;
...
async fn update_feed(
    ...
    if let Some(c) = patch.color.as_deref() {
        patch.color = clean_color(Some(c))?.map(str::to_string);
    }
...
async fn list_secrets(
    ...
    let secrets: Vec<Value> = feeds::list_secret_meta(&s.pool, id)
        ...
        .map(|(name, vault_item_id)| json!({ "name": name, "vault_item_id": vault_item_id }))
        .collect();
    Ok(Json(json!({ "secret_names": names, "secrets": secrets })))
}
...
struct SetSecret {
    ...
    #[serde(default)]
    value: String,
    /// Centralized vault item to resolve this secret from (wins over `value`).
    #[serde(default)]
...
async fn set_secret(
    ...
    match body.vault_item_id {
        Some(item) => feeds::set_secret_ref(&s.pool, id, body.name.trim(), item).await?,
        None => {
...
#[derive(Deserialize, schemars::JsonSchema)]
pub(crate) struct CreateDashboard {
    #[serde(default)]
    name: String,
}
...
async fn add_dashboard_source(
    ...
    let color = clean_color(body.color.as_deref())?.filter(|c| !c.is_empty());
    let feed =
        feeds::create_feed(&s.pool, &body.name, kind, &body.config, interval, color).await?;
    feeds::recompute_dedup_hash(&s.pool, feed.id).await?;
    apply_rate_limit(&s, feed.id, &rate_limit).await?;
