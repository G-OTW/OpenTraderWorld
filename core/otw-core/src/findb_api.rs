...
//!
//! The catalog has its own release cycle, independent of the app version: the publisher
//! ships a `findb.json` manifest next to the archive, and `GET /api/findb/update-check`
//! compares its snapshot version against the installed one.
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/findb/update-check", get(update_check))
...
// --- snapshot update check ---

/// Successful checks are cached in-process so opening the page doesn't re-hit the
/// publisher; the explicit "check for updates" button passes `force=1` to skip it.
/// Failures are not cached, so the next check retries.
const UPDATE_CHECK_TTL: Duration = Duration::from_secs(6 * 3600);

static SNAPSHOT_CACHE: Mutex<Option<(Instant, findb_import::Manifest)>> = Mutex::new(None);
...
/// Compare the installed snapshot against the published one. An unreachable publisher (or
/// an archive served from a local file, which has no manifest) reports `latest: null`
/// rather than an error: the page then simply offers a re-import.
async fn update_check(
    State(state): State<AppState>,
    Query(p): Query<CheckQuery>,
) -> Result<Json<Value>, ApiError> {
    let meta = findb::get_meta(&state.pool).await?;
    let cached = if p.force {
        None
    } else {
        SNAPSHOT_CACHE
            .lock()
    ...
    let latest = match cached {
        Some(m) => Some(m),
        None => match findb_import::fetch_manifest(&state.http).await {
            Ok(Some(m)) => {
                *SNAPSHOT_CACHE.lock().unwrap() = Some((Instant::now(), m.clone()));
                Some(m)
            }
            Ok(None) => None,
            Err(e) => {
                tracing::warn!("findb update check failed: {e:#}");
                None
            }
        },
    };
    let available = latest
        .as_ref()
        .is_some_and(|m| meta.installed && !m.version.is_empty() && m.version != meta.version);
    Ok(Json(json!({
        "current": meta.version,
        "installed": meta.installed,
        "latest": latest.as_ref().map(|m| m.version.clone()),
        "built_at": latest.as_ref().map(|m| m.built_at.clone()),
        "size": latest.as_ref().map(|m| m.size),
        "upstream_commit": latest.as_ref().map(|m| m.upstream_commit.clone()),
