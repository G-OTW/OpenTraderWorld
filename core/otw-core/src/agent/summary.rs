...
/// Trigger once the prompt the provider actually billed exceeds this many tokens. Measured,
/// not estimated: see [`measured_prompt`].
const SUMMARY_TRIGGER_TOKENS: usize = 12_000;
/// Fallback trigger, on the chars/4 estimate, used only until a first turn has been billed
/// (a resumed conversation, or a provider that reports no usage). Lower on purpose: the
/// estimate covers the messages alone and misses the system prompt and the tool schemas.
const ESTIMATE_TRIGGER_TOKENS: usize = 6000;
...
/// Cheap ~chars-per-token estimate, for the fallback path only.
...
pub async fn maybe_summarize(
    ...
    // What did the last turn really cost? The provider billed a prompt that included the
    // system block, the tool schemas and the module catalog — none of which a chars/4 pass
    // over the stored messages can see. Fall back to the estimate only when nothing has been
    // billed yet.
    let trigger = param_u32(params, "summary_trigger_tokens", SUMMARY_TRIGGER_TOKENS as u32) as usize;
    match agent::last_prompt_tokens(&state.pool, conversation_id).await? {
        Some(billed) if (billed.max(0) as usize) < trigger => return Ok(()),
        Some(_) => {}
        None => {
            let total_chars: usize = conv.summary.len()
                + fresh.iter().map(|m| m.content.to_string().len()).sum::<usize>();
            if est_tokens(total_chars) < ESTIMATE_TRIGGER_TOKENS {
                return Ok(());
            }
        }
    }
    ...
    // Compacting is mechanical work: a transcript in, terse notes out. Running it on the
    // conversation's own model means paying frontier prices for it, so `summary_model` can
    // point at a cheap one on the same provider.
    let summary_model = params
        .get("summary_model")
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|s| !s.is_empty())
    ...
    tracing::info!(
        "agent: rolled {} older messages into summary (model {summary_model})",
        older.len()
    );
    Ok(())
}
