...
fn wire_messages(system: &str, msgs: &[Msg]) -> Vec<Value> {
    ...
    for m in msgs {
        ...
        // Tool results become standalone tool-role messages (order preserved). Text riding
        // along in the same turn (Anthropic's shape for "here are the results, and now do
        // this") follows as its own user message rather than being dropped.
...
impl Provider for OpenAiCompat {
    ...
    async fn stream(
        ...
        // One system slot on this wire format, so the two halves are concatenated. Caching
        // is automatic and prefix-based wherever it exists (OpenAI, DeepSeek, some gateways),
        // which is exactly why the stable half is assembled first: nothing to send, but the
        // order still decides whether there is a reusable prefix at all.
        let mut body = json!({
            "model": req.model,
            "messages": wire_messages(&req.system_text(), &req.messages),
            "stream": true,
            ...
            'outer: while let Some(chunk) = byte_stream.next().await {
                ...
                for data in decoder.push(&bytes) {
                    ...
                    if let Some(u) = v.get("usage").filter(|u| !u.is_null()) {
                        ...
                        // Cached prompt tokens are reported INSIDE prompt_tokens here (the
                        // opposite of Anthropic). Subtract them so `input_tokens` means the
                        // same thing on both wires: prompt tokens actually re-processed.
                        let cached = u
                            .pointer("/prompt_tokens_details/cached_tokens")
                            .or_else(|| u.get("cached_tokens"))
                            .and_then(|x| x.as_i64())
                            .unwrap_or(0) as i32;
                        usage.cache_read_tokens = cached.clamp(0, prompt);
                        usage.input_tokens = prompt - usage.cache_read_tokens;
                        usage.output_tokens =
