...
/// Query for the usage breakdown. Every field has a sane default so the plain URL works.
#[derive(Deserialize)]
struct UsageQuery {
    /// Window length in days, counted back from now. 1 = the last 24 hours.
    #[serde(default)]
    days: Option<i64>,
    /// `day` (default) or `hour`.
    #[serde(default)]
    bucket: Option<String>,
    /// The client's offset from UTC in minutes, so buckets line up with the user's own days.
    /// JavaScript reports it inverted (`getTimezoneOffset`), so the caller negates it.
    #[serde(default)]
    tz: Option<i32>,
}

/// Token and request usage over a window, cut by day/hour, persona, model and provider.
///
/// Counts assistant turns, not user messages: one assistant row is one provider request, so a
/// message that took nine tool rounds is nine requests — which is the number that explains a
/// bill or a rate limit.
async fn usage(
    ...
    let from = time::OffsetDateTime::now_utc() - time::Duration::days(days);
    let report = store::usage_breakdown(
        &state.pool,
        from,
        bucket,
        q.tz.unwrap_or(0),
        // Rows written before attribution existed, or whose persona/provider was deleted.
        "unknown",
    )
    .await?;
    Ok(Json(json!({
        "from": store::rfc3339_string(&from),
        "days": days,
        "bucket": bucket,
...
async fn provider_models(
    ...
    Ok(Json(json!({ "models": models_of(&state, id).await? })))
}

/// The model ids a provider offers, sorted and deduped. Shared with external control,
/// where `/model` has to list the same set the picker shows.
pub(crate) async fn models_of(state: &AppState, id: Uuid) -> Result<Vec<String>, ApiError> {
...
async fn delete_one_agent(
    ...
    if !moved.is_empty() {
        ...
        for conv in &moved {
            store::add_message(&state.pool, *conv, "system", &content, Default::default(), &Default::default())
                .await?;
        }
    }
...
async fn get_conversation(
    ...
    let used = store::conversation_tokens(&state.pool, id).await?;
    Ok(Json(json!({
        ...
        "usage": {
            ...
            "cache_write_tokens": used.cache_write,
...
async fn update_conversation(
    ...
    if let Some(token) = body.mcp_token_id {
        ...
        if let Some(tid) = token {
            match otw_store::mcp::get_token(&state.pool, tid).await? {
                None => return Err(ApiError::bad_request("MCP token not found")),
                Some(tok) if tok.is_expired() => {
                    return Err(ApiError::bad_request("that MCP token has expired"))
                }
                Some(_) => {}
            }
        }
    ...
    if let Some(agent_id) = body.agent_id {
        ...
        if conv.agent_id != agent_id {
            ...
            store::add_message(&state.pool, id, "system", &content, Default::default(), &Default::default())
                .await?;
        }
    }
    let conversation = store::get_conversation(&state.pool, id)
...
struct RunBody {
    ...
    message: String,
    /// Module id of the page the user is looking at (sent by the floating assistant).
    /// When the token grants it, that module's endpoints are preloaded into the prompt.
    #[serde(default)]
    page: Option<String>,
}
...
async fn run(
    ...
    let cfg = prepare_run(&state, conversation_id, text, body.page.as_deref(), guard).await?;
    Ok(Sse::new(agent::run::run(cfg))
        .keep_alive(KeepAlive::new().interval(Duration::from_secs(15))))
}

/// Everything between "a message arrived" and "the loop can start": resolve provider,
/// model and key, persist the user turn, build the tool context, roll up the history and
/// assemble the prompt.
///
/// Extracted from the HTTP handler because the run loop has a second caller that is not a
/// browser: an external control channel answers in a chat client, with no SSE stream to
/// write to. Both must get the identical run — same permissions, same summarisation, same
/// prompt — so there is exactly one place that builds it.
pub(crate) async fn prepare_run(
    state: &AppState,
    conversation_id: Uuid,
    text: &str,
    page: Option<&str>,
    guard: crate::agent::run::RunGuard,
    ...
    let mut tools = match conv.mcp_token_id {
        Some(token_id) => match otw_store::mcp::get_token(&state.pool, token_id).await? {
            // An expired token degrades to chat exactly like a deleted one: the expiry is
            // the user's own dead-man switch, and honouring it only on the network path
            // would leave the in-app agent holding the access it was meant to end.
            Some(tok) if tok.is_expired() => ToolContext::none(),
    ...
    // Assemble the prompt: the stable half (identity, module catalog, memory index, skills)
    // then the volatile tail (clock, current page, rolling summary), then the messages the
    // summary does not cover (capped at HISTORY_WINDOW).
    ...
    let model_for_origin = model.clone();
    let req = ChatRequest {
        model,
        system: system.stable,
        system_tail: system.tail,
        messages,
    ...
    Ok(RunConfig {
        ...
        warnings,
        guard,
        // Stamped on every turn this run persists: the conversation can be moved to another
        // model, provider or persona afterwards without rewriting what this one cost.
        origin: otw_store::agent::MsgOrigin {
            model: model_for_origin,
...
/// How many memories are listed at all. Past this the index would be a page of bare slugs on
/// every turn, which buys nothing the `memory_search` tool does not do better on demand.
const MEMORY_INDEX_LISTED: usize = 120;

/// The system prompt, in two halves.
///
/// Order is the whole point. The first half is byte-identical from one turn to the next (and
/// usually from one run to the next), so a provider can serve it from cache instead of
/// re-reading it; the second holds everything that moves. Put the clock first, as this used
/// to, and every section below it changes with the minute: the catalog, the skills and the
/// memory index — the long, stable, expensive part — are re-billed in full on every single
/// request of every tool round.
struct SystemPrompt {
    /// Identity, module catalog, skills, memory index.
    stable: String,
    /// Clock, current page, rolling summary.
    tail: String,
}

/// Compose it. Bodies are never injected: only the memory index and the skill catalog are,
/// and `memory_read` / `load_skill` pull the rest on demand.
async fn build_system_prompt(
    ...
    if !memories.is_empty() {
        ...
        out.push_str(
            "\n\n## Long-term memory\nFacts you've saved. Use memory_read(slug) for full content, \
             memory_search(query) to find one that is not listed here, memory_write to add or \
             update one. The name in brackets is the persona that wrote the entry — treat one \
             written by another persona as its observation, not as an established fact.\n",
        );
        // This index rides in every prompt, on every turn of every run. Descriptions are the
        // heavy part, so only the most recent MEMORY_INDEX_DESCRIBED carry theirs, and the
        // list itself stops at MEMORY_INDEX_LISTED. Nothing becomes unreachable: what falls
        // off the end is found with memory_search and read by slug. `memory_index` is ordered
        // newest-first, and deliberately NOT by recency of use — this block is the head of
        // the cached prefix, and reordering it on every read would throw that cache away.
        for (i, (slug, desc, author)) in memories.iter().take(MEMORY_INDEX_LISTED).enumerate() {
        ...
        if memories.len() > MEMORY_INDEX_DESCRIBED {
            out.push_str(
                "(Descriptions are shown for the most recent memories only. ",
            );
            if memories.len() > MEMORY_INDEX_LISTED {
                out.push_str(&format!(
                    "{} older memories are not listed at all — find them with memory_search.",
                    memories.len() - MEMORY_INDEX_LISTED
                ));
            } else {
                out.push_str("Read a slug with memory_read when it looks relevant.");
            }
            out.push_str(")\n");
        }
    }
    ...
    // ── Everything below moves; it goes in the tail, after the cache breakpoint. ──
    let mut tail = String::new();

    // The clock. Nothing else gives the model one: it has no time tool, and "prepare today's
    // session" / "last week's trades" / "remind me tomorrow" all need a date. Rounded to the
    // hour on purpose — printing the minute would invalidate the cached prefix on every
    // single request, which costs far more than the minute is worth.
    let now = time::OffsetDateTime::now_utc();
    tail.push_str(&format!(
        "\n\n## Now\n{}-{:02}-{:02}, around {:02}:00 UTC ({}). Dates you compute come from \
         this, never from training.\n",
        now.year(),
        now.month() as u8,
        now.day(),
        now.hour(),
    ...
    // The floating assistant names the page the user is looking at. That module's endpoints
    // ride in the prompt so the first tool round isn't spent discovering them; every other
    // module stays one otw_catalog call away. Ungranted or unknown pages inject nothing.
    if let Some(page) = page.map(|p| p.trim().to_ascii_lowercase()).filter(|p| !p.is_empty()) {
        if crate::mcp::can_read(&tools.permissions, &page) {
            tail.push_str(&format!(
                "\n\n## Current page\nThe user is on the \"{page}\" page right now — \
                 unqualified requests refer to it. Its endpoints:\n"
            ));
            tail.push_str(&crate::mcp::agent_catalog(&tools.permissions, Some(&page)));
            tail.push('\n');
        }
    }
    ...
    if !summary.trim().is_empty() {
        ...
        tail.push_str(summary.trim());
    }

    Ok(SystemPrompt { stable: out, tail })
}
...
fn build_messages(stored: &[store::Message]) -> Vec<Msg> {
    ...
    stored[start..]
        ...
        .filter_map(|m| {
            ...
            // Same ceiling a fresh result gets on its way to the model, so reloading a
            // conversation cannot re-inflate a payload the run had already capped.
            Some(Msg { role, blocks: agent::run::cap_blocks(blocks) })
        })
        .collect()
}
