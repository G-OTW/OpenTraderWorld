...
pub struct Ctx {
    ...
    pub input: Value,
    pub workflow: Value,
    /// Namespaces a non-workflow caller supplies (paper trading reads `stats.*`, `session.*`).
    /// Empty for a workflow run, so the four roots above keep their exact meaning there.
...
impl<'a> Resolver<'a> {
    ...
    /// Render a template for a preview: a path that does not resolve becomes an inline marker
    /// instead of failing the whole string, so the rest of the message is still shown. Only
    /// the editor's preview uses this; a real run refuses a template it cannot resolve.
    pub async fn render_preview(&self, template: &str, ctx: &Ctx) -> Result<String, String> {
        if template.len() > MAX_RENDERED {
            return Err("template too long".into());
        }
        let mut out = String::with_capacity(template.len());
        for part in &split(template)? {
            match part {
                Piece::Text(t) => out.push_str(t),
                Piece::Expr(e) => match self.eval(e, ctx, false).await {
...
pub fn lookup(path: &str, ctx: &Ctx) -> Option<Value> {
    ...
    let mut current = match root {
        ...
        other => ctx.extra.get(other)?.clone(),
    };
    for seg in segments {
