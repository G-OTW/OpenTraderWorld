...
/// How often stale alerts are retired and price samples purged. Housekeeping only — it
/// touches rows the evaluation already ignores, so it doesn't need the refresh cadence.
const HOUSEKEEPING: Duration = Duration::from_secs(3600);

/// Spawn the auto-refresh loop: every tick, refresh each sync-enabled watchlist whose
/// `refresh_secs` has elapsed since its last refresh, then evaluate that list's price
/// alerts. Each list stamps `refreshed_at` even when some items fail, so one bad symbol
/// can't wedge the list into a hot retry loop.
///
/// Alerts are evaluated here rather than in the browser: that is what lets one fire while
/// the page is closed.
pub fn spawn(pool: PgPool, lock: RefreshLock, alert_cx: AlertCx) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            for wl in &due {
                ...
                if let Err(e) = alerts::evaluate(&pool, &alert_cx, wl).await {
                    tracing::error!("watchlist {} alert evaluation failed: {e:#}", wl.id);
                }
            }
            if last_housekeeping.elapsed() >= HOUSEKEEPING {
                last_housekeeping = tokio::time::Instant::now();
                if let Err(e) = otw_store::watchlist_alerts::expire_stale(&pool).await {
                    tracing::error!("expiring watchlist alerts failed: {e:#}");
                }
                if let Err(e) = otw_store::watchlist_alerts::purge_samples(&pool).await {
                    tracing::error!("purging watchlist samples failed: {e:#}");
                }
            }
        }
    });
