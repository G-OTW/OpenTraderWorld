...
/// The reserved provider id of a series that came from a file instead of a provider.
///
/// It is not a connector and never will be: there is no account behind it, nothing to
/// fetch and nothing to append. Keeping it in the same namespace is what lets every
/// reader that works on stored bars (backtests, journal enrichment, the portfolio's
/// factor proxies) find an imported series without knowing it was imported, while
/// [`connector_for`] refuses it loudly the moment something tries to download it.
pub const IMPORT_PROVIDER: &str = "import";

/// Look up a connector by provider id.
pub fn connector_for(provider: &str) -> Result<Box<dyn Connector>> {
    if provider == IMPORT_PROVIDER {
        return Err(anyhow!(
            "this series was imported from a file, so there is no provider to fetch it from: \
             import an updated file to extend it"
        ));
    }
    all_connectors()
        .into_iter()
...
/// Percent-encode one query value, the strict way (RFC 3986 unreserved set only).
///
/// Shared by the keyed connectors: a signed request signs the string it sends, so the
/// encoding and the signature must not disagree about what a symbol looks like.
...
/// A provider's error body is one useful line wrapped in a page of JSON. Bounded, because
/// what a remote service sends is not trusted to be short.
pub(super) fn trim_err(body: &str) -> String {
    let t = body.trim();
    match t.char_indices().nth(300) {
        Some((cut, _)) => format!("{}\u{2026}", &t[..cut]),
        None => t.to_string(),
    }
}
...
mod tests {
    ...
    /// A provider id is stored on every dataset and every connector row, so two connectors
    /// answering to one id would hand each other's data to the same rows.
    #[test]
    fn every_provider_id_is_its_own() {
        let mut seen: Vec<&str> = Vec::new();
        for cap in capabilities() {
            assert!(!cap.provider.is_empty(), "{}: a provider needs an id", cap.label);
            assert!(
                !seen.contains(&cap.provider),
                "two connectors answer to the provider id {}",
                cap.provider
            );
            assert_ne!(
                cap.provider, IMPORT_PROVIDER,
                "the import id is reserved for series that came from a file"
            );
            assert!(
                !cap.timeframes.is_empty() && !cap.asset_types.is_empty(),
                "{}: says nothing about what it can download",
                cap.provider
            );
            seen.push(cap.provider);
        }
    }
