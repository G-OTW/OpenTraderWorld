...
static AV: Capability = Capability {
    ...
    // Free key: 5 requests/minute. One every 12 s is the honest pace.
    min_interval_ms: 12_000,
    searchable: true,
    config_fields: &[],
    testable: false,
    stream_asset_types: &[],
...
static EODHD: Capability = Capability {
    ...
    adjusted: true,
    max_bars_per_req: 5000,
    // Paid plans meter per day, not per second; a light spacing is enough.
    min_interval_ms: 250,
    searchable: true,
    config_fields: &[],
    testable: false,
...
static YAHOO: Capability = Capability {
    ...
    adjusted: true,
    max_bars_per_req: 5000,
    // Unofficial endpoint with no published limit: stay deliberately slow.
    min_interval_ms: 600,
    searchable: true,
    config_fields: &[],
    testable: false,
...
static ALPACA: Capability = Capability {
    ...
    // `1w` is folded from daily bars here rather than asked of the vendor's own `1Week`
    // aggregate: see [`fold_weekly`].
    timeframes: &["1m", "5m", "15m", "1h", "1d", "1w"],
    adjusted: true,
    max_bars_per_req: 10000,
    // Free key: 200 requests/minute.
    min_interval_ms: 350,
    searchable: true,
    config_fields: ALPACA_FIELDS,
    testable: false,
    stream_asset_types: &["equity", "crypto", "option"],
    // Every downloadable timeframe streams: the one-minute source folds up, and the daily and
    // weekly buckets are anchored on Alpaca's own session stamp rather than the UTC epoch.
    stream_timeframes: &["1m", "5m", "15m", "1h", "1d", "1w"],
    stream_note: "Live streams one-minute bars folded up to your timeframe, so the chart \
                  updates once a minute (the daily and weekly candles too, seeded from the \
                  session's own bar). A free key streams IEX equities, crypto and the \
                  indicative options feed and allows one connection at a time; SIP and OPRA \
                  need a paid plan and the connector's Feed setting set to sip.",
};

/// Alpaca's one non-secret setting. Not a credential: which market-data entitlement the key
/// is on decides *which socket* the live feed opens, and asking for one the plan does not
/// have is a refusal rather than thinner data. Optional, because the free entitlement is
/// what a key created today has.
static ALPACA_FIELDS: &[ConfigField] = &[ConfigField {
    name: "feed",
    label: "Market data feed",
    placeholder: "iex",
    kind: "text",
    required: false,
    help: "Which Alpaca market-data entitlement this key is on: iex for the free plan (IEX \
           equities and the indicative options feed), sip for a paid one (full tape and OPRA). \
           Leave empty for iex. It selects the live socket; downloads are unaffected.",
}];
...
static MASSIVE: Capability = Capability {
    ...
    // Free key: 5 requests/minute, and 429 over it.
    min_interval_ms: 12_000,
    searchable: true,
    config_fields: &[],
    testable: false,
    stream_asset_types: &["equity", "etf", "option", "crypto", "fx", "index", "future"],
    // Minute aggregates folded up to every downloadable timeframe, daily and weekly included:
    // their buckets follow the stamp Massive's own bars carry, not the UTC epoch.
    stream_timeframes: &["1m", "5m", "15m", "1h", "1d", "1w"],
    stream_note: "Live streaming is a paid Massive feature: a free (end-of-day) key opens the \
                  socket and is refused at login, while the same key still downloads history. \
                  One live connection per market at a time, and futures are their own \
                  entitlement (Futures Starter and up) on top of the equity one.",
};
...
impl Connector for Generic {
    ...
    /// Only Alpaca declares a setting, and its own parser is the last word on it: a feed
    /// name it does not know would otherwise be stored and only fail on the first live
    /// connect, minutes later and somewhere else.
    fn validate_config(&self, config: &HashMap<String, String>) -> Result<()> {
        match self.kind {
            Kind::Alpaca => crate::live::alpaca_feed(config).map(|_| ()),
            _ => Ok(()),
        }
    }
    ...
    async fn search_symbols(
        ...
        match self.kind {
            Kind::Yahoo => search_yahoo(client, q, asset_type, limit).await,
            Kind::AlphaVantage => search_av(client, secrets, q, limit).await,
            Kind::Eodhd => search_eodhd(client, secrets, q, limit).await,
            Kind::Alpaca => search_alpaca(client, secrets, q, asset_type, limit).await,
            Kind::Massive => search_massive(client, secrets, q, asset_type, limit).await,
        }
    }
}
...
// ── Symbol lookup ────────────────────────────────────────────────────────────────
//
// Each provider spells "search" differently; all of them answer with a symbol the same
// provider's bar endpoint accepts, which is the whole contract the chart needs.

/// Map a provider's own instrument-type label to one of our asset types. Unknown labels
/// fall back to `equity` — the chart only uses this to preselect a type.
fn asset_of(kind: &str) -> &'static str {
    match kind.to_ascii_lowercase().as_str() {
        "etf" | "etp" | "fund" | "mutual fund" | "mutualfund" => "etf",
        "crypto" | "cryptocurrency" | "digital currency" => "crypto",
        "currency" | "fx" | "forex" | "physical currency" => "fx",
        "index" | "indices" => "index",
...
async fn search_yahoo(
    ...
    let quotes = body
        ...
        .ok_or_else(|| anyhow!("yahoo: no search results"))?;
    Ok(quotes
        .iter()
        .filter_map(|q| {
            let symbol = q.get("symbol").and_then(Value::as_str)?;
            let at = asset_of(q.get("quoteType").and_then(Value::as_str).unwrap_or(""));
            if !asset_type.is_empty() && at != asset_type {
                return None;
            }
            Some(SymbolHit {
                ...
                name: q
                    ...
                    .unwrap_or("")
                    .to_string(),
                asset_type: at.to_string(),
                exchange: q
                    .get("exchDisp")
                    .or_else(|| q.get("exchange"))
                    .and_then(Value::as_str)
                    .unwrap_or("")
                    .to_string(),
            })
...
async fn search_av(
    ...
    let url = format!(
        "https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords={}&apikey={key}",
        urlencoding(query)
    );
    let body: Value = crate::rate::send("alphavantage", client.get(&url))
        .await
        .context("alphavantage search request")?
        .error_for_status()
        .context("alphavantage search status")?
        .json()
        .await
        .context("alphavantage search decode")?;
        ...
        .and_then(Value::as_str)
    {
        ...
        return Err(anyhow!("alphavantage: {note}"));
    }
    let matches = body
        .get("bestMatches")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow!("alphavantage: no matches"))?;
    Ok(matches
        .iter()
        .filter_map(|m| {
            let symbol = m.get("1. symbol").and_then(Value::as_str)?;
            Some(SymbolHit {
                symbol: symbol.to_string(),
                name: m.get("2. name").and_then(Value::as_str).unwrap_or("").to_string(),
                asset_type: asset_of(m.get("3. type").and_then(Value::as_str).unwrap_or("")).to_string(),
                exchange: m.get("4. region").and_then(Value::as_str).unwrap_or("").to_string(),
            })
        })
        .take(limit)
        .collect())
...
async fn search_eodhd(
    ...
    let url = format!(
        "https://eodhd.com/api/search/{}?api_token={key}&fmt=json&limit={}",
        urlencoding(query),
        limit.clamp(1, 30),
    );
    let arr: Vec<Value> = crate::rate::send("eodhd", client.get(&url))
        .await
        .context("eodhd search request")?
        .error_for_status()
        .context("eodhd search status")?
        .json()
        .await
        .context("eodhd search decode")?;
    Ok(arr
        .iter()
        .filter_map(|m| {
            let code = m.get("Code").and_then(Value::as_str)?;
            let exch = m.get("Exchange").and_then(Value::as_str).unwrap_or("US");
            Some(SymbolHit {
                // EODHD addresses instruments as CODE.EXCHANGE — that is what its bar
                // endpoints expect, so hand the joined form back as the symbol.
                symbol: format!("{code}.{exch}"),
                name: m.get("Name").and_then(Value::as_str).unwrap_or("").to_string(),
                asset_type: asset_of(m.get("Type").and_then(Value::as_str).unwrap_or("")).to_string(),
...
/// Alpaca has no search endpoint — it publishes the full asset list per class, so cache
/// that (per class) and filter locally, the same way the exchanges are handled.
async fn search_alpaca(
    client: &reqwest::Client,
    secrets: &HashMap<String, String>,
    query: &str,
    asset_type: &str,
    ...
    let (class, at) = match asset_type {
        "crypto" => ("crypto", "crypto"),
        "option" => return Err(anyhow!("Alpaca does not list option contracts by search — enter the OCC symbol")),
        _ => ("us_equity", "equity"),
    };
    let cache_key = format!("alpaca:{class}");
    if let Some(all) = super::cached_symbols(&cache_key) {
        return Ok(super::filter_symbols(&all, query, limit));
    }
    let url = format!(
        "https://api.alpaca.markets/v2/assets?status=active&asset_class={class}"
    );
    let arr: Vec<Value> = crate::rate::send(
        "alpaca",
        client
            .get(&url)
    ...
    .await
    .context("alpaca assets request")?
    .error_for_status()
    .context("alpaca assets status")?
    .json()
    .await
    .context("alpaca assets decode")?;
    let all: Vec<SymbolHit> = arr
        .iter()
        .filter(|a| a.get("tradable").and_then(Value::as_bool) != Some(false))
        .filter_map(|a| {
            let symbol = a.get("symbol").and_then(Value::as_str)?;
            Some(SymbolHit {
                symbol: symbol.to_string(),
                name: a.get("name").and_then(Value::as_str).unwrap_or("").to_string(),
                asset_type: at.to_string(),
                exchange: a.get("exchange").and_then(Value::as_str).unwrap_or("").to_string(),
            })
        })
        .collect();
    super::cache_symbols(&cache_key, &all);
    Ok(super::filter_symbols(&all, query, limit))
}

/// Massive/Polygon reference tickers. `market` narrows the universe to the asset type the
/// caller is browsing; the returned ticker already carries its market prefix, which
/// [`massive_ticker`] leaves alone.
async fn search_massive(
    client: &reqwest::Client,
    secrets: &HashMap<String, String>,
    query: &str,
    asset_type: &str,
    ...
    let url = format!(
        "https://api.massive.com/v3/reference/tickers?search={}&market={market}&active=true&limit={}&apiKey={key}",
        urlencoding(query),
        limit.clamp(1, 50),
    );
    ...
    Ok(arr
        .iter()
        .filter_map(|r| {
            let ticker = r.get("ticker").and_then(Value::as_str)?;
            Some(SymbolHit {
                symbol: ticker.to_string(),
                name: r.get("name").and_then(Value::as_str).unwrap_or("").to_string(),
                asset_type: if asset_type.is_empty() { "equity".into() } else { asset_type.to_string() },
                exchange: r
...
/// Minimal percent-encoding for a query string value (the search terms users type are
/// letters, digits, spaces and the odd punctuation — no need for a dependency).
...
/// Days one weekly request may cover: Alpaca serves 10 000 bars a page and this connector
/// does not follow `next_page_token`, so the daily fetch behind a weekly fold is sliced well
/// under the cap rather than silently truncated.
const ALPACA_WEEKLY_SPAN_DAYS: i64 = 9000;
...
async fn fetch_alpaca(
    ...
    if timeframe == "1w" {
        return fetch_alpaca_weekly(client, secrets, ticker, asset_type, from, to).await;
    }
    ...
    alpaca_bars(client, secrets, ticker, asset_type, tf, from, to).await
}

/// Alpaca's weekly candle, folded from its daily bars.
///
/// The vendor's own `1Week` aggregate is not used: it does not state where its week starts,
/// and a weekly row off by a day lands in the wrong bucket for every cross-asset alignment
/// downstream. Daily bars carry no such ambiguity.
///
/// **The period is the ISO week, Monday through Sunday, whatever the asset.** What differs is
/// where the days stop inside it: an equity or option week is the five sessions ending at
/// Friday's close (four when a holiday falls in it), a crypto week runs through Sunday.
/// Folding on the Monday boundary covers both and matches the weekly candle every other
/// connector stores.
///
/// The first bar of the range is re-read from its own Monday so a chunk boundary lands on a
/// whole week rather than splitting one in two; the rewritten row carries the same stamp, so
/// the store's upsert replaces the partial week with the complete one.
async fn fetch_alpaca_weekly(
    client: &reqwest::Client,
    secrets: &HashMap<String, String>,
    ticker: &str,
    asset_type: &str,
    ...
    let mut daily: Vec<Bar> = Vec::new();
    let mut cursor = crate::timeframe::WEEKLY.bucket(from);
    while cursor < to {
        let stop = (cursor + time::Duration::days(ALPACA_WEEKLY_SPAN_DAYS)).min(to);
        let chunk = alpaca_bars(client, secrets, ticker, asset_type, "1Day", cursor, stop).await?;
        daily.extend(chunk.bars);
        cursor = stop;
    }
    Ok(Chunk { bars: fold_weekly(daily) })
}

/// Fold daily bars into ISO weeks. The week's stamp is its **first** daily bar, a real
/// provider stamp (Alpaca's New York session midnight for equities, UTC midnight for crypto)
/// rather than an invented one: it buckets to the right Monday, it survives a holiday Monday,
/// and the live fold takes its session anchor from it.
fn fold_weekly(mut daily: Vec<Bar>) -> Vec<Bar> {
    daily.sort_by_key(|b| b.ts);
    let mut out: Vec<Bar> = Vec::new();
    let mut week: Option<OffsetDateTime> = None;
    for b in daily {
        let bucket = crate::timeframe::WEEKLY.bucket(b.ts);
        match out.last_mut() {
            Some(cur) if week == Some(bucket) => {
...
/// One Alpaca bars request, at the vendor's own timeframe string.
async fn alpaca_bars(
    client: &reqwest::Client,
    secrets: &HashMap<String, String>,
    ticker: &str,
    asset_type: &str,
...
mod tests {
    ...
    fn day(ts: OffsetDateTime, o: f64, h: f64, l: f64, c: f64, v: f64) -> Bar {
        Bar {
            ts,
            open: o,
            high: h,
    ...
    /// The week is Monday to Sunday and the days inside it stop where the market does:
    /// Friday for a session asset, Sunday for a 24/7 one. Getting this wrong is a weekly
    /// candle that either loses the weekend or splits a week across two rows.
    #[test]
    fn the_weekly_fold_runs_monday_to_sunday_for_both_calendars() {
        // Equity: five sessions stamped at New York midnight, Monday 5 to Friday 9 January.
        let equity = vec![
            day(datetime!(2026-01-05 05:00:00 UTC), 10.0, 11.0, 9.5, 10.5, 100.0),
            day(datetime!(2026-01-06 05:00:00 UTC), 10.5, 12.0, 10.0, 11.0, 100.0),
            day(datetime!(2026-01-07 05:00:00 UTC), 11.0, 11.5, 8.0, 9.0, 100.0),
            day(datetime!(2026-01-08 05:00:00 UTC), 9.0, 10.0, 8.5, 9.5, 100.0),
            day(datetime!(2026-01-09 05:00:00 UTC), 9.5, 10.5, 9.0, 10.2, 100.0),
        ];
        let w = fold_weekly(equity);
        assert_eq!(w.len(), 1, "one week, not five days");
        // Stamped on the week's own first session, never an invented midnight.
        assert_eq!(w[0].ts, datetime!(2026-01-05 05:00:00 UTC));
        assert_eq!(w[0].open, 10.0, "Monday's open");
        assert_eq!(w[0].close, 10.2, "Friday's close");
        assert_eq!((w[0].high, w[0].low), (12.0, 8.0));
        assert_eq!(w[0].volume, 500.0);

        // Crypto: the same week runs two days longer, and Sunday belongs to it, not to the
        // week that starts the next day.
        let crypto = vec![
            day(datetime!(2026-01-05 00:00:00 UTC), 10.0, 11.0, 9.0, 10.0, 10.0),
            day(datetime!(2026-01-10 00:00:00 UTC), 10.0, 13.0, 10.0, 12.0, 10.0),
            day(datetime!(2026-01-11 00:00:00 UTC), 12.0, 12.5, 11.0, 11.5, 10.0),
            day(datetime!(2026-01-12 00:00:00 UTC), 11.5, 14.0, 11.5, 13.0, 10.0),
        ];
        let w = fold_weekly(crypto);
        assert_eq!(w.len(), 2, "Monday starts the second week, Sunday closes the first");
        assert_eq!(w[0].ts, datetime!(2026-01-05 00:00:00 UTC));
        assert_eq!(w[0].close, 11.5, "Sunday's close");
        assert_eq!(w[0].high, 13.0);
        assert_eq!(w[1].ts, datetime!(2026-01-12 00:00:00 UTC));
    }

    /// A holiday Monday does not shift the period: the week is still one row, stamped on the
    /// first session that actually traded.
    #[test]
    fn a_holiday_monday_keeps_one_row_for_the_week() {
        let w = fold_weekly(vec![
            day(datetime!(2026-01-06 05:00:00 UTC), 10.0, 11.0, 9.0, 10.0, 1.0),
            day(datetime!(2026-01-09 05:00:00 UTC), 10.0, 12.0, 9.0, 11.0, 1.0),
        ]);
        assert_eq!(w.len(), 1);
        assert_eq!(w[0].ts, datetime!(2026-01-06 05:00:00 UTC));
        assert_eq!(
            crate::timeframe::WEEKLY.bucket(w[0].ts),
            datetime!(2026-01-05 00:00:00 UTC),
            "it still normalizes onto its ISO Monday"
        );
        assert_eq!(w[0].close, 11.0);
    }
