...
impl Connector for Binance {
    ...
    async fn fetch_chunk(
        ...
        let from_ms = from.unix_timestamp() * 1000;
        let to_ms = to.unix_timestamp() * 1000;
        ...
        for r in rows {
            ...
            // `endTime` is inclusive on Binance's side; the window is half-open on ours.
            if open_ms < from_ms || open_ms >= to_ms {
                continue;
            }
            // The bar still forming is not a period yet: `closeTime` is its last millisecond.
