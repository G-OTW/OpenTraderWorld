...
async fn run_with(
    ...
    let secrets = store::load_creds(pool, crate::histdata_cipher::get(), &job.provider).await?;
    ...
    while cursor < end {
        ...
        let bars = match fetch_retry(connector, client, &secrets, job, cursor, chunk_to).await {
            Ok(b) => b,
            Err(e) => {
