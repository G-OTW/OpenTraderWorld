...
fn validate(input: &GoalInput) -> Result<(), ApiError> {
    ...
    // The colour is echoed straight into a CSS custom property in the frontend, so only
    // an exact `#rrggbb` is accepted here.
    if let Some(c) = input.color.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        let ok = c.len() == 7
            && c.starts_with('#')
