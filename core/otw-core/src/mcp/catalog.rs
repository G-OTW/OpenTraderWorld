...
pub const CATALOG: &[Endpoint] = &[
    ...
    get("journal", "/api/journal/fx/pending", "FX pairs (date + currency) awaiting a rate, with what can still resolve each."),
    get("journal", "/api/journal/fx/quotes", "Currencies that can hold a rate (USD-based): what the journal is booked in, plus the majors."),
    get("journal", "/api/journal/fx/rates/{date}", "FX rates on a date (YYYY-MM-DD)."),
    post("journal", "/api/journal/fx/rates/{date}", "Resolve pending currencies on a date with manual rates.", schema::<crate::fx_api::ResolveBody>),
    // The other two rungs of the resolution chain. Both are single-pair writes, so they
    // are in for the same reason the manual rate is: an agent that can see a breakdown is
    // short a currency should be able to close the gap it just reported.
    post_empty("journal", "/api/journal/fx/pending/{date}/{quote}/histdata", "Resolve one pending pair from a histdata series already in store (USD<ccy>, <ccy>USD, or a stablecoin pair, which assumes the peg held). Stores the derived rate; 404 when no stored market prices that currency, which means only a manual rate will do."),
    post_empty("journal", "/api/journal/fx/pending/{date}/{quote}/download", "Queue the download of a series that could price this currency, as an ordinary histdata job (follow it on /api/histdata/jobs); spends the connector's quota. Refused, naming the currency, when no granted connector serves a market for it."),
    ...
    get("backtest", "/api/backtest/runs/{id}", "One recorded run: the exact settings it was simulated with plus its stats. Read it back to check what a run actually used (echoed costs, sizing, window) instead of trusting what you meant to send; the history list is the way to find the id."),
