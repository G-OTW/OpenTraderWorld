...
/// The instant a bar opens. Distinct from the ledger's `DATE`: a candle file names this
/// column after the period, not after a transaction.
const BAR_TS: &[&str] = &[
    "timestamp", "time", "date", "datetime", "date time", "bar time", "open time",
    "candle time", "period", "period start", "begin", "start", "unix", "unix time",
    "epoch", "utc", "date utc", "horodatage", "heure", "temps", "periode", "debut",
    "fecha", "hora", "marca de tiempo", "periodo", "inicio",
    "zeit", "zeitstempel", "datum", "periode start", "beginn",
    "ora", "data ora", "orario", "inizio", "data hora", "inicio",
];

const BAR_OPEN: &[&str] = &[
    "open", "o", "open price", "opening", "opening price", "first", "ouverture",
    "prix d ouverture", "apertura", "precio de apertura", "eroffnung", "eroffnungskurs",
    "start price", "abertura",
];

const BAR_HIGH: &[&str] = &[
    "high", "h", "high price", "highest", "max", "maximum", "plus haut", "haut", "plus hait",
    "maximo", "massimo", "hoch", "hochst", "hochstkurs", "maxima", "peak", "maxima do dia",
];

const BAR_LOW: &[&str] = &[
    "low", "l", "low price", "lowest", "min", "minimum", "plus bas", "bas", "minimo",
    "tief", "tiefst", "tiefstkurs", "minima", "trough",
];

const BAR_CLOSE: &[&str] = &[
    "close", "c", "close price", "closing", "closing price", "last", "last price",
    "settle", "settlement", "cloture", "prix de cloture", "dernier", "cierre",
    "precio de cierre", "ultimo", "schluss", "schlusskurs", "chiusura", "fechamento",
    "nav", "value", "price",
];

const BAR_VOLUME: &[&str] = &[
    "volume", "vol", "v", "quantity", "qty", "traded volume", "base volume", "size",
    "turnover", "shares traded", "volumen", "volumen negociado", "volumes", "umsatz",
    "handelsvolumen", "volume scambiato", "quantite", "nombre de titres",
];

/// Split/dividend-adjusted columns. Yahoo writes "Adj Close", most others prefix or
/// suffix "adjusted"; an unadjusted file simply maps none of them.
const BAR_ADJ_OPEN: &[&str] = &["adj open", "adjusted open", "adj o", "ouverture ajustee", "apertura ajustada"];
const BAR_ADJ_HIGH: &[&str] = &["adj high", "adjusted high", "adj h", "plus haut ajuste", "maximo ajustado"];
const BAR_ADJ_LOW: &[&str] = &["adj low", "adjusted low", "adj l", "plus bas ajuste", "minimo ajustado"];
const BAR_ADJ_CLOSE: &[&str] = &[
    "adj close", "adjusted close", "adj c", "adjusted closing price", "adj price",
    "cloture ajustee", "cours ajuste", "cierre ajustado", "bereinigter schlusskurs",
    "chiusura rettificata", "fechamento ajustado",
];

/// Every column an OHLCV file can be mapped onto. One row is one **period**, not one
/// transaction: nothing here names a side, a fee or a counterparty.
///
/// `close` leads the price fields on purpose. A file that carries a single price column
/// is a close series (a NAV, an index level), and reading it as an open would put the
/// value on the wrong end of the bar.
pub static HISTDATA: TargetSet = TargetSet(&[
    Target { id: "ts", kind: Kind::Date, synonyms: BAR_TS },
    Target { id: "close", kind: Kind::Num, synonyms: BAR_CLOSE },
    Target { id: "open", kind: Kind::Num, synonyms: BAR_OPEN },
    Target { id: "high", kind: Kind::Num, synonyms: BAR_HIGH },
    Target { id: "low", kind: Kind::Num, synonyms: BAR_LOW },
    Target { id: "volume", kind: Kind::Num, synonyms: BAR_VOLUME },
    Target { id: "adj_close", kind: Kind::Num, synonyms: BAR_ADJ_CLOSE },
    Target { id: "adj_open", kind: Kind::Num, synonyms: BAR_ADJ_OPEN },
    Target { id: "adj_high", kind: Kind::Num, synonyms: BAR_ADJ_HIGH },
    Target { id: "adj_low", kind: Kind::Num, synonyms: BAR_ADJ_LOW },
]);
