...
fn resolve_rates(
    ...
    Rates {
        flat_rate: t.as_ref().and_then(|t| t.flat_rate),
...
fn capital_gains_rate(r: &Rates, context: &str, holding_days: Option<i64>) -> f64 {
    if context == "investing" {
        ...
        // No relief tier matched: flat rate if defined, else income+social.
...
pub fn compute(profile: &Value, scenario: &Value) -> Value {
    ...
    let rates = resolve_rates(regime, marginal_override, social_override, &allowances, &holding_rules);
    ...
    if let Some(brackets) = profile.get("wealth_tax").and_then(Value::as_array) {
        ...
        if pv > 0.0 && !brackets.is_empty() {
            // Marginal brackets: [{ up_to, rate }] sorted ascending; last may omit up_to (top).
            let mut remaining = pv;
            let mut prev_cap = 0.0;
            let mut wealth_tax = 0.0;
            for b in brackets {
                let cap = b.get("up_to").and_then(Value::as_f64).unwrap_or(f64::INFINITY);
                let rate = b.get("rate").and_then(Value::as_f64).unwrap_or(0.0);
