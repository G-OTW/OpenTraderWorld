...
pub fn known_limit(provider: &str) -> Option<&'static str> {
    ...
    match provider {
        ...
        // The account-only brokers: every other one shares its id with a data connector,
        // whose capability already carries the note.
        "ninjatrader" => Some(
            "Trading API: the access token lasts ~90 minutes and is shared. Two sessions per \
             login; a third closes the oldest.",
        ),
