...
//! acknowledge/clear/delete actions.
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/notifications", get(list_notifs).delete(clear_all))
...
fn normalize(input: &mut ReminderInput) {
    input.name = input.name.trim().to_string();
    input.url = input.url.trim().to_string();
    input.link_label = input.link_label.trim().to_string();
}
...
fn validate(input: &ReminderInput) -> Result<(), ApiError> {
    ...
    // Only http(s): the URL ends up in an <a href> and in outgoing channel messages.
    if !input.url.is_empty()
        && !(input.url.starts_with("http://") || input.url.starts_with("https://"))
    {
        return Err(ApiError::bad_request("link must start with http:// or https://"));
    }
    if input.url.chars().count() > 2000 {
        return Err(ApiError::bad_request("link is too long"));
    }
    if let Some(w) = input.weekdays {
        if !(0..=127).contains(&w) {
            return Err(ApiError::bad_request("weekdays must be a mask of 0..127 (Mon=1 … Sun=64)"));
        }
    }
...
async fn clear_all(State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    let removed = reminders::delete_all_notifications(&state.pool).await?;
