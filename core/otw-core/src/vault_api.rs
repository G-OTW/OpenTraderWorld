...
async fn set_item(
    ...
    // Writing a vault item replaces a credential other modules then use. Values are never
    // readable back through this API, so the risk is substitution, not disclosure: swapping
    // an API key for one the attacker controls. That costs a password.
