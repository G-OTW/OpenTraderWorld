...
//!
//! Two providers. Everything is **stored in USD** (the store converts USD → portfolio currency at
//! read time), which for one of them means converting first:
...
//!     come from `/simple/price?vs_currencies=usd` — already USD.
//!   - **stock/etf** → Yahoo Finance `query1.../v8/finance/chart` (last close) for spot, and
//!     `query1.../v1/finance/search` for symbol search. A Yahoo quote is in the **listing's**
//!     currency (Xetra in EUR, SIX in CHF, London in pence), so it is converted to USD with the
//!     journal's FX rates before being stored; a currency with no rate leaves the asset unpriced
//!     rather than storing a foreign number in a USD column.
//!
...
/// Fetch the latest price (most recent close) for a Yahoo ticker, **in the currency the
/// listing quotes in** — a Xetra ETF comes back in EUR, a SIX one in CHF. The caller
/// converts; storing the raw number as USD would price a portfolio in a mix of currencies.
pub async fn stock_quote(ticker: &str) -> Result<Option<(f64, String)>> {
    ...
    let currency = res
        .and_then(|r| r.pointer("/meta/currency"))
        .and_then(Value::as_str)
        .unwrap_or("USD")
        .to_string();
    ...
    let price = res
        ...
        .or_else(|| {
            res.and_then(|r| r.pointer("/indicators/quote/0/close"))
                .and_then(Value::as_array)
                .and_then(|a| a.iter().rev().find_map(Value::as_f64))
        });
    Ok(price.map(|p| (p, currency)))
}

/// A Yahoo quote converted to USD with the journal's FX rates (today, carry-forward).
///
/// `None` when the source has no price **or** when its currency cannot be converted —
/// an unconvertible quote is left unpriced (→ unresolved) rather than stored as if it
/// were dollars, which is what made a EUR-listed holding read 17% rich against a cost
/// basis that *was* converted.
async fn stock_price_usd(pool: &PgPool, ticker: &str) -> Result<Option<f64>> {
    let Some((price, currency)) = stock_quote(ticker).await? else {
        return Ok(None);
    };
    to_usd(pool, price, &currency).await
}

/// Convert a quote into USD. Handles the minor-unit tickers (London quotes in pence,
/// Johannesburg in cents, Tel Aviv in agorot) before asking for a rate.
async fn to_usd(pool: &PgPool, price: f64, currency: &str) -> Result<Option<f64>> {
    let (price, code) = match currency {
        "GBp" | "GBX" | "gbp" => (price / 100.0, "GBP"),
        "ZAc" | "ZAX" => (price / 100.0, "ZAR"),
        "ILA" => (price / 100.0, "ILS"),
        c => (price, c),
    };
    ...
    Ok(otw_store::journal_fx::convert(pool, price, &code, "USD", today).await?)
}
...
async fn spot_for(
    ...
    match a.spot_provider.as_deref() {
        ...
        Some("yahoo") => stock_price_usd(pool, &a.spot_symbol).await,
        ...
        _ => match a.provider.as_str() {
            ...
            _ => stock_price_usd(pool, &a.provider_id).await,
        },
    }
}
...
pub async fn reconcile_portfolio(pool: &PgPool, pf: &store::Portfolio) -> Result<Vec<AssetRecon>> {
    ...
    for a in &assets {
        ...
        let (status, price, note) = match spot_for(pool, a, &crypto).await {
...
pub async fn price_new_asset(pool: &PgPool, a: &store::Asset) -> Result<Option<store::Asset>> {
    ...
    let Some(p) = spot_for(pool, a, &crypto).await? else {
        return Ok(None);
    };
...
pub async fn refresh_portfolio(pool: &PgPool, pf: &store::Portfolio) -> Result<()> {
    ...
    for a in &assets {
        ...
        if let Ok(Some(p)) = spot_for(pool, a, &crypto).await {
            store::set_asset_price(pool, a.id, p).await?;
        }
    }
