...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/api/resources/preview", post(preview_link))
...
async fn add_resource(
    ...
    input.thumb_url = input.thumb_url.trim().to_string();
    validate(&input)?;
...
/// POST /api/resources/preview — find a link's social-card image and keep a copy.
///
/// The bytes are stored as an ordinary upload so the thumbnail is served from us: the
/// page it came from may vanish or change, and the gallery must not phone a third party
/// on every render. `thumb_url: null` = the page was read but declares no usable image.
async fn preview_link(
    State(state): State<AppState>,
    Json(input): Json<PreviewInput>,
) -> Result<Json<Value>, ApiError> {
    let Some(img) = link_preview::thumbnail(&input.link).await? else {
        return Ok(Json(json!({ "thumb_url": Value::Null })));
    };

    let id = Uuid::new_v4();
    let path = state.upload_dir.join(id.to_string());
    tokio::fs::write(&path, &img.bytes)
        .await
        .map_err(|e| ApiError::internal(&format!("writing preview image: {e}")))?;
    ...
    Ok(Json(json!({ "thumb_url": format!("/api/files/{id}"), "source": img.source })))
}
