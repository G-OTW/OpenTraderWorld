...
///
/// A favorite is identified by `(asset_type, symbol)`, which it carries itself; `instrument_id`
/// is a cache re-resolved after every import and is `None` when the current catalog snapshot no
/// longer lists the symbol. The remaining fields then come back empty.
#[derive(Debug, Clone, Serialize, sqlx::FromRow)]
pub struct Favorite {
    pub id: Uuid,
...
pub async fn list_favorites(pool: &PgPool) -> anyhow::Result<Vec<Favorite>> {
    let rows = sqlx::query_as::<_, Favorite>(
        "SELECT f.id, f.instrument_id, f.folder_id, f.note, f.asset_type, f.symbol, \
                COALESCE(i.name, '') AS name, COALESCE(i.currency, '') AS currency, \
                COALESCE(i.exchange, '') AS exchange \
         FROM findb_favorites f \
         LEFT JOIN findb_instruments i ON i.id = f.instrument_id \
...
pub async fn add_favorite(pool: &PgPool, input: &FavoriteInput) -> anyhow::Result<Uuid> {
    let id = Uuid::new_v4();
    // The caller stars an instrument, but the row is keyed by (asset_type, symbol) so it can be
    // re-linked after a catalog re-import — hence INSERT … SELECT rather than plain VALUES.
    // Idempotent: re-favoriting updates the folder/note instead of erroring on the UNIQUE.
    let row: Option<(Uuid,)> = sqlx::query_as(
        "INSERT INTO findb_favorites (id, instrument_id, folder_id, note, asset_type, symbol) \
         SELECT $1, i.id, $3, $4, i.asset_type, i.symbol \
         FROM findb_instruments i WHERE i.id = $2 \
    ...
    // No row means the SELECT matched nothing: the instrument id does not exist.
    row.map(|r| r.0)
        .with_context(|| format!("unknown instrument {}", input.instrument_id))
}
...
/// Clear the catalog before a fresh import.
///
/// Favorites are kept: they carry their own (asset_type, symbol) key. Their cached
/// `instrument_id` is dropped here — ids RESTART IDENTITY, so a stale one would point at an
/// unrelated instrument while the import runs — and `relink_favorites` restores it after.
pub async fn truncate_instruments(pool: &PgPool) -> anyhow::Result<()> {
    let mut tx = pool.begin().await?;
    sqlx::query("UPDATE findb_favorites SET instrument_id = NULL")
        .execute(&mut *tx)
        .await
        .context("unlinking findb favorites")?;
...
/// Point every favorite back at its instrument in the freshly imported catalog.
///
/// Returns the number of favorites the new snapshot no longer lists (they stay, unlinked, and
/// come back on their own if a later snapshot lists the symbol again).
pub async fn relink_favorites(pool: &PgPool) -> anyhow::Result<i64> {
    let mut tx = pool.begin().await?;
    sqlx::query("UPDATE findb_favorites SET instrument_id = NULL WHERE instrument_id IS NOT NULL")
        .execute(&mut *tx)
        .await
        .context("unlinking findb favorites")?;
    // One grouped pass over the catalog beats a correlated lookup per favorite; the catalog can
    // hold two rows for one (asset_type, symbol), so the lowest id wins, deterministically.
    sqlx::query(
        "UPDATE findb_favorites f SET instrument_id = m.id \
         FROM (SELECT asset_type, symbol, MIN(id) AS id FROM findb_instruments \
               GROUP BY asset_type, symbol) m \
         WHERE m.asset_type = f.asset_type AND m.symbol = f.symbol",
    )
    .execute(&mut *tx)
    .await
    .context("relinking findb favorites")?;
    let missing: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM findb_favorites WHERE instrument_id IS NULL")
            .fetch_one(&mut *tx)
            .await
            .context("counting unlinked findb favorites")?;
    tx.commit().await?;
    Ok(missing.0)
}
