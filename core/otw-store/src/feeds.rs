...
pub struct Feed {
    ...
    /// Identifying colour (`#rrggbb`), NULL when the user picked none.
...
const FEED_COLS: &str = "id, name, kind, config, enabled, interval_secs, \
                         last_fetched_at, last_success_at, last_error, color, updated_at";
...
const FEED_COLS_WITH_COUNT: &str =
    "f.id, f.name, f.kind, f.config, f.enabled, f.interval_secs, \
     f.last_fetched_at, f.last_success_at, f.last_error, f.color, f.updated_at, \
...
pub async fn create_feed(
    ...
    let sql = format!(
        "INSERT INTO feeds (id, name, kind, config, interval_secs, color, next_run_at) \
         VALUES ($1, $2, $3, $4, $5, $6, now()) RETURNING {FEED_COLS}, 0::bigint AS item_count"
    );
    let row = sqlx::query_as::<_, FeedRow>(AssertSqlSafe(sql))
        .bind(id)
        .bind(name)
...
pub struct FeedPatch {
    ...
    /// `#rrggbb`, or an empty string to clear the colour.
    pub color: Option<String>,
}
...
pub async fn update_feed(pool: &PgPool, id: Uuid, p: &FeedPatch) -> anyhow::Result<bool> {
    let res = sqlx::query(
        "UPDATE feeds SET \
            ...
            color = CASE WHEN $6 = '' THEN NULL ELSE COALESCE($6, color) END, \
            updated_at = now() \
         WHERE id = $1",
    )
    .bind(id)
...
pub async fn claim_due_feeds(pool: &PgPool, limit: i64) -> anyhow::Result<Vec<Feed>> {
    let sql = format!(
        "WITH eff AS ( \
         ...
         feeds.last_fetched_at, feeds.last_success_at, feeds.last_error, feeds.color, feeds.updated_at, \
