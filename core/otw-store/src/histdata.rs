...
//!
//! Three concerns: project-wide provider credentials (encrypted like feed secrets),
//! the `histdata_datasets` catalog the management page reads, and the
//! `histdata_jobs` download queue drained by the background worker. Bars themselves
//! are bulk-written via [`write_bars`], which also maintains the dataset summary so
//! the catalog never has to scan `histdata_bars`.
...
// ── Provider credentials (project-wide, encrypted) ─────────────────────────────

/// Store (or replace) a named secret for a provider. Plaintext is sealed immediately
...
pub async fn set_cred(
    ...
    sqlx::query(
        "INSERT INTO histdata_provider_creds (id, provider, name, nonce, ciphertext) \
         VALUES ($1, $2, $3, $4, $5) \
...
pub async fn delete_cred(pool: &PgPool, provider: &str, name: &str) -> anyhow::Result<bool> {
    let res = sqlx::query("DELETE FROM histdata_provider_creds WHERE provider = $1 AND name = $2")
        .bind(provider)
        .bind(name)
        .execute(pool)
        .await?;
...
/// Names of the secrets set for a provider (never the values).
pub async fn list_cred_names(pool: &PgPool, provider: &str) -> anyhow::Result<Vec<String>> {
    let rows: Vec<(String,)> = sqlx::query_as(
        "SELECT name FROM histdata_provider_creds WHERE provider = $1 ORDER BY name",
    )
    .bind(provider)
    .fetch_all(pool)
    .await?;
...
/// Decrypt all secrets for a provider into a name→plaintext map (connector use only).
pub async fn load_creds(
    ...
    let rows: Vec<(String, Vec<u8>, Vec<u8>)> = sqlx::query_as(
        "SELECT name, nonce, ciphertext FROM histdata_provider_creds WHERE provider = $1",
    )
    .bind(provider)
    .fetch_all(pool)
    .await?;
...
const JOB_COLS: &str = "id, dataset_id, provider, asset_type, ticker, timeframe, range_from, \
                        range_to, kind, status, chunks_done, chunks_total, bars_written, \
                        error, created_at";
...
pub async fn enqueue_job(pool: &PgPool, j: &NewJob<'_>) -> anyhow::Result<Uuid> {
    ...
    sqlx::query(
        "INSERT INTO histdata_jobs \
           (id, dataset_id, provider, asset_type, ticker, timeframe, range_from, range_to, kind) \
