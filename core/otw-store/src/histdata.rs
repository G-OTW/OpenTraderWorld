...
pub struct ConnectorRow {
    ...
    pub provider: String,
    pub name: String,
    /// Owning module: 'histdata' or 'watchlists'. Each module manages its own list.
    pub scope: String,
    #[serde(with = "time::serde::rfc3339")]
    pub created_at: OffsetDateTime,
}

const CONNECTOR_COLS: &str = "id, provider, name, scope, created_at";

pub async fn list_connectors(pool: &PgPool, scope: &str) -> anyhow::Result<Vec<ConnectorRow>> {
    let sql = format!(
        "SELECT {CONNECTOR_COLS} FROM histdata_connectors WHERE scope = $1 \
...
pub async fn create_connector(
    ...
    let sql = format!(
        "INSERT INTO histdata_connectors (id, provider, name, scope) VALUES ($1, $2, $3, $4) \
...
pub async fn default_connector_for(
    ...
    let sql = format!(
        "SELECT {CONNECTOR_COLS} FROM histdata_connectors \
         WHERE provider = $1 AND scope = 'histdata' \
