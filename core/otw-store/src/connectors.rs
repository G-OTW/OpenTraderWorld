...
pub struct ConnectorRow {
    ...
    /// Non-secret provider settings (an IB Gateway host and port, say), keyed by the field
    /// names the provider declares. Readable on purpose: an address is not a credential,
    /// and a write-only one could not be diagnosed. Secrets stay in the vault.
...
const CONNECTOR_COLS: &str = "c.id, c.provider, c.name, c.config, c.created_at, \
...
pub async fn create(
    ...
    sqlx::query(
        "INSERT INTO histdata_connectors (id, provider, name, config) VALUES ($1, $2, $3, $4)",
    )
        .bind(id)
        .bind(provider)
        .bind(name)
...
/// Replace a connector's non-secret settings wholesale (the API validates the field names
/// against the provider's declared list before calling this).
pub async fn set_config(
    ...
    let res =
        sqlx::query("UPDATE histdata_connectors SET config = $2, updated_at = now() WHERE id = $1")
            .bind(id)
            .bind(config)
            .execute(pool)
            .await
            .context("updating connector config")?;
    Ok(res.rows_affected() > 0)
}
...
/// Same, restricted to the connectors a module is granted. The plain [`default_for`] is
/// the right fallback for a queued job (the grant was checked when it was queued); a live
/// subscription is started from a coordinate the user is looking at right now, so it must
/// not fall back onto a connector the module was never allowed to spend.
pub async fn default_for_module(
    pool: &PgPool,
    provider: &str,
    module: &str,
    ...
    let sql = format!(
        "SELECT {CONNECTOR_COLS} FROM histdata_connectors c \
         WHERE c.provider = $1 \
           AND EXISTS (SELECT 1 FROM connector_modules m \
                       WHERE m.connector_id = c.id AND (m.module = $2 OR m.module = '*')) \
...
/// Everything a connector needs to talk to its provider, as one name→value map: the
/// non-secret `config` first, then the decrypted credentials over it, so a name declared
/// both ways always resolves to the sealed value.
///
...
pub async fn load_creds(
    ...
    let config: Option<(serde_json::Value,)> =
        sqlx::query_as("SELECT config FROM histdata_connectors WHERE id = $1")
            .bind(connector_id)
            .fetch_optional(pool)
            .await?;
    if let Some((serde_json::Value::Object(obj),)) = config {
        for (k, v) in obj {
            // Settings are typed by their form field (a port is a number); the connectors
            // read strings, so numbers and booleans are rendered rather than dropped.
            let text = match v {
