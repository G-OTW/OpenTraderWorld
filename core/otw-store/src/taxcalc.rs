...
pub struct Profile {
    ...
    pub regime: String,
    /// Flat rate on gains (percent). NULL = use the regime template's flat rate.
...
const PROFILE_COLS: &str = "id, name, country, region, currency, person_type, regime, \
    flat_rate, marginal_income_rate, social_charges_rate, allowances, loss_carry, \
    holding_period_rules, wealth_tax, notes, is_custom, created_at, updated_at";
...
pub async fn create_profile(pool: &PgPool, p: &NewProfile<'_>) -> anyhow::Result<Uuid> {
    ...
    sqlx::query(
        "INSERT INTO taxcalc_profiles (id, name, country, region, currency, person_type, regime, \
         flat_rate, marginal_income_rate, social_charges_rate, allowances, loss_carry, \
         holding_period_rules, wealth_tax, notes, is_custom) \
...
pub async fn update_profile(pool: &PgPool, id: Uuid, p: &NewProfile<'_>) -> anyhow::Result<bool> {
    let res = sqlx::query(
        "UPDATE taxcalc_profiles SET name=$2, country=$3, region=$4, currency=$5, person_type=$6, \
         regime=$7, flat_rate=$8, marginal_income_rate=$9, social_charges_rate=$10, \
         allowances=$11, loss_carry=$12, holding_period_rules=$13, wealth_tax=$14, notes=$15, \
         is_custom=$16, updated_at=now() WHERE id=$1",
    )
    .bind(id)
    .bind(p.name)
    .bind(p.country)
