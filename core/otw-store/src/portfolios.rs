...
const ASSET_COLS: &str = "id, portfolio_id, asset_class, provider, provider_id, symbol, name, last_price_usd, last_price_at, spot_provider, spot_symbol, recon_status, recon_checked_at, recon_note";
...
pub async fn add_asset(
    ...
    let sql = format!(
        "INSERT INTO portfolio_assets (id, portfolio_id, asset_class, provider, provider_id, symbol, name) \
         VALUES ($1,$2,$3,$4,$5,$6,$7) \
         ON CONFLICT (portfolio_id, provider, provider_id) DO UPDATE SET symbol = EXCLUDED.symbol, name = EXCLUDED.name \
...
pub async fn list_portfolio_operations(
    pool: &PgPool,
    portfolio_id: Uuid,
) -> anyhow::Result<Vec<(Operation, String)>> {
    let rows = sqlx::query_as::<_, (Uuid, Uuid, String, Date, f64, f64, f64, String, String)>(
        "SELECT o.id, o.asset_id, o.side, o.op_date, o.quantity, o.price, o.fee, o.note, a.symbol \
    ...
    Ok(rows
        .into_iter()
        .map(|(id, asset_id, side, op_date, quantity, price, fee, note, symbol)| {
            (Operation { id, asset_id, side, op_date, quantity, price, fee, note }, symbol)
        })
        .collect())
}
...
/// Walk an asset's operations (oldest first) accumulating an average-cost position.
/// All op amounts are in USD (prices are USD). Returns (open_qty, open_cost_basis_usd,
/// realized_pnl_usd). Buy fee is added to basis; sell fee reduces proceeds.
fn position_usd(ops: &[Operation]) -> (f64, f64, f64) {
...
async fn asset_view(
    ...
    let (qty, basis_usd, realized_usd) = position_usd(ops);
