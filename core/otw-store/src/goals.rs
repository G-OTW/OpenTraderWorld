...
pub struct Goal {
    ...
    pub category: String,
    /// Accent colour (`#rrggbb`); None means the UI picks its default.
...
pub struct GoalInput {
    ...
    #[serde(default)]
    pub category: String,
    /// `#rrggbb`, or null/empty for no colour.
    pub color: Option<String>,
}
...
impl GoalInput {
    /// An empty colour string means "no colour", stored as NULL.
    fn color_value(&self) -> Option<&str> {
        self.color.as_deref().map(str::trim).filter(|s| !s.is_empty())
    }
...
const COLUMNS: &str = "id, name, deadline, details, kpis, category, color, position";
...
pub async fn add_goal(pool: &PgPool, input: &GoalInput) -> anyhow::Result<Goal> {
    ...
    sqlx::query(
        "INSERT INTO goals (id, name, deadline, details, kpis, category, color, position) \
...
pub async fn update_goal(pool: &PgPool, id: Uuid, input: &GoalInput) -> anyhow::Result<bool> {
    let res = sqlx::query(
        "UPDATE goals SET name = $2, deadline = $3, details = $4, kpis = $5, category = $6, \
         color = $7, updated_at = now() WHERE id = $1",
    )
    .bind(id)
    .bind(&input.name)
    .bind(input.deadline_date())
