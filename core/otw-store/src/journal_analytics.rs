...
///
/// Visible to the crate because [`crate::journal_behavior`] reads the same projection: the
/// behaviour metrics measure the same trades, on the same filter, after the same FX pass.
pub(crate) struct Closed {
    ...
    pub(crate) at_utc: OffsetDateTime,
    /// Local entry timestamp, when the trade recorded one.
    pub(crate) entry_at: Option<OffsetDateTime>,
    /// Entry timestamp, unshifted.
    pub(crate) entry_at_utc: Option<OffsetDateTime>,
    pub(crate) hold_min: Option<i64>,
    ...
    /// What the candles said about this trade, when it has been measured against them
    /// (`journal_trade_metrics`). Money is already converted, like every other money
    /// field on this struct.
    pub(crate) market: Option<crate::journal_market::ClosedMarket>,
}
...
/// Convert an optional amount, keeping `None` as `None`. A closure cannot do this: it
/// would have to borrow the FX cache across an await point it also owns.
async fn convert_opt(
    pool: &PgPool,
    fx: &mut crate::journal_fx::FxCache,
    v: Option<f64>,
    from: &str,
    ...
    match v {
        Some(x) => fx.convert(pool, x, from, to, on).await,
        None => Ok(None),
    }
}
...
async fn load(
    ...
    // Market measurements for the same trades, when the enrichment pass has run. One
    // extra query for the whole scope, and the money inside converts in the loop below
    // so nothing takes a second pass over the FX table.
    let trade_ids: Vec<Uuid> = trades.iter().map(|t| t.id).collect();
    let mut metrics = crate::journal_market::metrics_for(pool, &trade_ids).await?;
    ...
    for t in &trades {
        ...
        // The measurement's money columns ride the trade's own currency and the trade's
        // own effective date, exactly like its PnL.
        let market = match metrics.remove(&t.id) {
            Some(m) => {
                let cur = t.currency.as_str();
                let on = effective.date();
                Some(crate::journal_market::ClosedMarket {
                    mae: convert_opt(pool, &mut fx, m.mae, cur, display_currency, on).await?,
                    mfe: convert_opt(pool, &mut fx, m.mfe, cur, display_currency, on).await?,
                    giveback: convert_opt(pool, &mut fx, m.giveback, cur, display_currency, on)
                        .await?,
                    mae_r: m.mae_r,
                    mfe_r: m.mfe_r,
                    exit_efficiency: m.exit_efficiency,
                    time_to_mae_min: m.time_to_mae_min,
                    time_to_mfe_min: m.time_to_mfe_min,
                    vol_pct: m.vol_pct,
                    regime: m.regime,
                    trend: m.trend,
                    stop_distance_atr: m.stop_distance_atr,
                    stop_hit: m.stop_hit,
...
impl Bucket {
    ...
    pub(crate) fn push(&mut self, net: f64) {
        self.trades += 1;
        if net > 0.0 {
            self.wins += 1;
        }
...
pub struct Analytics {
    ...
    /// Behavioural metrics over the same trades: concentration, sizing after losses,
    /// pace after a loss, decay inside the session, after-win vs after-loss.
    pub behavior: crate::journal_behavior::Behavior,
    /// What the candles said, when the market-data enrichment has measured these trades.
    /// `None` = nothing measured in this scope, and the enriched cards draw disabled.
    pub market: Option<crate::journal_market::MarketStats>,
}
...
pub(crate) fn total<I: IntoIterator<Item = f64>>(it: I) -> f64 {
...
pub async fn analytics(
    ...
    Ok(Analytics {
        ...
        points,
        behavior: crate::journal_behavior::behavior(closed),
        market: crate::journal_market::stats(closed),
    })
}
