...
pub struct ImportBatch {
    ...
    pub filename: String,
    /// 'file' or 'broker': where the rows came from. A broker batch names an account
    /// instead of a file, and `filename` carries that account's name.
    pub source: String,
...
const BATCH_SELECT: &str = "SELECT b.id, b.portfolio_id, b.mapping_id, b.mapping_name, b.filename, \
        b.source, b.broker_account_id, b.source_rows, b.imported, b.duplicates, b.failed, b.assets_created, b.created_at, \
...
/// A batch whose rows came from a broker account rather than a file. The account's name
/// stands in for the filename: it is what the history row has to show.
pub async fn add_broker_batch(
    ...
    sqlx::query(
        "INSERT INTO portfolio_import_batches \
            (id, portfolio_id, mapping_name, filename, source_rows, source, broker_account_id) \
         VALUES ($1, $2, '', $3, $4, 'broker', $5)",
    )
    .bind(id)
    .bind(portfolio_id)
    .bind(label)
    ...
    .execute(pool)
    .await
    .context("inserting portfolio broker import batch")?;
    Ok(id)
}

/// The account a portfolio was last aligned with.
...
pub async fn get_broker_sync(
    ...
    sqlx::query_as::<_, BrokerSync>(
        "SELECT portfolio_id, account_id, last_synced_at FROM portfolio_broker_syncs \
         WHERE portfolio_id = $1",
    )
    .bind(portfolio_id)
    .fetch_optional(pool)
    .await
    .context("reading the portfolio's last broker sync")
}
...
pub async fn set_broker_sync(
    ...
    sqlx::query(
        "INSERT INTO portfolio_broker_syncs (portfolio_id, account_id, last_synced_at, updated_at) \
         VALUES ($1, $2, now(), now()) \
         ON CONFLICT (portfolio_id) DO UPDATE SET \
           account_id = EXCLUDED.account_id, last_synced_at = now(), updated_at = now()",
    )
    .bind(portfolio_id)
    .bind(account_id)
    .execute(pool)
    .await
    .context("saving the portfolio's last broker sync")?;
    Ok(())
}
