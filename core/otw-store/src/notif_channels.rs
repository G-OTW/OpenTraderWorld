...
const COLUMNS: &str = "id, kind, name, config, secret_nonce, secret_vault_item, enabled, last_ok, last_error, last_sent_at";
...
/// Each distinct vault used is counted once against its vault-wide quota.
pub async fn load_enabled(
    pool: &PgPool,
    cipher: &SecretCipher,
) -> anyhow::Result<Vec<ChannelWithSecret>> {
    let sql = format!("{SECRET_SELECT} WHERE nc.enabled");
