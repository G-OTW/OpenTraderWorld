...
pub struct Resource {
    ...
    /// Image shown in the gallery display: an uploaded `/api/files/{id}`, an auto-fetched
    /// social preview (stored the same way), or a pasted URL. Empty = placeholder.
...
const COLUMNS: &str = "id, category_id, name, link, description, thumb_url";
...
pub async fn add_resource(pool: &PgPool, input: &ResourceInput) -> anyhow::Result<Resource> {
    ...
    sqlx::query(
        "INSERT INTO resources (id, category_id, name, link, description, thumb_url) \
         VALUES ($1,$2,$3,$4,$5,$6)",
    )
    .bind(id)
    .bind(category_id)
...
pub async fn update_resource(
    ...
    let res = sqlx::query(
        "UPDATE resources SET category_id = $2, name = $3, link = $4, description = $5, \
         thumb_url = $6, updated_at = now() WHERE id = $1",
    )
    .bind(id)
    .bind(category_id)
    .bind(&input.name)
