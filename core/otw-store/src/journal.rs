...
pub async fn breakdown(
    ...
    for t in &trades {
        ...
        if let Some(f) = fx.convert(pool, t.fees, &t.currency, display_currency, date).await? {
            total_fees += f;
        }
        ...
        match net {
            Some(net) => {
                if let Some(m) = fx
                    .convert(pool, trade_margin(t), &t.currency, display_currency, date)
                    .await?
                {
                    total_margin += m;
                }
...
pub async fn report_data(
    ...
    for t in &trades {
        let date = trade_effective_date(t);
        let fees = fx.convert(pool, t.fees, &t.currency, display_currency, date).await?;
