...
pub async fn add_category(pool: &PgPool, name: &str, color: Option<&str>) -> anyhow::Result<Category> {
    let id = Uuid::new_v4();
    // Next position computed inside the INSERT so concurrent adds can't pick the same slot.
    let (position,): (f64,) = sqlx::query_as(
        "INSERT INTO journal_categories (id, name, color, position, is_default) \
         VALUES ($1, $2, $3, \
             (SELECT COALESCE(MAX(position), 0) + 1 FROM journal_categories), FALSE) \
         RETURNING position",
    )
    .bind(id)
    .bind(name)
...
pub async fn add_strategy(
    ...
    let (position,): (f64,) = sqlx::query_as(
        "INSERT INTO journal_strategies (id, name, description, signals, position) \
         VALUES ($1, $2, $3, $4, \
             (SELECT COALESCE(MAX(position), 0) + 1 FROM journal_strategies)) \
         RETURNING position",
    )
    .bind(id)
    .bind(name)
...
pub async fn add_template(
    ...
    let (position,): (f64,) = sqlx::query_as(
        "INSERT INTO journal_templates (id, name, description, fields, position, default_fee_schedule_id) \
         VALUES ($1, $2, $3, $4, \
             (SELECT COALESCE(MAX(position), 0) + 1 FROM journal_templates), $5) \
         RETURNING position",
    )
    .bind(id)
    .bind(name)
...
pub async fn add_fee_schedule(pool: &PgPool, input: &FeeScheduleInput) -> anyhow::Result<FeeSchedule> {
    ...
    let (position,): (f64,) = sqlx::query_as(
        "INSERT INTO journal_fee_schedules (id, name, amount, amount_kind, per, currency, position) \
         VALUES ($1, $2, $3, $4, $5, $6, \
             (SELECT COALESCE(MAX(position), 0) + 1 FROM journal_fee_schedules)) \
         RETURNING position",
    )
    .bind(id)
    .bind(&input.name)
...
pub struct Trade {
    ...
    /// Planned stop for a simple trade (advanced trades carry SL brackets instead).
    /// Feeds the risk-per-trade figure and the R-multiple.
    pub stop_price: Option<f64>,
    /// Ids of the tags attached to this trade. Filled by the list/get read path.
    pub tags: Vec<Uuid>,

    #[serde(with = "time::serde::rfc3339")]
    pub created_at: OffsetDateTime,
    /// Last write to the row. The market-data enrichment compares it against its own
    /// `computed_at` to know which measurements a re-run actually has to redo.
    #[serde(with = "time::serde::rfc3339")]
    pub updated_at: OffsetDateTime,
...
const TRADE_COLUMNS: &str = "id, category_id, template_id, strategy_id, ticker, asset_class, \
    ...
    advanced, cost_basis_method, entries, exits, brackets, stop_price, created_at, updated_at";
...
fn row_to_trade(r: TradeRow) -> Trade {
    ...
    let pnl = compute_pnl(&r.side, &r.cost_basis_method, &entries, &exits, r.multiplier, r.fees);
...
/// PnL of a trade that is not stored yet (import preview). Runs the same leg synthesis
/// and math as the read path, so what the preview shows is exactly what the journal
/// will compute once the trade is committed.
#[derive(Debug, Serialize)]
pub struct PnlPreview {
    pub gross_pnl: Option<f64>,
    pub net_pnl: Option<f64>,
    pub avg_entry: Option<f64>,
    pub avg_exit: Option<f64>,
    pub open_qty: f64,
    pub entry_qty: f64,
}

pub fn preview_pnl(input: &TradeInput) -> PnlPreview {
    // Mirror row_to_trade: advanced trades use their legs, simple trades synthesize one
    // leg per side from the flat price/quantity fields.
    let (entries, exits) = if input.advanced {
        (input.entries.clone(), input.exits.clone())
    } else {
        let leg = |price: Option<f64>, at: Option<OffsetDateTime>| match (price, input.quantity) {
            (Some(price), Some(qty)) => vec![Leg {
                id: String::new(),
                price,
                qty,
        ...
        (
            leg(input.entry_price, input.entry_at),
            leg(input.exit_price, input.exit_at),
        )
    };
    let pnl = compute_pnl(
        &input.side,
        &input.cost_basis_method,
        &entries,
        &exits,
        input.multiplier,
        input.fees,
    ...
    let avg_exit = if exit_qty > 0.0 {
        Some(exits.iter().map(|l| l.price * l.qty).sum::<f64>() / exit_qty)
    } else {
        None
    };
    PnlPreview {
        ...
        avg_exit,
        open_qty: pnl.open_qty,
        entry_qty: entries.iter().map(|l| l.qty).sum(),
    }
}

/// Realized PnL over entry/exit legs, using the trade's cost-basis method.
///
/// - `avg` (default): avg_entry = Σ(price·qty)/Σ(qty); each exit realizes
///   (exit−avg_entry)·qty·mult·dir.
/// - `fifo`: each exit consumes the oldest remaining entry quantity in leg order;
///   each matched slice realizes (exit−entry)·qty·mult·dir. `avg_entry` is still
///   reported as the weighted average (it labels the position, not the basis).
///
...
/// Simple (single-leg) trades produce identical results under both methods.
fn compute_pnl(
    side: &str,
    method: &str,
    entries: &[Leg],
    ...
    let gross: f64 = if method == "fifo" {
        fifo_gross(entries, exits, multiplier, dir)
    } else {
        exits
            .iter()
...
/// FIFO-matched gross PnL: exits consume entry lots oldest-first (leg array order — the
/// order the fills were logged). An exit remainder with no entry left to match is ignored
/// (validation already forbids closing more than was opened).
fn fifo_gross(entries: &[Leg], exits: &[Leg], multiplier: f64, dir: f64) -> f64 {
    let mut lots: Vec<(f64, f64)> = entries.iter().map(|l| (l.price, l.qty)).collect();
    let mut i = 0;
    let mut gross = 0.0;
    for x in exits {
        let mut need = x.qty;
        while need > 1e-12 && i < lots.len() {
            let take = need.min(lots[i].1);
            gross += (x.price - lots[i].0) * take * multiplier * dir;
            lots[i].1 -= take;
            need -= take;
            if lots[i].1 <= 1e-12 {
                i += 1;
...
mod pnl_tests {
    ...
    fn leg(price: f64, qty: f64) -> Leg {
        Leg { id: String::new(), price, qty, at: None, fees: 0.0, signal: None }
    }

    /// Two entries at different prices, partial exit: FIFO realizes against the first
    /// lot only; avg realizes against the blended basis.
    #[test]
    fn fifo_differs_from_avg_on_partial_exit() {
        let entries = [leg(100.0, 1.0), leg(200.0, 1.0)];
        let exits = [leg(150.0, 1.0)];
        let avg = compute_pnl("long", "avg", &entries, &exits, 1.0, 0.0);
        let fifo = compute_pnl("long", "fifo", &entries, &exits, 1.0, 0.0);
        assert_eq!(avg.gross, Some(0.0)); // 150 − avg(150)
        assert_eq!(fifo.gross, Some(50.0)); // 150 − first-in 100
        // avg_entry stays the weighted average under both methods.
        assert_eq!(avg.avg_entry, Some(150.0));
        assert_eq!(fifo.avg_entry, Some(150.0));
        assert_eq!(fifo.open_qty, 1.0);
    }

    /// Fully closed, single-price legs: both methods agree.
    #[test]
    fn fifo_matches_avg_when_fully_closed_single_lot() {
        let entries = [leg(100.0, 2.0)];
        let exits = [leg(110.0, 2.0)];
        let avg = compute_pnl("short", "avg", &entries, &exits, 1.0, 0.0);
        let fifo = compute_pnl("short", "fifo", &entries, &exits, 1.0, 0.0);
        assert_eq!(avg.gross, fifo.gross);
        assert_eq!(avg.gross, Some(-20.0)); // short loses on a rise
    }

    /// One exit spanning two entry lots consumes them oldest-first.
    #[test]
    fn fifo_exit_spans_lots() {
        let entries = [leg(100.0, 1.0), leg(200.0, 1.0)];
        let exits = [leg(250.0, 2.0)];
        let fifo = compute_pnl("long", "fifo", &entries, &exits, 1.0, 0.0);
        assert_eq!(fifo.gross, Some(150.0 + 50.0));
    }

    /// Un-triggering a bracket on edit removes its previously folded synthetic exit leg.
    #[test]
    fn normalize_drops_stale_bracket_exits() {
        let mut input = TradeInput {
            advanced: true,
            entries: vec![leg(100.0, 2.0)],
        ...
        assert_eq!(input.exits.len(), 1);
        assert_eq!(input.exits[0].id, "bracket:b1");

        // Edit: bracket back to untriggered — the synthetic exit must disappear.
        input.brackets[0].triggered = false;
        input.normalize(None);
        assert!(input.exits.is_empty());

        // Re-trigger: folds again exactly once (idempotent).
        input.brackets[0].triggered = true;
        input.normalize(None);
        input.normalize(None);
        assert_eq!(input.exits.len(), 1);
    }
...
/// Full editable trade payload from the client. Also serialized back out by the import
/// preview, which shows an unsaved trade exactly as it would be stored.
#[derive(Debug, Deserialize, Serialize, Default, schemars::JsonSchema)]
pub struct TradeInput {
    ...
    /// Planned stop for a simple trade; advanced trades take theirs from an SL bracket.
    pub stop_price: Option<f64>,
    /// Tag ids to attach to the trade (replaces the existing set on update).
    #[serde(default)]
    pub tags: Vec<Uuid>,
}
...
impl TradeInput {
    ...
    pub fn normalize(&mut self, schedule: Option<&FeeSchedule>) {
        ...
        // Drop previously folded exits whose bracket was edited back to untriggered (or
        // removed) — otherwise an edit that un-triggers a bracket would leave its synthetic
        // exit leg behind and the fold would no longer be idempotent.
        let triggered: std::collections::HashSet<String> = self
            .brackets
            .iter()
            .filter(|b| b.triggered)
            .map(|b| format!("bracket:{}", b.id))
            .collect();
        self.exits.retain(|e| !e.id.starts_with("bracket:") || triggered.contains(&e.id));
...
pub async fn list_trades(pool: &PgPool, filter: &TradeFilter) -> anyhow::Result<Vec<Trade>> {
        ...
        "SELECT {TRADE_COLUMNS} FROM journal_trades \
         WHERE ($1::uuid IS NULL OR category_id = $1) \
           ...
           AND ($7::uuid IS NULL OR EXISTS ( \
                 SELECT 1 FROM journal_trade_tags tt \
                 WHERE tt.trade_id = journal_trades.id AND tt.tag_id = $7)) \
           AND ($8::timestamptz IS NULL OR COALESCE(exit_at, entry_at, created_at) >= $8) \
           AND ($9::timestamptz IS NULL OR COALESCE(exit_at, entry_at, created_at) <= $9) \
    ...
    let mut trades: Vec<Trade> = rows.into_iter().map(row_to_trade).collect();
    crate::journal_analytics::hydrate_tags(pool, &mut trades).await?;
    Ok(trades)
}
...
pub async fn get_trade(pool: &PgPool, id: Uuid) -> anyhow::Result<Option<Trade>> {
    ...
    let mut trades: Vec<Trade> = row.map(row_to_trade).into_iter().collect();
    crate::journal_analytics::hydrate_tags(pool, &mut trades).await?;
    Ok(trades.pop())
}

/// Serialize the typed leg vectors to JSONB for storage (infallible for these types).
pub(crate) fn legs_json<T: Serialize>(legs: &[T]) -> JsonValue {
...
pub async fn add_trade(pool: &PgPool, input: &TradeInput) -> anyhow::Result<Trade> {
    ...
    sqlx::query(
        "INSERT INTO journal_trades \
            (id, category_id, template_id, strategy_id, ticker, asset_class, exchange, side, \
             ...
             advanced, cost_basis_method, entries, exits, brackets, stop_price) \
    ...
    crate::journal_analytics::set_trade_tags(pool, id, &input.tags).await?;
    get_trade(pool, id)
        .await?
...
pub async fn update_trade(pool: &PgPool, id: Uuid, input: &TradeInput) -> anyhow::Result<bool> {
    let res = sqlx::query(
        "UPDATE journal_trades SET \
            ...
            exits = $27, brackets = $28, stop_price = $29, updated_at = now() \
         WHERE id = $1",
    )
    .bind(id)
    .bind(input.category_id)
...
// ── Name lookups (report/export) ─────────────────────────────────────────────

/// id → name map for strategies (used to label exports and report groupings).
pub async fn strategy_names(pool: &PgPool) -> anyhow::Result<std::collections::HashMap<Uuid, String>> {
    let rows: Vec<(Uuid, String)> = sqlx::query_as("SELECT id, name FROM journal_strategies")
        .fetch_all(pool)
        .await
        .context("loading strategy names")?;
    Ok(rows.into_iter().collect())
}

/// id → name map for categories.
pub async fn category_names(pool: &PgPool) -> anyhow::Result<std::collections::HashMap<Uuid, String>> {
    let rows: Vec<(Uuid, String)> = sqlx::query_as("SELECT id, name FROM journal_categories")
        .fetch_all(pool)
        .await
        .context("loading category names")?;
    Ok(rows.into_iter().collect())
}
...
pub struct Breakdown {
    ...
    /// Sharpe over daily returns, annualized. Same definition as the analytics view
    /// (`daily_ratios`), so the two screens never disagree.
    pub sharpe: Option<f64>,
    /// Sortino over the same daily returns, annualized.
    pub sortino: Option<f64>,
    pub max_drawdown: Option<f64>,
    pub best_trade: Option<f64>,
    pub worst_trade: Option<f64>,
    /// Largest margin tied up at any one moment: the peak of the running sum of every
    /// trade's notional / leverage over its holding window. A stock, not a flow — adding
    /// up 91 round trips would say how much margin passed through the account over the
    /// year, which is not comparable to capital or to a PnL.
    pub peak_margin: f64,
    /// Realized PnL as a percentage of `peak_margin` (leverage-aware return).
...
/// Sharpe and Sortino over **daily** returns, annualized (x sqrt(252)).
///
/// One definition for the whole journal: the breakdown and the analytics view must not
/// disagree about what "Sharpe" means. Days are the unit (a per-trade ratio is not
/// annualizable without inventing a trade frequency), and each day's return is measured
/// against the equity the account actually carried into that day, so an early good day on
/// a small account is not flattened by a larger account later.
///
/// `days` is the per-day net PnL in date order. Sortino divides by the RMS of the negative
/// days over *all* days, so a run of flat days does not flatter the ratio.
pub fn daily_ratios(days: &[(time::Date, f64)], invested_capital: f64) -> (Option<f64>, Option<f64>) {
    const ANNUAL: f64 = 252.0;

    let mut returns: Vec<f64> = Vec::with_capacity(days.len());
    let mut equity = invested_capital;
    for (_, net) in days {
        if equity > 0.0 {
            returns.push(net / equity);
        }
        equity += net;
    }
    if returns.len() < 2 {
    ...
    let sd = var.sqrt();
    let sharpe = (sd > 0.0).then(|| mean / sd * ANNUAL.sqrt());

    let downside = (returns.iter().map(|r| r.min(0.0).powi(2)).sum::<f64>()
        / returns.len() as f64)
        .sqrt();
    let sortino = (downside > 0.0).then(|| mean / downside * ANNUAL.sqrt());

    (sharpe, sortino)
}

/// Margin tied up by a trade while it is held: notional (|entry| * qty * multiplier)
/// divided by leverage. Returns 0 when the trade lacks the inputs needed to size it.
...
pub async fn breakdown(
    ...
    // One rate cache for the whole breakdown: capital events + every trade's conversions.
    ...
    for (amount, currency, occurred_at, kind) in &cap_events {
        ...
        match fx
            ...
            .await?
        {
            Some(v) => invested_capital += v,
            // No rate — the event is omitted from invested capital (which skews return %).
            // The cache remembers the (date, currency) that failed; the whole set is queued
            // once at the end of the breakdown.
            None => {}
        }
    }
    ...
    // Margin is measured over time, not summed: each trade contributes +margin while it
    // is held and −margin once it is out. Open trades never give theirs back.
    let mut margin_events: Vec<(OffsetDateTime, f64)> = Vec::new();
    for t in &trades {
        let date = trade_effective_date(t);
        // Fees apply to every trade (open or closed) that has any.
        if let Some(f) = fx
            .convert(pool, trade_total_fees(t), &t.currency, display_currency, date)
            .await?
        {
            total_fees += f;
        }
        match fx.convert(pool, trade_margin(t), &t.currency, display_currency, date).await? {
            Some(m) if m > 0.0 => {
                margin_events.push((t.entry_at.unwrap_or(t.created_at), m));
                if let Some(exit) = t.exit_at {
                    margin_events.push((exit, -m));
                }
            }
            _ => {}
        }
        ...
        let net = fx.convert(pool, native_net, &t.currency, display_currency, date).await?;
        match net {
            Some(net) => closed.push(Conv { t, net }),
            None => unconverted_trades += 1,
        }
    }

    // Every currency/date the pass could not price becomes a pending task, once, whatever
    // it was needed for. Queuing here rather than at each failure means a book with a
    // thousand USDT trades files one task per date, not a thousand writes.
    fx.queue_misses(pool, "needed to convert the breakdown").await?;

    // Sweep the events in time order, releases first at an equal timestamp so a position
    // closed the moment another opens is not counted as held twice.
    margin_events.sort_by(|a, b| a.0.cmp(&b.0).then(a.1.total_cmp(&b.1)));
    let mut running = 0.0;
    let mut peak_margin: f64 = 0.0;
    for (_, delta) in &margin_events {
        running += delta;
        peak_margin = peak_margin.max(running);
    }
    ...
    // fold from +0.0: `Iterator::sum` folds from −0.0, so an empty book would report a
    // realized PnL of "−0.00".
    let realized_pnl: f64 = pnls.iter().fold(0.0, |a, b| a + b);
    ...
    // Sharpe / Sortino over daily returns, annualized. Grouped by the trade's effective
    // local date, exactly like the analytics view.
    let (sharpe, sortino) = if invested_capital > 0.0 {
        let mut by_day: std::collections::BTreeMap<time::Date, f64> =
            std::collections::BTreeMap::new();
        for c in &closed {
            *by_day.entry(trade_effective_date(c.t)).or_insert(0.0) += c.net;
        }
        let days: Vec<(time::Date, f64)> = by_day.into_iter().collect();
        daily_ratios(&days, invested_capital)
    } else {
        (None, None)
    };
    ...
    let return_on_margin = (peak_margin > 0.0).then(|| realized_pnl / peak_margin * 100.0);
...
// ── Report data (periodic Markdown/PDF report) ───────────────────────────────

/// Per-group aggregate for the report's "by strategy" / "by category" tables.
/// Monetary figures are FX-converted into the display currency; unconvertible
/// closed trades are counted in `trades` but excluded from `closed`/`net`/`fees`.
#[derive(Debug)]
...
impl GroupStat {
    pub fn win_rate(&self) -> Option<f64> {
        (self.closed > 0).then(|| self.wins as f64 / self.closed as f64)
    }
    pub fn expectancy(&self) -> Option<f64> {
        (self.closed > 0).then(|| self.net / self.closed as f64)
    }
}

/// One row of the report's trade log (converted into the display currency).
#[derive(Debug)]
pub struct ReportTradeRow {
    ...
    pub avg_exit: Option<f64>,
    pub currency: String,
    /// Trade-level fees, converted. `None` when the FX rate is missing.
    pub fees: Option<f64>,
    /// Net PnL, converted. `None` when still open or the FX rate is missing.
    pub net: Option<f64>,
    pub open: bool,
...
/// Everything the periodic report needs: overall breakdown (stats + equity
/// curve), per-strategy and per-category aggregates, and the trade log.
#[derive(Debug)]
pub struct ReportData {
    pub breakdown: Breakdown,
    pub by_strategy: Vec<GroupStat>,
    pub by_category: Vec<GroupStat>,
    pub trades: Vec<ReportTradeRow>,
}

/// Total entry quantity of a trade (legs for advanced, flat qty otherwise).
pub fn trade_entry_qty(t: &Trade) -> f64 {
    if t.advanced {
        serde_json::from_value::<Vec<Leg>>(t.entries.clone())
            .unwrap_or_default()
            .iter()
...
/// Every fee the trade carries: the trade-level `fees` column plus each leg's own.
///
/// An advanced trade puts the fee on the fill that paid it (an import does this for
/// every execution and leaves the trade-level column at 0), so reading `t.fees` alone
/// reports nothing for those. `compute_pnl` already nets both — this is the same sum,
/// for the places that show fees on their own.
pub fn trade_total_fees(t: &Trade) -> f64 {
    if !t.advanced {
        // Simple mode ignores the leg columns entirely (they may still hold rows from a
        // trade that was advanced once) — exactly like the PnL path.
        return t.fees;
    }
    let leg_fees = |v: &serde_json::Value| {
        serde_json::from_value::<Vec<Leg>>(v.clone())
            .unwrap_or_default()
            .iter()
            .map(|l| l.fees)
            .sum::<f64>()
    };
    t.fees + leg_fees(&t.entries) + leg_fees(&t.exits)
}

/// Weighted-average exit price across exit legs (flat exit in simple mode).
pub fn trade_avg_exit(t: &Trade) -> Option<f64> {
    if t.advanced {
        let exits = serde_json::from_value::<Vec<Leg>>(t.exits.clone()).unwrap_or_default();
        let qty: f64 = exits.iter().map(|l| l.qty).sum();
        (qty > 0.0).then(|| exits.iter().map(|l| l.price * l.qty).sum::<f64>() / qty)
    } else {
        t.exit_price
    }
}

/// Build the report dataset for a filter, in `display_currency`. Reuses
/// `breakdown` for the headline stats and equity curve, then groups the same
/// trades by strategy and category (FX-converted per trade, like the breakdown).
pub async fn report_data(
    ...
    let breakdown = breakdown(pool, filter, display_currency).await?;
    let strategies = strategy_names(pool).await?;
    let categories = category_names(pool).await?;

    let mut trades = list_trades(pool, filter).await?;
    trades.sort_by_key(|t| t.exit_at.or(t.entry_at).unwrap_or(t.created_at));

    // Group accumulators keyed by the optional owning id (None = "no strategy").
    let mut by_strategy: HashMap<Option<Uuid>, GroupStat> = HashMap::new();
    let mut by_category: HashMap<Option<Uuid>, GroupStat> = HashMap::new();
    let mut rows: Vec<ReportTradeRow> = Vec::with_capacity(trades.len());
    ...
    for t in &trades {
        ...
        let fees = fx
            .convert(pool, trade_total_fees(t), &t.currency, display_currency, date)
            .await?;
        let net = match t.net_pnl {
            Some(native) => {
                fx.convert(pool, native, &t.currency, display_currency, date).await?
            }
            None => None,
        };

        let strategy_name = t.strategy_id.and_then(|id| strategies.get(&id).cloned());
        ...
        for (map, key, name) in [
            (&mut by_strategy, t.strategy_id, strategy_name.clone()),
            (&mut by_category, Some(t.category_id), Some(category_name.clone())),
        ] {
            let g = map.entry(key).or_insert_with(|| GroupStat {
        ...
        rows.push(ReportTradeRow {
            date: t.exit_at.or(t.entry_at),
            ticker: t.ticker.clone(),
            side: t.side.clone(),
            qty: trade_entry_qty(t),
            avg_entry: t.avg_entry,
            avg_exit: trade_avg_exit(t),
            currency: t.currency.clone(),
            fees,
            net,
            open: t.net_pnl.is_none(),
            strategy: strategy_name,
            category: category_name,
        });
    }

    // Sort groups by net PnL, best first.
    let sort = |m: HashMap<Option<Uuid>, GroupStat>| -> Vec<GroupStat> {
        let mut v: Vec<GroupStat> = m.into_values().collect();
        v.sort_by(|a, b| b.net.partial_cmp(&a.net).unwrap_or(std::cmp::Ordering::Equal));
        v
    };

    Ok(ReportData {
        breakdown,
        by_strategy: sort(by_strategy),
        by_category: sort(by_category),
        trades: rows,
    })
}

// ── Calendar (daily PnL heatmap) ─────────────────────────────────────────────

/// One calendar cell: realized net PnL (display currency) and trade tallies for a
/// single day, keyed by the trade's effective date (exit, else entry, else creation).
#[derive(Debug, Serialize)]
pub struct CalendarDay {
    /// Local calendar date, "YYYY-MM-DD".
    pub date: String,
    /// Realized net PnL for the day (converted). Excludes still-open trades and
    /// closed trades whose FX rate was unavailable.
    pub net_pnl: f64,
    /// Trades whose effective date falls on this day (open + closed + unconverted).
    pub trades: i64,
    pub wins: i64,
    pub losses: i64,
}

/// The date used to price a trade's FX conversion, and the full effective timestamp
/// used to bucket it into a calendar day: its exit, else entry, else creation.
fn trade_effective_at(t: &Trade) -> OffsetDateTime {
    t.exit_at.or(t.entry_at).unwrap_or(t.created_at)
}

/// Per-day realized-PnL buckets over the filtered trades, expressed in
/// `display_currency`. Read-only aggregation for the PnL calendar heatmap; shares
/// the FX-conversion and effective-date rules with `breakdown`. Days with no trade
/// are simply absent (the frontend fills the month grid). Closed trades whose rate
/// is missing count toward `trades` but not `net_pnl`/`wins`/`losses`.
///
/// `tz_offset_min` is the viewer's UTC offset in minutes as JS `getTimezoneOffset()`
/// reports it — i.e. minutes to add to local time to get UTC (New York DST = 240).
/// Trades are bucketed by their *local* effective day (UTC minus this offset), so the
/// grid matches what the Trades list shows when filtered by the same local date. FX
/// still prices on the UTC effective date (rates are daily; the shift is immaterial).
pub async fn calendar(
    ...
    let trades = list_trades(pool, filter).await?;
    let offset = time::Duration::minutes(tz_offset_min as i64);
    let mut fx = crate::journal_fx::FxCache::new();
    ...
    let mut days: BTreeMap<time::Date, Bucket> = BTreeMap::new();

    for t in &trades {
        // Bucket by the viewer's local calendar day; price FX on the UTC date.
        let local_date = (trade_effective_at(t) - offset).date();
        let fx_date = trade_effective_date(t);
        let b = days.entry(local_date).or_insert(Bucket {
            net: 0.0,
            trades: 0,
            wins: 0,
            losses: 0,
        ...
        b.trades += 1;
        let Some(native_net) = t.net_pnl else {
            continue; // still open — counts in `trades` only
        };
        // Closed: convert; a missing rate leaves it out of net/wins/losses.
        if let Some(net) =
            fx.convert(pool, native_net, &t.currency, display_currency, fx_date).await?
        {
            b.net += net;
            if net > 0.0 {
                b.wins += 1;
    ...
    Ok(days
        .into_iter()
        .map(|(date, b)| CalendarDay {
            date: date.format(fmt).unwrap_or_default(),
            net_pnl: round_dp(b.net, 6),
            trades: b.trades,
            wins: b.wins,
            losses: b.losses,
        })
