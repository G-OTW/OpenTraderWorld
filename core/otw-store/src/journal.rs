...
pub async fn breakdown(
    ...
    for (amount, currency, occurred_at, kind) in &cap_events {
            ...
            .await?
        {
            ...
            None => {
                // No rate for this date — the event is omitted from invested capital (which
                // skews return %), so queue the date for the FX job / manual entry, exactly
                // like an unconvertible trade.
                crate::journal_fx::mark_pending(
                    pool,
                    occurred_at.date(),
                    "needed to convert a capital event",
                )
                .await?;
            }
        }
