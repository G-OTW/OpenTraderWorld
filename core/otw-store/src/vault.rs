...
//!
//! Consumers (feed_secrets, histdata_provider_creds, broker_account_creds, agent_providers,
//! agent_mcp_servers, notif_channels, mailbox_accounts — password *and* OAuth refresh
//! token) reference an item by id. Deleting an item or a vault that is still referenced is
//! refused (`item_usage` / `vault_usage` pre-checks, FK as backstop).
...
const ITEM_USE_SQL: &str = "(SELECT count(*) FROM feed_secrets WHERE vault_item_id = vault_items.id) \
     ...
     + (SELECT count(*) FROM broker_account_creds WHERE vault_item_id = vault_items.id) \
     ...
     + (SELECT count(*) FROM notif_channels \
        WHERE config::text LIKE '%' || vault_items.id::text || '%') \
     + (SELECT count(*) FROM mailbox_accounts WHERE vault_item_id = vault_items.id) \
     + (SELECT count(*) FROM mailbox_accounts WHERE refresh_vault_item = vault_items.id)";
