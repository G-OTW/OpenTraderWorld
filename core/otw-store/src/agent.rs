...
pub struct Message {
    ...
    /// Prompt tokens written to the provider's cache this turn (billed above input rate).
    #[serde(default)]
    pub cache_write_tokens: i32,
    /// Prompt tokens served from that cache (billed at a fraction of the input rate).
    #[serde(default)]
...
const MSG_COLS: &str = "id, conversation_id, role, content, input_tokens, output_tokens, \
    cache_write_tokens, cache_read_tokens, created_at";

/// What produced a billed turn, stamped on the row. Model, provider and persona are all
/// switchable on the conversation, so reading them back through it would re-attribute the
/// whole history to the latest choice. Empty/None on every row that costs nothing.
#[derive(Debug, Clone, Default)]
pub struct MsgOrigin {
    pub model: String,
    pub provider_id: Option<Uuid>,
    pub agent_id: Option<Uuid>,
}

/// What one turn cost, as the provider reported it. A struct rather than four positional
/// arguments: the cache pair is meaningless without the other two, and three of the four
/// call sites record a message that cost nothing.
#[derive(Debug, Clone, Copy, Default)]
pub struct MsgTokens {
    pub input: i32,
    pub output: i32,
    pub cache_write: i32,
    pub cache_read: i32,
    ...
    sqlx::query(
        "INSERT INTO agent_messages \
         (id, conversation_id, role, content, input_tokens, output_tokens, \
          cache_write_tokens, cache_read_tokens, model, provider_id, agent_id) \
...
/// Sum of tokens used across a conversation (for the usage badge). Cache reads are counted
/// apart: they are prompt tokens the provider did not re-process, so folding them into the
/// input total would report a cost that was never paid.
pub async fn conversation_tokens(pool: &PgPool, conversation_id: Uuid) -> anyhow::Result<MsgTokens> {
    ...
    Ok(MsgTokens {
        input: row.0.unwrap_or(0) as i32,
        output: row.1.unwrap_or(0) as i32,
        cache_write: row.2.unwrap_or(0) as i32,
        cache_read: row.3.unwrap_or(0) as i32,
    })
}

/// The prompt size the provider actually billed on the most recent assistant turn, cache
/// reads included (they are prompt tokens: cheap, but they still occupy the window).
///
/// This is what the summarizer trips on. A chars/4 estimate over the stored messages misses
/// the system prompt, the tool schemas and the module catalog entirely, and mis-measures JSON
/// on top — it can be off by 3x on exactly the conversations that need compacting.
pub async fn last_prompt_tokens(
    pool: &PgPool,
    conversation_id: Uuid,
) -> anyhow::Result<Option<i32>> {
    let row: Option<(i32, i32)> = sqlx::query_as(
        "SELECT input_tokens, cache_read_tokens FROM agent_messages \
          WHERE conversation_id = $1 AND role = 'assistant' AND input_tokens > 0 \
          ORDER BY created_at DESC LIMIT 1",
    )
    .bind(conversation_id)
    .fetch_optional(pool)
    .await
    .context("reading last prompt size")?;
    Ok(row.map(|(input, cached)| input + cached))
}

// ── Usage breakdown ───────────────────────────────────────────────────────────
//
// Reads back what the agent actually spent, from the stamps `add_message` writes. Only
// assistant rows are billed: one row per provider turn, tool rounds included, which is why
// "requests" is their count and not the number of user messages.

/// One bucket or one dimension value of the breakdown. `key` identifies the row for the
/// client (a date, a model id, a uuid); `label` is what to print.
...
/// The whole report: a time series plus the same totals cut three ways.
#[derive(Debug, Clone, Serialize)]
pub struct UsageReport {
    pub series: Vec<UsageRow>,
    pub by_persona: Vec<UsageRow>,
    pub by_model: Vec<UsageRow>,
    pub by_provider: Vec<UsageRow>,
}

/// Raw aggregate shape shared by every grouping: (key, label, in, out, cache_write,
/// cache_read, requests).
type AggRow = (Option<String>, Option<String>, Option<i64>, Option<i64>, Option<i64>, Option<i64>, i64);

fn to_rows(rows: Vec<AggRow>, unknown: &str) -> Vec<UsageRow> {
    rows.into_iter()
        .map(|(key, label, i, o, cw, cr, n)| {
            ...
            UsageRow {
                ...
                cache_write_tokens: cw.unwrap_or(0),
                cache_read_tokens: cr.unwrap_or(0),
                requests: n,
            }
        })
        .collect()
...
/// The four sums, written once: every grouping totals the same columns.
const USAGE_SUMS: &str = "SUM(m.input_tokens), SUM(m.output_tokens), \
    SUM(m.cache_write_tokens), SUM(m.cache_read_tokens), COUNT(*)";

/// Usage since `from`, bucketed by day or hour in the caller's timezone.
///
/// `bucket` is an identifier interpolated into the SQL, so it is matched against a closed set
/// rather than passed through; `tz_minutes` is the client's offset from UTC, applied to the
/// timestamp before truncation so "today" means the user's today and not the server's.
pub async fn usage_breakdown(
    ...
    // Offsets beyond a day are not a timezone, they are a typo.
    let tz = tz_minutes.clamp(-14 * 60, 14 * 60);

    let series_sql = format!(
        "SELECT to_char(date_trunc('{unit}', m.created_at + make_interval(mins => $2)), \
                        'YYYY-MM-DD\"T\"HH24:MI'), \
                NULL::text, {USAGE_SUMS} \
           FROM agent_messages m \
          WHERE m.role = 'assistant' AND m.created_at >= $1 \
          GROUP BY 1 ORDER BY 1"
    );
    let series: Vec<AggRow> = sqlx::query_as(sqlx::AssertSqlSafe(series_sql))
        .bind(from)
        .bind(tz)
        .fetch_all(pool)
        .await
        .context("usage series")?;

    let persona_sql = format!(
        "SELECT COALESCE(m.agent_id::text, ''), a.name, {USAGE_SUMS} \
           FROM agent_messages m LEFT JOIN agent_agents a ON a.id = m.agent_id \
          WHERE m.role = 'assistant' AND m.created_at >= $1 \
          GROUP BY m.agent_id, a.name ORDER BY 7 DESC"
    );
    let by_persona: Vec<AggRow> = sqlx::query_as(sqlx::AssertSqlSafe(persona_sql))
        .bind(from)
        .fetch_all(pool)
        .await
        .context("usage by persona")?;

    // The model is a plain string: it is the id the provider was called with, and it stays
    // meaningful even for a model that has since been removed from the account.
    let model_sql = format!(
        "SELECT m.model, NULLIF(m.model, ''), {USAGE_SUMS} \
           FROM agent_messages m \
          WHERE m.role = 'assistant' AND m.created_at >= $1 \
          GROUP BY m.model ORDER BY 7 DESC"
    );
    let by_model: Vec<AggRow> = sqlx::query_as(sqlx::AssertSqlSafe(model_sql))
        .bind(from)
        .fetch_all(pool)
        .await
    ...
    let provider_sql = format!(
        "SELECT COALESCE(m.provider_id::text, ''), p.label, {USAGE_SUMS} \
           FROM agent_messages m LEFT JOIN agent_providers p ON p.id = m.provider_id \
          WHERE m.role = 'assistant' AND m.created_at >= $1 \
          GROUP BY m.provider_id, p.label ORDER BY 7 DESC"
    );
    let by_provider: Vec<AggRow> = sqlx::query_as(sqlx::AssertSqlSafe(provider_sql))
        .bind(from)
        .fetch_all(pool)
        .await
        .context("usage by provider")?;

    Ok(UsageReport {
        series: to_rows(series, ""),
        by_persona: to_rows(by_persona, unknown),
        by_model: to_rows(by_model, unknown),
        by_provider: to_rows(by_provider, unknown),
    })
}
...
pub struct Memory {
    ...
    #[serde(with = "rfc3339")]
    pub updated_at: OffsetDateTime,
    /// Last time the agent read it. NULL = written and never recalled since.
...
const MEMORY_COLS: &str = "m.id, m.slug, m.description, m.content, m.kind, m.agent_id, \
    a.name AS agent_name, m.created_at, m.updated_at, m.last_used_at";
...
///
/// Ordered newest-first and capped by the caller. The order is deliberately independent of
/// reads: it is the head of the cached prompt prefix, so ranking it by recency of USE would
/// reshuffle it on every run and throw away the cache it sits in.
...
/// Substring search over slug, description and body — what makes an index that no longer
/// lists every description still usable: the agent can find a memory it cannot see.
/// Case-insensitive, newest-first, caller-capped.
pub async fn search_memories(
    ...
    let rows: Vec<(String, String, Option<String>)> = sqlx::query_as(
        "SELECT m.slug, m.description, a.name FROM agent_memories m \
           LEFT JOIN agent_agents a ON a.id = m.agent_id \
          WHERE m.slug ILIKE $1 OR m.description ILIKE $1 OR m.content ILIKE $1 \
    ...
    .fetch_all(pool)
    .await
    .context("searching memories")?;
    Ok(rows)
}

/// Stamp a memory as read. Feeds the prune suggestion, nothing else: it must never reorder
/// the injected index (see [`memory_index`]).
pub async fn touch_memory(pool: &PgPool, slug: &str) -> anyhow::Result<()> {
    sqlx::query("UPDATE agent_memories SET last_used_at = now() WHERE slug = $1")
        .bind(slug)
        .execute(pool)
        .await
        .context("stamping memory use")?;
    Ok(())
}

/// The least recently useful memories: never read first, then longest unread. Named in the
/// "memory is full" error so the answer is a shortlist to confirm, not a dead end.
pub async fn stale_memories(pool: &PgPool, limit: i64) -> anyhow::Result<Vec<String>> {
    let rows: Vec<(String,)> = sqlx::query_as(
        "SELECT slug FROM agent_memories \
          ORDER BY last_used_at ASC NULLS FIRST, updated_at ASC LIMIT $1",
    )
    .bind(limit)
    .fetch_all(pool)
    .await
