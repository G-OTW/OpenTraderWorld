...
pub async fn list_tree(pool: &PgPool) -> anyhow::Result<Vec<DocumentMeta>> {
    let rows = sqlx::query_as::<_, (Uuid, Option<Uuid>, String, String, f64, Option<String>, Option<String>, OffsetDateTime)>(
        "SELECT id, parent_id, kind, title, position, icon, flag, updated_at \
    ...
    Ok(rows
        .into_iter()
        .map(|(id, parent_id, kind, title, position, icon, flag, updated_at)| DocumentMeta {
            id,
            parent_id,
            kind,
            title,
...
pub async fn get(pool: &PgPool, id: Uuid) -> anyhow::Result<Option<Document>> {
    let row = sqlx::query_as::<_, (Uuid, Option<Uuid>, String, String, Option<JsonValue>, f64, Option<String>, Option<String>, String, OffsetDateTime, OffsetDateTime)>(
        "SELECT id, parent_id, kind, title, content, position, icon, flag, layout, created_at, updated_at \
         FROM documents WHERE id = $1",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?
    .map(|(id, parent_id, kind, title, content, position, icon, flag, layout, created_at, updated_at)| Document {
        id,
        parent_id,
        kind,
        title,
...
/// Set (or clear, with `None`) a document's colour flag.
///
/// Its own statement rather than a field of `update`: clearing means writing
/// NULL, which the COALESCE-based patch there cannot express.
pub async fn set_flag(pool: &PgPool, id: Uuid, flag: Option<&str>) -> anyhow::Result<bool> {
    let res = sqlx::query("UPDATE documents SET flag = $2, updated_at = now() WHERE id = $1")
        .bind(id)
        .bind(flag)
        .execute(pool)
        .await
        .context("setting document flag")?;
    Ok(res.rows_affected() > 0)
}
